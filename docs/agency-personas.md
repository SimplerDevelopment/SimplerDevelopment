...
- **~21 roles are real, invokable agent files** in `.claude/agents/`. The rest of the org (the exec/advisory/design-strategy roles in §4) are **adopt-a-lens personas** (§6): the conductor role-plays them *in-context*, because a CEO/CTO/CMO call needs the whole conversation, which a detached subagent doesn't have. Any lens can be **promoted to a real agent file** the moment it needs to do discrete, repeatable work.
