...
#### Brand preset (the optional "brand preset" field)

The Button block's Content tab has an optional **brand preset** field. A brand
preset is a named, reusable set of button styles (colors, radius, border,
shadow) defined once on the site's **brand profile** (`buttonPresets`, authored
in **Portal → Branding**) so every button can opt into a consistent look by
name instead of re-styling each one by hand.

How it resolves, in order:

1. **Base variant** (`primary` / `secondary` / `outline`) sets the default look.
2. **Brand preset** — if a preset key is set, its styles apply *on top of* the
   variant. This is how you get site-wide consistency: change the preset on the
   brand profile and every button using it updates at once.
3. **Style tab overrides** — anything you set on this specific button's Style
   tab wins over both the variant and the preset. Use this for one-off tweaks.

Leave the preset field empty to use the plain variant styling. Presets are
optional — they exist purely to avoid restyling the same button dozens of times
across a site.
