...
The platform exposes a **Model Context Protocol (MCP)** endpoint at `POST /api/mcp` (Streamable HTTP). Under a wildcard scope, **473 tools** are available across all product domains. The tool count is locked by a baseline test (`tests/unit/mcp-tool-registry-baseline.test.ts`) — any drift fails the pre-push hook.
