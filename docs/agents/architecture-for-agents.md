...
| API / MCP surface | REST v1 + 473-tool MCP server for automation and AI-agent integrations |
...
| HNSW index on brain_embeddings | Managed manually via raw SQL (see the comment above `brainEmbeddings` in `lib/db/schema/brain.ts`), not a Drizzle migration file; `drizzle-kit push --force` silently drops it — never run `--force` against a DB with real brain data |
...
|---|---|---|---|
| MCP (Streamable HTTP) | `POST /api/mcp` | `sd_mcp_` or `sd_oauth_` bearer | 473 tools; scope-gated; approval-link pattern for writes |
...
The MCP server (`lib/mcp/server.ts`) exposes **473 tools** locked by `tests/unit/mcp-tool-registry-baseline.test.ts`. Drift (tool added/removed/renamed without updating `EXPECTED_TOOLS`) fails pre-push CI.
