...
| API / MCP surface | REST v1 + 477-tool MCP server for automation and AI-agent integrations |
...
|---|---|---|---|
| MCP (Streamable HTTP) | `POST /api/mcp` | `sd_mcp_` or `sd_oauth_` bearer | 477 tools; scope-gated; approval-link pattern for writes |
...
The MCP server (`lib/mcp/server.ts`) exposes **477 tools** locked by `tests/unit/mcp-tool-registry-baseline.test.ts`. Drift (tool added/removed/renamed without updating `EXPECTED_TOOLS`) fails pre-push CI.
