...
export default function PreviewModal({
  ...
  // View mode: "single" shows the current tint per surface (the default
  // since PR-72), "all" composites the layers-only snapshot onto every
  // tint at once so the customer can spot the best shirt colour at a
  // glance. Defaults to single so the modal stays familiar.
  const [viewMode, setViewMode] = useState<'single' | 'all'>('single');
  // Layers-only PNG keyed by surface slug — captured once when the
  // customer flips to the "all colours" view. Re-used across every tint
  // tile via CSS compositing (mix-blend-mode: multiply) so we don't pay
  // an extra canvas roundtrip for every (surface × tint) combination.
  const [layersOverlay, setLayersOverlay] = useState<Record<string, string>>({});
  ...
  // to the multi-tint grid. Snapshots each surface's layers on a
  // transparent backdrop (mockup background, print-area, guides all
  // temporarily hidden) so each tile can composite the same overlay over
  // a CSS-tinted mockup. We accept that per-tint text/icon fill overrides
  // won't reflect in the grid (those need an actual canvas re-render);
  // the customer can click any tile to drop into the single view and see
  // the per-tint adjusted version.
  useEffect(() => {
    if (!open || !canvas || viewMode !== 'all') return;
    let cancelled = false;
    const originalSurface = activeSurface;

    const captureLayersOnly = (slug: string): Promise<string> =>
      new Promise((resolve) => {
        setActiveSurface(slug);
        // Same polling pattern as the single-tile capture above.
        const expectedLayerCount =
          useCanvasStore.getState().layersBySurface[slug]?.length ?? 0;
        const POLL_MS = 60;
        const MAX_POLLS = 50;
        ...
        const captureNow = () => {
          try {
            ...
            canvas.getObjects().forEach((obj) => {
              ...
              if (
                tagged.id === 'designer-canvas-background' ||
    ...
    (async () => {
      ...
      for (const s of surfaces) {
        ...
        if (!layersBySurface[s.slug]?.length) continue;
        // eslint-disable-next-line no-await-in-loop
        next[s.slug] = await captureLayersOnly(s.slug);
      }
      if (!cancelled) {
        setLayersOverlay(next);
        setActiveSurface(originalSurface);
      }
    })();
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl bg-background border border-border shadow-2xl">
        ...
        {/* Tint swatch strip + view-mode toggle — lets the customer flip
            the preview through every shirt colour one-at-a-time, or jump
            to the side-by-side grid of every colour at once. */}
        <div className="px-5 pt-3 pb-2 border-b border-border flex flex-wrap items-center gap-2">
            ...
            {TINT_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setMockupTint(opt.hex);
                    setViewMode('single');
                  }}
                  aria-pressed={active && viewMode === 'single'}
                  title={opt.label}
          ...
          <button
            type="button"
            onClick={() =>
              setViewMode((m) => (m === 'all' ? 'single' : 'all'))
            }
            aria-pressed={viewMode === 'all'}
            className={`ml-auto inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs border transition-colors ${
              viewMode === 'all'
                ? 'border-primary bg-primary/10 text-primary'
                : 'border-border bg-background text-foreground hover:bg-muted'
            }`}
            title="Side-by-side every shirt colour"
          >
        ...
        {viewMode === 'single' ? (
          ...
          // All-colours grid — every tint applied to the same layers-only
          // overlay via CSS mix-blend-mode: multiply. Each tile uses the
          // active surface only, since cramming every (surface × tint)
          // combination would scroll forever. The customer can switch
          // surfaces with the existing surface selector before opening
          // the modal, and clicking any tile flips them into the single
          // view at that tint so per-tint text overrides become visible.
          <div className="p-5 space-y-3">
            {(() => {
              const surface =
                surfaces.find((s) => s.slug === activeSurface) ?? surfaces[0];
              if (!surface) return null;
              const overlay = layersOverlay[surface.slug];
              const empty = !layersBySurface[surface.slug]?.length;
              if (empty) {
                return (
                  <div className="text-sm text-muted-foreground text-center py-12">
                    This surface is empty — add a layer to see it on every shirt
                    colour.
                  </div>
                );
              }
              if (!overlay) {
                return (
                  <div className="flex items-center justify-center py-12 gap-2 text-sm text-muted-foreground">
                    ...
                    Capturing {surface.name} on every colour…
                  </div>
                );
              }
              return (
                <>
                  <p className="text-xs text-muted-foreground">
                    Showing <span className="font-medium">{surface.name}</span>{' '}
                    on every shirt colour. Click any tile to fine-tune.
                  </p>
                    ...
                    {TINT_OPTIONS.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className="group rounded-md border border-border overflow-hidden bg-muted/30 hover:border-primary transition-colors text-left"
                          title={`See ${opt.label} full size`}
                        >
                          <div
                            className="relative aspect-square bg-white"
                            style={{ isolation: 'isolate' }}
                          >
                            {/* Mockup background — same image fabric loads. */}
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                              src={surface.mockupImage}
                              alt=""
                              className="absolute inset-0 w-full h-full object-contain"
                              crossOrigin="anonymous"
                            />
                            {/* Multiply blend matches Fabric BlendColor multiply. */}
                            {!isNone && (
                              <div
                                className="absolute inset-0 pointer-events-none"
                                style={{
                                  backgroundColor: opt.hex ?? undefined,
                                  mixBlendMode: 'multiply',
                                }}
                              />
                            )}
                            {/* Design layers on top — captured transparent. */}
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                              src={overlay}
                              alt={`${surface.name} on ${opt.label}`}
                              className="absolute inset-0 w-full h-full object-contain"
                            />
                          </div>
                          <div className="px-2 py-1.5 flex items-center gap-2 border-t border-border">
                            <span
