...
export default function ServiceRequestForm({ serviceId, serviceName, serviceDescription, surveyFields }: Props) {
  ...
  // Count answered required fields for progress
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!data.success) {
        ...
        router.push('/portal/services?requested=1');
      }
    } catch {
      setLoading(false);
        ...
        <div className="ml-9">
          ...
          {field.type === 'rating' && (
            <div className="flex items-center gap-1">
              ...
              {val && (
                <span className="text-sm text-muted-foreground ml-2 font-medium">{val} / 5</span>
              )}
            </div>
          )}
        </div>
    ...
    <div className="pb-12">
      ...
      <div>
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
          <Link href="/portal/services" className="hover:text-foreground transition-colors">Services</Link>
          <span className="material-icons text-sm">chevron_right</span>
          <span className="text-foreground">Request</span>
        </div>
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center flex-shrink-0">
            <span className="material-icons text-xl text-primary">assignment</span>
          </div>
          <div>
            <h1 className="text-2xl font-bold text-foreground">{serviceName}</h1>
            {serviceDescription && (
              <p className="mt-1 text-muted-foreground text-sm">{serviceDescription}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
            ...
            {hasSurvey ? (
              ...
              <div className="group">
                ...
                <textarea
                  ...
                  placeholder="Describe what you need, your timeline, any specific requirements..."
          ...
          <div className="border-t border-border px-6 py-4 bg-muted/30 flex items-center justify-between gap-4">
            <Link href="/portal/services" className="text-sm text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-6 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:bg-primary/90 disabled:opacity-50 transition-colors shadow-sm"
            >
              {loading ? (
