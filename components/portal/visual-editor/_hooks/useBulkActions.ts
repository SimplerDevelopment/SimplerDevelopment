...
import { findBlockById, removeBlockById, deepCloneBlock, insertBlockAfter } from '@/lib/utils/blockHelpers';
...
export function useBulkActions({
  ...
  const bulkDuplicate = useCallback(() => {
    // Use the recursive, container-aware helpers so duplicating a block that
    // lives inside a columns/tabs/section still works. The previous flat
    // `updated.findIndex(...)` only matched top-level blocks, so it silently
    // no-op'd for any nested block. deepCloneBlock regenerates ids recursively.
    let updated = blocks;
    const newIds: string[] = [];
    for (const id of selectedBlockIds) {
      ...
      if (block) {
        ...
        updated = insertBlockAfter(updated, id, dup);
        newIds.push(dup.id);
      }
    }
