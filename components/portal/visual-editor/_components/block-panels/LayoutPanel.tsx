...
export function LayoutPanel({ block, onUpdate }: PanelProps) {
  ...
  return (
    <>
      ...
      {block.type === 'section' && (
        <>
          {/* All section style fields — Background Color/Image, Text Color,
              Max Width and Font Family — now live on the Style tab. The renderer
              reads block.style / block.responsiveStyle for them (Style overrides
              the legacy top-level props, so existing sections still render).
              HTML Tag stays here — it's structural (which element the section
              renders as), not a CSS style. */}
