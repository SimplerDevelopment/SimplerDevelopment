...
interface PortalPostFormProps {
  ...
  siteDomain?: string;
  /**
   * clientWebsites.publicAccess for this site — false means the site is
   * still walled (noindex/coming-soon) even after a post is published.
   * Read-only signal threaded down to PostEditorLayout for a Publish hint.
   */
...
function PortalPostFormInner({
  ...
  const { ydoc, awareness, peers, status: realtimeStatus, setCursor } = useCollaboration();
  ...
  } = usePostForm({ siteId, post, mode, editorMode, ydoc, realtimeStatus });
