...
import SidebarCreditBadge from './SidebarCreditBadge';
...
export default function PortalSidebar({
  ...
  const isPortalClient = useSession().data?.user?.role === 'client'; // QAD-036: staff have no portal client
  ...
  // Poll pending MCP-approvals count (every 60s) — portal (tenant) sessions only.
  useEffect(() => {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Footer: AI credits + theme cycle + sign out */}
        <div className="border-t border-border p-2.5 space-y-0.5 shrink-0">
          <SidebarCreditBadge collapsed={collapsed} onNavigate={onCloseMobile} />
          <button
            onClick={cycleTheme}
          ...
          <button
            onClick={async () => { await fetch('/api/portal/sign-out', { method: 'POST' }); await signOut({ callbackUrl: '/portal/login' }); }}
