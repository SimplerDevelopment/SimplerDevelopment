...
export default function WidgetShell({
  ...
  return (
    ...
    >
      ...
      {/* Body — p-3 gives every widget's content consistent inset on all sides
          (left/right so it lines up with the title, top so it doesn't hug the
          header border, bottom so it doesn't hug the card edge). Widgets that
          need edge-to-edge rows (none today) would opt out with a negative
          margin. */}
      {!collapsed && <div className="flex-1 p-3">{children}</div>}
    </div>
  );
}
