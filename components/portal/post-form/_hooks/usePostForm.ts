...
interface UsePostFormArgs {
  ...
  ydoc?: Y.Doc | null;
  /**
   * Live realtime connection status. The ydoc exists as a local object even
   * when the websocket never connects (token 503 / server down), so autosave
   * must gate on this — not on ydoc's existence — or unsaved edits are
   * silently lost while realtime is unavailable.
   */
 ...
 */
export function usePostForm({ siteId, post, mode, editorMode, ydoc, realtimeStatus }: UsePostFormArgs): UsePostFormReturn {
  ...
  // Read connection status via a ref: the realtime client's retry loop flaps
  // connecting↔disconnected while the server is down, and keeping status in
  // the effect deps meant every flap re-ran the effect, whose cleanup
  // cancelled the pending debounce — autosave never fired.
  ...
  useEffect(() => {
    ...
    // Realtime owns persistence only while actually CONNECTED. The ydoc is a
    // local object that exists even when the socket never connects, so gating
    // on it alone silently dropped every edit whenever the realtime service
    // was down (QAD-004). While disconnected, REST autosave is the safety
    // net; status is re-checked at fire time. ponytail: right after a
    // reconnect the server snapshot and one REST autosave can both persist —
    // acceptable vs. data loss; revisit if realtime gains offline merge.
    if (ydoc && realtimeStatusRef.current === 'connected') return;
