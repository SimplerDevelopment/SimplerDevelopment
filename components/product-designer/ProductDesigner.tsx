...
export interface ProductDesignerProps {
  ...
  /** When true, DesignApi attaches the x-portal-staff header to its save/load
   *  calls so the API's staff gate authorizes via the portal session — lets
   *  staff edit/read designs with no customerId/sessionId owner. */
  staffMode?: boolean;
}
export const ProductDesigner: React.FC<ProductDesignerProps> = ({
  ...
  React.useEffect(() => {
    ...
    DesignApi.staffMode = !!staffMode;
  }, [apiBaseUrl, websiteId, staffMode]);
...
setControlMode,
  setLoadModalOpen, userId, product, setStyle, style, setCarouselMode, setHoveredStyleId, setHoveredStyleIndex, carouselMode, hoveredStyleId, selectedLayer, updateLayer, removeLayer, addLayer, setSelectedLayer, setSide, layerControlsStyle, lastClickedCarouselStyle, cartMode, setCartMode, CartContext
        ...
        {saveError && (
          <div
            role="alert"
            className="absolute top-14 left-1/2 -translate-x-1/2 z-40 rounded-lg bg-red-600 text-white text-sm px-3 py-1.5"
          >
