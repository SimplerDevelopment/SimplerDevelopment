  ...
  return (
    <div className={`py-4 my-6 ${responsiveClasses}`}>
      <div className={`rounded-lg ${style.backgroundColor ? '' : 'bg-slate-900 dark:bg-slate-950'} overflow-hidden`}>
        {block.language && (
          <div className="px-4 py-2 text-xs text-slate-400 border-b border-slate-700 bg-slate-800" style={getElementCSS(block.elementStyles, 'titleBar')}>
            {block.language}
          </div>
        )}
        <pre className="p-4 overflow-x-auto" style={getElementCSS(block.elementStyles, 'body')}>
          <code className={`text-sm ${style.color ? '' : 'text-slate-100'} font-mono`}>{block.code}</code>
        </pre>
      </div>
    </div>
  );
