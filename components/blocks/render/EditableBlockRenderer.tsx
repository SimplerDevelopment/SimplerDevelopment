...
function DraggableBlockList({
  ...
  // External drag from parent block picker — hit-testing lives in externalDropTarget.ts.
  useEffect(() => {
    if (!editor.externalDrag.active) {
      void Promise.resolve().then(() => setExternalDropTarget(null));
      return;
    }
    const container = contentRef.current;
    if (!container) return;
    setExternalDropTarget(findExternalDropTarget(container, editor.externalDrag.x, editor.externalDrag.y, blocks.length));
  }, [editor.externalDrag.active, editor.externalDrag.x, editor.externalDrag.y, blocks.length]);

  // Handle external drop event
  useEffect(() => {
    const handleDrop = () => {
      const blockType = editor.externalDrag.blockType;
      if (!blockType || !externalDropTarget) return;
      const newBlock = createExternalDropBlock(blockType, externalDropTarget.kind === 'top-level' ? externalDropTarget.index : 0);
      const updated = applyExternalDrop(blocks, externalDropTarget, newBlock);
  ...
  }, [blocks, editor, externalDropTarget]);
      ...
      <SortableContext items={ids} strategy={noMovementStrategy}>
        <div className="block-content" ref={contentRef} onClick={(e) => {
          ...
          {blocks.map((block, i) => {
            ...
            return (
            <div key={reactKey} data-toplevel-slot={i}>
              {/* External drop indicator before this block */}
              {editor.externalDrag.active && externalDropTarget?.kind === 'top-level' && externalDropTarget.index === i && (
              ...
              <SortableBlock
                ...
                isForceHover={!!block.id && editor.forceHoverBlockId === block.id}
              ...
              {i === blocks.length - 1 && (
                <>
                  ...
                  {editor.externalDrag.active && externalDropTarget?.kind === 'top-level' && externalDropTarget.index === blocks.length && (
                    <ExternalDropIndicator />
                  )}
                </>
              )}
...
function ContainerSlotDropZone({
  ...
  slotIndex: number;
  hasChildren: boolean;
  /** Palette drag hovering this slot — see findExternalDropTarget (externalDropTarget.ts). */
  isExternalOver?: boolean;
}) {
  const { setNodeRef, isOver } = useDroppable({ id: `container:${containerId}:${slotIndex}` });
  const active = isOver || !!isExternalOver;

  return (
    <div
      ref={setNodeRef}
      data-container-drop-id={`${containerId}:${slotIndex}`}
    ...
    >
      {active ? '+ Drop here' : hasChildren ? '' : 'Drag a block here, or use the "Add Block" panel to insert one'}
    </div>
  );
}
...
function SortableBlock({
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      ...
      >
        {isContainer ? (
          <ContainerBlockRenderer block={liveBlock} registry={registry} draggingId={draggingId} editor={editor} externalDropTarget={externalDropTarget} />
        ) : (
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      ...
      <SelectableBlock
        ...
        isForceHover={editor.forceHoverBlockId === block.id}
        onClicked={editor.onBlockClicked}
        onHovered={editor.onBlockHovered}
        onAddAfter={editor.onAddBlockAfter}
        onResize={editor.onBlockResized}
...
function ContainerBlockRenderer({
            ...
            return (
              ...
              >
                ...
                <ContainerSlotDropZone
                  ...
                  isExternalOver={externalDropTarget?.kind === 'container' && externalDropTarget.containerId === block.id && externalDropTarget.slotIndex === i}
                />
              </div>
            );
          })}
  ...
  if (block.type === 'tabs') {
    return (
      <BlockStyleWrapper block={block}>
        <TabsContainerEditor block={block} registry={registry} draggingId={draggingId} editor={editor} externalDropTarget={externalDropTarget} />
      </BlockStyleWrapper>
    );
  }
  ...
  if (block.type === 'section') {
    ...
    const sectionOuterStyle: React.CSSProperties = {
      ...
      ...bgImageStyle,
      ...(color ? { color } : {}),
      // Style-tab Font Family (block.style.fontFamily) — mirrors SectionBlockRender.
      ...(s?.fontFamily ? { fontFamily: s.fontFamily } : {}),
      padding,
      ...(s?.margin ? { margin: s.margin } : {}),
      ...(s?.borderWidth ? { borderWidth: s.borderWidth } : {}), // Border
      ...
      // Clip on the bordered element so bg respects a rounded border — mirrors
      // SectionBlockRender (honor explicit overflow; default hidden if rounded).
      ...(s?.overflow ? { overflow: s.overflow } : (s?.borderRadius ? { overflow: 'hidden' } : {})),
    };
    // Layout controls write block.responsiveStyle.desktop (not flat block.style);
    // merge desktop-responsive over the flat style so flex-direction/gap set in
    // the panel actually reach the children container — mirrors SectionBlockRender.
      ...
      <div className="p-4 bg-card min-h-[120px]">
        ...
        {activeTab && (
          <ContainerSlotDropZone
            ...
            isExternalOver={externalDropTarget?.kind === 'container' && externalDropTarget.containerId === block.id && externalDropTarget.slotIndex === activeIndex}
          />
        )}
      </div>
    </div>
