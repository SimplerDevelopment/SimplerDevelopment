...
export function BookingFormInline({
  ...
  // Land the calendar on the first month with availability so visitors don't
  // hunt through empty months. Never selects a date or changes the step — the
  // choice stays with the visitor (smart default, not auto-advance).
  useEffect(() => {
    if (!pageInfo || selectedDate) return;
    for (let i = 0; i < 60; i++) {
