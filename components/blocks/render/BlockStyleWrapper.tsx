...
export function BlockStyleWrapper({ block, children }: BlockStyleWrapperProps) {
  ...
  // `selfStyled` blocks render their own border/shadow/opacity on an inner
  // element, so the full-width wrapper must not paint them. The button block
  // additionally renders its own background on its narrow inner <Link>
  // (see ButtonBlockRender) — painting bg on this wrapper would color the whole
  // block band and the border would appear twice. A section's wrapper, by
  // contrast, legitimately *is* its background, so it keeps wrapper bg.
  const selfStyled = block.type === 'section' || block.type === 'button';

  if (style.backgroundColor && block.type !== 'button') customStyles.backgroundColor = r(style.backgroundColor);
