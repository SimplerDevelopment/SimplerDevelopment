...
export function MarqueeBlockPreview({ block, isSelected }: MarqueeBlockPreviewProps) {
  ...
  const isVertical = block.direction === 'up' || block.direction === 'down';
  ...
  return (
    <div
      className={isVertical ? 'marquee-vertical' : undefined}
      data-marquee-vertical={isVertical ? 'true' : undefined}
      style={{ overflow: 'hidden', position: 'relative', height: isVertical ? (block.height || '300px') : undefined }}
    >
      {/* Preview intentionally does not animate (see note above) — layout axis
          still switches to a column for up/down so the editor canvas reads as
          a vertical scroller, matching the production custom track. */}
      <div
        style={
          isVertical
            ? { display: 'flex', flexDirection: 'column', alignItems: 'center', gap, padding: '8px 0' }
            : { display: 'flex', alignItems: 'center', gap, padding: '8px 0' }
        }
      >
        {items.map((item) => (
