...
function parseGradient(css: string): BuilderState | null {
  ...
  state.stops = stopParts
    .map((part, i) => {
      ...
      // Stable, index-derived id (not Date.now()) so the same stop keeps the
      // same React key across re-parses of the same gradient string — a
      // Date.now()-based id here caused the range input to remount on every
      // parse, making a dragged stop appear to "stick"/jump (VEQA-019).
      return { id: `s-${i}`, color, position };
    })
...
export function GradientBuilder({ backgroundColor, backgroundGradient, onChange }: BackgroundColorsControlProps) {
  ...
  // Tracks the last `backgroundGradient` string *this component* emitted via
  // onChange, so the effect below can tell a real external prop change apart
  // from an echo of our own update round-tripping back through the parent —
  // reparsing an echo would mint fresh stop ids and remount the drag handles
  // mid-drag (VEQA-019).
  const lastEmittedRef = useRef<string | null>(null);

  useEffect(() => {
    const isEcho = !!backgroundGradient && backgroundGradient === lastEmittedRef.current;
    if (!isEcho) {
  ...
  const emit = (next: BuilderState) => {
    ...
    if (next.stops.length === 0) {
      ...
      const gradient = buildGradient(next);
      lastEmittedRef.current = gradient;
      onChange({ backgroundColor: '', backgroundGradient: gradient });
    }
  };
  ...
  const addStop = () => {
    if (state.stops.length === 0) {
      emit({ ...state, stops: [{ id: crypto.randomUUID(), color: '#ffffff', position: 0 }] });
      return;
    }
    if (state.stops.length === 1) {
      ...
      emit({
        ...
        stops: distributeEvenly([
          { ...first, position: 0 },
          { id: crypto.randomUUID(), color: '#ffffff', position: 100 },
        ]),
      });
      return;
    }
    ...
    emit({
      ...state,
      stops: [...state.stops, { id: crypto.randomUUID(), color: last.color, position: newPos }],
    });
  };
