...
const SPACER_PRESETS = ['sm', 'md', 'lg', 'xl'] as const;
...
function SpacerBlockSettings({ block, onChange, currentViewport }: { block: SpacerBlock; onChange: (updates: Partial<SpacerBlock>) => void; currentViewport: Breakpoint }) {
  // VEQA-058: a height that isn't one of the four presets is a raw CSS length —
  // show the select as "Custom" and reveal a length input for it.
  const isCustom = !SPACER_PRESETS.includes(block.height as (typeof SPACER_PRESETS)[number]);
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">Height</label>
        <select
          value={isCustom ? 'custom' : block.height}
          onChange={(e) =>
            onChange({ height: e.target.value === 'custom' ? '40px' : (e.target.value as SpacerBlock['height']) })
          }
          className="w-full text-sm rounded border border-border bg-background px-3 py-2 text-foreground"
        >
        ...
        {isCustom && (
          <input
            ...
            placeholder="e.g. 40px, 5rem, 10vh"
            className="mt-2 w-full text-sm rounded border border-border bg-background px-3 py-2 text-foreground"
          />
        )}
      </div>
    </div>
