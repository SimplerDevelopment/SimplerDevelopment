...
- `components/blocks/visual/block-settings/panels/SectionsPanel.tsx` (1258) — covers hero, CTA, services-grid, stats, testimonial, social-links, logo-strip, metric-cards, flip-card-grid, timeline, team, bento-grid, and site-footer inline. A targeted `Read` with `limit:` + `offset:` is acceptable for surgical edits.
