...
import { sessionForNewClientUser, sessionForStaff, grantService, type TenantCtx } from '../../../helpers/session';
...
describe('Tenancy @tenancy @security', () => {
  ...
  beforeEach(async () => {
    ...
    // CRM writes are entitlement-gated (crmEntitlementError). Grant both
    // tenants the module so these tests exercise the ownership checks, not
    // the paid-module gate.
    await Promise.all([grantService(A.client.id, 'crm'), grantService(B.client.id, 'crm')]);
  });
