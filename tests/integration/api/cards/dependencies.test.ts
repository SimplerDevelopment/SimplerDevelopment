...
async function seedTwoCards(client: TenantCtx, opts?: { clientRole?: ProjectRole }) {
  ...
  const [proj] = await sql<{ id: number }[]>`
    ...
    VALUES ('Dep project', ${client.client.id}, 'active', ${client.user.id})
    RETURNING id
  `;
  await sql`
...
describe('POST /api/portal/cards/[id]/dependencies @cards @dependencies', () => {
  ...
  it('403 client on agency project', async () => {
    const { c1, c2 } = await seedTwoCards(A, { clientRole: 'viewer' });
    mockedAuth.mockResolvedValue(A.session);
    const route = await import('@/app/api/portal/cards/[id]/dependencies/route');
    const res = await callHandler(
      route as unknown as Record<string, unknown>, 'POST',
