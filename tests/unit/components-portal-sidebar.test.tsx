...
// scenario in this file, which exercises the sidebar as a logged-in client.
// QAD-036 gates the approvals poll on role === 'client'; tests that need a
// staff/admin session (no poll) override this per-test via mockSessionRole.
const mockSessionRole = { value: 'client' as string | null };

vi.mock('next-auth/react', () => ({
  signOut: vi.fn(() => Promise.resolve()),
  useSession: () => ({
    data: mockSessionRole.value ? { user: { role: mockSessionRole.value } } : null,
    status: mockSessionRole.value ? 'authenticated' : 'unauthenticated',
  }),
}));
...
// Stub CreditBalance — it fetches its own credit balance on mount (VEQA-003);
// the sidebar test should not exercise that async fetch.
vi.mock('@/components/portal/CreditBalance', () => ({
  default: () => null,
}));
...
describe('PortalSidebar — AI credits badge (footer)', () => {
  it('renders the compact AI Credits element linking to the AI settings page when the client has credits', async () => {
    global.fetch = vi.fn((url: string) => {
      if (typeof url === 'string' && url.includes('/api/portal/credits')) {
        return makeFetchOk({ balance: 12_000, monthlyGrant: 50_000, payAsYouGo: false, monthlyUsage: 0, packages: [] });
      }
      if (typeof url === 'string' && url.includes('/api/portal/approvals')) {
        return makeFetchOk({ success: true, data: { count: 0 } });
      }
    ...
    await waitFor(() => {
      const link = screen.getByText('AI Credits').closest('a');
  ...
  it('hides the credits element when the client has no AI services', async () => {
    // Default setupFetch returns a body without balance/monthlyGrant → treated as 0/0.
    render(<PortalSidebar mobileOpen={true} />);
    await waitFor(() => {
      expect((global.fetch as any).mock.calls.some(
        (c: any[]) => c[0].includes('/api/portal/credits'),
      )).toBe(true);
    });
