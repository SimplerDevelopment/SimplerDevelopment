...
describe('VisualEditorShell — outgoing iframe messages', () => {
  ...
  it('JUL9-011 — leading edge: the first blocks change of a new burst reaches the iframe immediately (no debounce wait); a second rapid change coalesces into one trailing send', async () => {
    const props = baseProps();
    const { rerender } = render(<VisualEditorShell {...props} />);
    // Let the mount's own leading-edge send + its ~16ms quiet window close
    // so the next change below starts a fresh burst.
    await waitFor(() => expect(sendBlocksUpdateSpy).toHaveBeenCalledTimes(1));
    await new Promise((resolve) => setTimeout(resolve, 20));
    sendBlocksUpdateSpy.mockClear();

    const afterFirstEdit = [...props.blocks, block('c', 'text', { content: 'first' })];
    rerender(<VisualEditorShell {...props} blocks={afterFirstEdit} />);
    // Optimistic/leading-edge send — applied synchronously on this render,
    // not after waiting out the coalescing window (JUL9-011).
    expect(sendBlocksUpdateSpy).toHaveBeenCalledTimes(1);
    expect(sendBlocksUpdateSpy).toHaveBeenLastCalledWith(afterFirstEdit, { coalesce: false });

    const afterSecondEdit = [...afterFirstEdit, block('d', 'text', { content: 'second' })];
    rerender(<VisualEditorShell {...props} blocks={afterSecondEdit} />);
    // Still inside the quiet window opened by the leading send — must NOT
    // go out synchronously, or a rapid burst (drag/fast typing) would flood
    // postMessage with one call per change.
    expect(sendBlocksUpdateSpy).toHaveBeenCalledTimes(1);

    // ...but it does flush once the trailing window elapses, carrying the
    // latest value — so the burst's final state always lands.
    await waitFor(() =>
      expect(sendBlocksUpdateSpy).toHaveBeenLastCalledWith(afterSecondEdit, { coalesce: false }),
    );
    expect(sendBlocksUpdateSpy).toHaveBeenCalledTimes(2);
  });
