...
describe('registerAutomationsTools — tool registration', () => {
  ...
  it('registers only write tools when scopes=automations:write', () => {
    ...
    // automations_delete now requires the granular automations:delete scope.
    expect(tools.has('automations_delete')).toBe(false);
  });

  it('registers automations_delete only when scopes=automations:delete', () => {
    const tools = registerAll(['automations:delete']);
