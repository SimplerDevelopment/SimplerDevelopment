...
function defaultFetch(url: string, init?: any): any {
  ...
  if (url.startsWith('/api/portal/crm/activities?contactId=')) {
    return jsonResponse({ success: true, data: { activities: baseActivities, total: baseActivities.length, page: 1, limit: 20 } });
  }
...
describe('CrmContactDetailPage', () => {
  ...
  describe('contact info section', () => {
    ...
    it('renders custom fields panel with contact entity', async () => {
      const { container } = await renderPage();
      // QAD-027: Custom Fields moved behind its own tab in the CRM
      // consolidation (previously always visible in the info column) —
      // switch tabs before asserting the panel is rendered.
      fireEvent.click(
        Array.from(container.querySelectorAll('button')).find(
          b => b.textContent?.includes('Custom Fields')
        )!
      );
  ...
  describe('notes section', () => {
    // QAD-027: the standalone Notes textarea (auto-save on blur) was removed
    // from this page in the pre-campaign CRM consolidation — contact notes
    // are no longer surfaced/edited inline here. Coverage for the removed
    // feature is dropped rather than asserting on UI that no longer exists.
    it.skip('renders existing notes value', async () => {
      const { container } = await renderPage();
    ...
    it.skip('updates notes textarea on change', async () => {
      const { container } = await renderPage();
    ...
    it.skip('saves notes on blur', async () => {
      const fetchSpy = vi.fn((url: string, init?: any) =>
        Promise.resolve(handlers[0](url, init))
      );
      // @ts-ignore
  ...
  describe('deals section', () => {
    // QAD-027: deals now live behind their own tab (redesign moved them out
    // of the always-visible layout) — switch to the Deals tab before
    // asserting on deal content.
    function openDealsTab(container: HTMLElement) {
      fireEvent.click(
        Array.from(container.querySelectorAll('button')).find(
