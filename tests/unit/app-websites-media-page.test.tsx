...
/**
 * Unit tests for `app/portal/websites/[siteId]/media/page.tsx` — the
 * Website-scoped Portal Media Library page. Covers filter UI, search,
 * pagination, upload flow (incl. drag-and-drop), detail modal (edit
 * metadata, delete, copy URL, replace file), and the version-history
 * flyout.
 *
 * useParams is mocked to return a fixed siteId, fetch, FileReader,
 * navigator.clipboard, window.confirm, and window.alert are all stubbed.
 */
...
vi.mock('next/navigation', () => ({
  ...
  usePathname: () => '/portal/websites/42/media',
...
beforeEach(() => {
  ...
  // FileReader — calls onload with a data URL after a microtask.
...
// Import after mocks
import WebsiteMediaPage from '@/app/portal/websites/[siteId]/media/page';

function renderPage() {
  return render(<WebsiteMediaPage />);
}

// ─── Top-level shell ────────────────────────────────────────────────────────

describe('WebsiteMediaPage — shell + header', () => {
  ...
  it('scopes its list fetch under /api/portal/cms/websites/<siteId>/media', async () => {
    renderPage();
    await waitFor(() => {
      const calls = fetchMock.mock.calls.map(c => String(c[0]));
      expect(calls.some(u => u.includes('/api/portal/cms/websites/42/media'))).toBe(true);
    });
  });
...
describe('WebsiteMediaPage — filters', () => {
...
describe('WebsiteMediaPage — grid', () => {
  ...
  it('renders the branding profile name when set', async () => {
    fetchMock.mockImplementation(async () => makeRes({
      ...
      pagination: { total: 1 },
      brandingProfiles: [{ id: 1, name: 'Globex' }],
    }));
    const { container } = renderPage();
...
describe('WebsiteMediaPage — pagination', () => {
  ...
  it('Previous clamps to zero after going forward then back', async () => {
    fetchMock.mockImplementation(async () => makeRes({
      success: true,
      data: [makeItem(1)],
      pagination: { total: 50 },
...
describe('WebsiteMediaPage — upload modal', () => {
  ...
  it('uploads a file successfully and closes the modal', async () => {
    ...
    await waitFor(() => {
      ...
      expect(uploadUrl).toContain('/api/portal/cms/websites/42/media/upload');
      expect(container.textContent).not.toContain('Upload Media');
    });
  });
  ...
  it('clicking the dropzone is a no-op visible side-effect', async () => {
    const { container } = renderPage();
    await waitFor(() => expect(container.textContent).toContain('Media Library'));
    const uploadBtn = Array.from(container.querySelectorAll('button')).find(b =>
      b.textContent?.includes('Upload') && b.querySelector('.material-icons'),
...
describe('WebsiteMediaPage — detail modal', () => {
  ...
  it('Saves metadata via PUT and exits edit mode', async () => {
    ...
    await waitFor(() => {
      ...
      expect(putUrl).toContain('/api/portal/cms/websites/42/media/1');
    });
  });
  ...
  it('Delete confirms and DELETEs', async () => {
    ...
    await waitFor(() => {
      ...
      expect(deleteUrl).toContain('/api/portal/cms/websites/42/media/1');
      expect(container.textContent).not.toContain('Media Details');
    });
  });
...
describe('WebsiteMediaPage — versions', () => {
  ...
  it('toggles Version history open and fetches versions from the global media endpoint', async () => {
    ...
    await waitFor(() => {
      const calls = fetchMock.mock.calls.map(c => String(c[0]));
      // Per source: versions use /api/portal/media (global, not /websites/<id>)
      expect(calls.some(u => u.includes('/api/portal/media/1/versions'))).toBe(true);
  ...
  it('Replace File POSTs the file and updates the detail', async () => {
    ...
    await waitFor(() => {
      const calls = fetchMock.mock.calls.map(c => String(c[0]));
      expect(calls.some(u => u.includes('/api/portal/media/1/replace'))).toBe(true);
    });
  });
