...
describe('POST /api/portal/reset-password', () => {
  ...
  it('returns 400 when password is missing', async () => {
    ...
    expect(res.status).toBe(400);
    // Route's own guard rejects a missing password before the strength policy.
    expect((await res.json()).error).toMatch(/required/i);
  });

  it('returns 400 when password is too short', async () => {
    // Strength validation (AUTH79) runs after the token→user lookup (it needs the
    // user's email/name for the identifier guard), so seed a valid user first.
    selectQueue.push([{ id: 42, email: 'u@e.com', name: 'User', password: 'h', passwordHistory: [] }]);
    const res = await resetPasswordRoute.POST(
  ...
  it('hashes the new password, clears the reset token, and returns success', async () => {
    ...
    const res = await resetPasswordRoute.POST(
      makeJsonReq('http://x/api/portal/reset-password', 'POST', {
        token: 'tok',
        // policy-valid: >=12 chars so it clears the letters-only composition rule
