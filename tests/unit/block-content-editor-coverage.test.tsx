...
// Image Width preset and section Font Family/Text Color moved from the
// Content tab to the Style tab (StyleSettings.tsx) — imported directly for
// the tests below that exercise their new location. Nothing StyleSettings
// renders by default (Layout section only) needs GradientBuilder/MediaPicker,
// so no extra mocks are required beyond what's already mocked above.
...
describe('BlockContentEditor — image block extra selects', () => {
  it('updates width via SelectField (moved to Style tab)', () => {
    // The image Width preset moved off the Content tab (BlockContentEditor)
    // entirely — it's now a semantic block-level field on the Style tab
    // (StyleSettings.tsx), written directly via onChange rather than via
    // updateResponsiveStyle. The Content tab's image panel now has only one
    // select (Alignment) — see the next test.
    const onChange = vi.fn();
    const block = { id: 'i1', type: 'image', order: 0, url: 'x.png', alt: 'alt' } as any;
    const { container } = render(
      <StyleSettings block={block} onChange={onChange} currentViewport="desktop" />,
    );
    const widthSelect = Array.from(container.querySelectorAll('select')).find((s) =>
      Array.from((s as HTMLSelectElement).options).some((o) => o.value === 'small'),
    ) as HTMLSelectElement;
    fireEvent.change(widthSelect, { target: { value: 'small' } });
    expect(onChange).toHaveBeenCalledWith({ width: 'small' });
  });

  it('updates alignment via SelectField', () => {
    // MediaPanel's image branch now renders a single select (Alignment).
    const { container, onUpdate } = renderBlock({
      id: 'i1', type: 'image', order: 0, url: 'x.png',
    });
    const selects = container.querySelectorAll('select');
    fireEvent.change(selects[0], { target: { value: 'right' } });
...
describe('BlockContentEditor — cta block all fields', () => {
  it('updates title, description, button text and url', () => {
    ...
    // VEQA-064: description is a rich-text control (not a plain textarea)
    // sized with the "tall" min-height convention used elsewhere for
    // multi-line copy (min-h-[60px]), not the cramped single-line default.
    expect(descRte.getAttribute('data-single-line')).toBe('false');
    expect(descRte.parentElement?.className).toContain('min-h-[60px]');
  ...
  it('no longer renders a background control on the CTA Content tab (VEQA-065)', () => {
    const { container } = renderBlock({
      id: 'c1', type: 'cta', order: 0,
    });
    // Background moved to the Style tab; the CTA Content tab has no <select>.
...
describe('BlockContentEditor — section fontFamily', () => {
  // Font Family and Text Color for section blocks moved from the Content tab
  // to the Style tab's Typography section (StyleSettings.tsx) — see the
  // comment atop LayoutPanel.tsx's section-block branch.
  function openTypography(container: HTMLElement) {
    const header = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Typography'));
    fireEvent.click(header!);
  }

  it('updates fontFamily via GoogleFontPicker', () => {
    const onChange = vi.fn();
    const block = { id: 's1', type: 'section', order: 0, blocks: [] } as any;
    const { container } = render(
      <StyleSettings block={block} onChange={onChange} currentViewport="desktop" />,
    );
    openTypography(container);
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ style: expect.objectContaining({ fontFamily: 'Inter' }) }),
    );
  });
...
describe('BlockContentEditor — youtube block caption', () => {
  it('updates caption field', () => {
    // Caption is now a RichTextField (rendered by the mocked RichTextEditable
    // as a `rte-Caption` textarea), not a plain text Field.
    const { container, onUpdate } = renderBlock({
      id: 'y1', type: 'youtube', order: 0, url: 'https://youtube.com', caption: 'old',
    });
    const captionInput = container.querySelector('[data-testid="rte-Caption"]') as HTMLTextAreaElement;
    expect(captionInput.value).toBe('old');
    fireEvent.change(captionInput, { target: { value: 'new caption' } });
    expect(onUpdate).toHaveBeenCalledWith({ caption: 'new caption' });
  });
});
...
describe('BlockContentEditor — quote block author and citation', () => {
  it('updates author and citation fields', () => {
    // Author and Citation are RichTextFields (rendered via the mocked
    // RichTextEditable as `rte-Author` / `rte-Citation` textareas), not plain
    // text Fields.
    const { container, onUpdate } = renderBlock({
      id: 'q1', type: 'quote', order: 0, content: 'q', author: 'Auth', citation: 'Cite',
    });
    const authorInput = container.querySelector('[data-testid="rte-Author"]') as HTMLTextAreaElement;
    expect(authorInput.value).toBe('Auth');
    fireEvent.change(authorInput, { target: { value: 'New Author' } });
    expect(onUpdate).toHaveBeenCalledWith({ author: 'New Author' });

    const citeInput = container.querySelector('[data-testid="rte-Citation"]') as HTMLTextAreaElement;
    expect(citeInput.value).toBe('Cite');
