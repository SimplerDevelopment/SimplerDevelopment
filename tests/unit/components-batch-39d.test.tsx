...
describe('ImageBlockRender', () => {
  ...
  it('renders a placeholder (no broken <img>) when block.url is missing', () => {
    const { container } = render(
      <ImageBlockRender block={makeImage({ url: '' })} />,
    );
    // Renders a placeholder figure instead of null so the block stays
    // selectable in the editor; must not emit a broken <img> with no src.
