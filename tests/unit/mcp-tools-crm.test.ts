...
describe('crm_deals_delete', () => {
  ...
  it('returns deleted echo on hit', async () => {
    // MEB-001: existence/tenancy pre-check, then delete runs via the (mocked)
    // stageOrApply apply-path when the credential does not require approval.
    stageOrApplyMock.mockImplementationOnce(async (opts) => ({ pending: false, data: await opts.apply() }));
  ...
  it('stages a pending envelope when the credential requires approval (MEB-001)', async () => {
    stageOrApplyMock.mockResolvedValueOnce({ pending: true, pendingId: 55, summary: 'Delete CRM deal #4', status: 'pending' });
    dbState.selectQueue = [[{ id: 4 }]];   // deal exists — passes the pre-check, then stages
    const tools = registerAll(['*']);
    const res = await tools.get('crm_deals_delete')!.handler({ dealId: 4 });
    const out = parseJson(res) as { pending: boolean; pendingId: number };
    expect(out.pending).toBe(true);
    expect(out.pendingId).toBe(55);
  });
});
