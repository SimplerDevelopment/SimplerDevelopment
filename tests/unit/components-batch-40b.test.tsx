...
describe('CardGridBlockRender', () => {
  ...
  it('applies a per-card iconColor as an inline style on the icon element', () => {
    const block = {
      type: 'card-grid',
      cards: [{ id: 'x', title: 'X', description: 'D', icon: 'star', iconColor: '#ff0000' }],
    };
    const { container } = render(<CardGridBlockRender block={block as any} />);
    const icon = container.querySelector('.text-primary.mb-4');
    expect(icon).not.toBeNull();
    expect((icon as HTMLElement).style.color).toBe('rgb(255, 0, 0)');
  });

  it('leaves the icon color unset when the card has no iconColor', () => {
    const block = {
      type: 'card-grid',
      cards: [{ id: 'x', title: 'X', description: 'D', icon: 'star' }],
    };
    const { container } = render(<CardGridBlockRender block={block as any} />);
    const icon = container.querySelector('.text-primary.mb-4');
    expect(icon).not.toBeNull();
