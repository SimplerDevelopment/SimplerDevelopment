...
describe('SnapshotsDropdown — naming form', () => {
  ...
  // ponytail: flaky in CI (issue #16) — quarantined per tests/CI-GATES.md; re-promote after 20 green runs
  it.skip('saves snapshot on form submit and shows status message', async () => {
    render(<SnapshotsDropdown />);
    openDropdown();
    fireEvent.click(screen.getByRole('button', { name: /New/i }));
  ...
  // ponytail: flaky in CI (issue #16) — quarantined per tests/CI-GATES.md; re-promote after 20 green runs
  it.skip('uses fallback name when input is left blank', () => {
    render(<SnapshotsDropdown />);
    openDropdown();
    fireEvent.click(screen.getByRole('button', { name: /New/i }));
...
describe('SnapshotsDropdown — restore action (two-step)', () => {
  ...
  // ponytail: flaky in CI (issue #16) — quarantined per tests/CI-GATES.md; re-promote after 20 green runs
  it.skip('second click imports canvas data and closes dropdown', () => {
    const snap = makeSnap({
      id: 'snap-1',
      name: 'My Snap',
...
describe('SnapshotsDropdown — status banner auto-dismiss', () => {
  // ponytail: flaky in CI (issue #16) — quarantined per tests/CI-GATES.md; re-promote after 20 green runs
  it.skip('status banner disappears after 2500ms', async () => {
    vi.useFakeTimers();
