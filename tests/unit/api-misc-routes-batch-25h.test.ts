...
describe('POST /api/portal/change-password', () => {
  ...
  it('returns 400 when newPassword shorter than 8 chars', async () => {
    authMock.mockResolvedValue(SESSION);
    // Strength validation (AUTH79) runs after the user fetch + current-password
    // check (it needs the user's email/name for the identifier guard), so seed a
    // valid user + a context-aware compare (true only for the current password).
    selectQueue.push([{ id: 7, email: 'u@example.com', name: 'User', password: 'hashed-old', passwordHistory: [] }]);
    compareMock.mockImplementation((p: unknown) => Promise.resolve(p === 'oldpass'));
    const res = await changePasswordRoute.POST(
  ...
  it('returns 400 when newPassword is not a string', async () => {
    ...
    expect(res.status).toBe(400);
    const body = await res.json();
    // The route's own type guard rejects a non-string before the strength policy.
    expect(body.error).toMatch(/New password is required/);
  });
  ...
  it('hashes new password and updates the user when current is valid', async () => {
    authMock.mockResolvedValue(SESSION);
    selectQueue.push([{ id: 7, password: 'hashed-old' }]);
    // Context-aware: current password matches, but the new password must NOT be
    // seen as reused (isPasswordReused also calls compare).
    compareMock.mockImplementation((p: unknown) => Promise.resolve(p === 'oldpass'));
