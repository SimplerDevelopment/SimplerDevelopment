...
// ---------------------------------------------------------------------------
// 5b. External drag into a container (VEQA-011)
// ---------------------------------------------------------------------------
describe('EditableBlockRenderer — external drag into containers', () => {
  it('nests a new block into an empty section instead of the top level', () => {
    mockEditor = makeEditorState({
      active: true,
      blocks: [{ id: 'sec1', type: 'section', order: 0, blocks: [] }],
      // jsdom's getBoundingClientRect() always returns a zeroed rect, so
      // (x: 0, y: 0) is a guaranteed hit against the section's drop zone —
      // the only `[data-container-drop-id]` element in this tree.
      externalDrag: { active: true, blockType: 'text', x: 0, y: 0 },
    });
    const { container } = render(<EditableBlockRenderer content={wrapContent([])} />);
    const dropZone = container.querySelector('[data-container-drop-id="sec1:0"]');
    ...
    const updated = mockEditor.onBlocksReordered.mock.calls.at(-1)?.[0];
    // Still a single top-level block — the new block went inside the section.
    expect(updated).toHaveLength(1);
    expect(updated[0].id).toBe('sec1');
    expect(updated[0].blocks).toHaveLength(1);
    expect(updated[0].blocks[0].type).toBe('text');
  });

  it('appends into a populated column via insertIntoContainer instead of splicing top-level', () => {
    mockEditor = makeEditorState({
      active: true,
      blocks: [
        {
          ...
          gap: 'md',
          columns: [
            { id: 'c1', width: 50, blocks: [{ id: 'existing', type: 'text', order: 0, content: 'x' }] },
            { id: 'c2', width: 50, blocks: [] },
          ],
        },
      ],
      externalDrag: { active: true, blockType: 'heading', x: 0, y: 0 },
    });
    render(<EditableBlockRenderer content={wrapContent([])} />);
    act(() => {
      window.dispatchEvent(new Event('sd-external-drop'));
    ...
    const updated = mockEditor.onBlocksReordered.mock.calls.at(-1)?.[0];
    // Still one top-level block (the columns block itself).
    expect(updated).toHaveLength(1);
    expect(updated[0].columns[0].blocks.map((b: any) => b.type)).toEqual(['text', 'heading']);
    expect(updated[0].columns[1].blocks).toHaveLength(0);
  });

  it('falls back to top-level insertion when the cursor is not over any container zone', () => {
    mockEditor = makeEditorState({
      active: true,
      blocks: [
      ...
      // x: 50 never matches jsdom's zeroed container-zone rects (which only
      // hit at x: 0), so the container hit-test misses and the existing
      // top-level y-based fallback takes over.
      externalDrag: { active: true, blockType: 'heading', x: 50, y: -9999 },
    });
    render(<EditableBlockRenderer content={wrapContent([])} />);
    act(() => {
      window.dispatchEvent(new Event('sd-external-drop'));
    ...
    expect(updated[1].id).toBe('sec1');
    expect(updated[1].blocks).toHaveLength(0);
  });
});
