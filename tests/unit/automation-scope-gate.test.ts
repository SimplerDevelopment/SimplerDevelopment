...
// ───────────────────────────────────────────────────────────────────────────
// 3. UAG-002 — per-rule approval gating
// ───────────────────────────────────────────────────────────────────────────
describe('runRule UAG-002 approval gating', () => {
  beforeEach(() => {
    mockExecutePortalTool.mockClear();
    mockLogAgentAction.mockClear();
    state.inserts = [];
  ...
  it('passes an approval gate (requireCmsApproval) for a requiresApproval rule', async () => {
    const rule = makeRule({
      scopes: ['crm:write'],          // clears the scope gate
      requiresApproval: true,
      actions: [{ tool: 'update_crm_deal', params: { id: 1 } }],
    });
    await runRule(rule, { _userId: 7 }, 'test.event');
    expect(mockExecutePortalTool).toHaveBeenCalledTimes(1);
    const ctxArg = mockExecutePortalTool.mock.calls[0][4] as { gate?: { requireCmsApproval: boolean } };
    expect(ctxArg.gate).toBeDefined();
    expect(ctxArg.gate!.requireCmsApproval).toBe(true);
  });

  it('passes NO gate for a human-authored rule (requiresApproval false)', async () => {
    const rule = makeRule({
      scopes: ['crm:write'],
    ...
    await runRule(rule, { _userId: 7 }, 'test.event');
    expect(mockExecutePortalTool).toHaveBeenCalledTimes(1);
    const ctxArg = mockExecutePortalTool.mock.calls[0][4] as { gate?: unknown };
    expect(ctxArg.gate).toBeUndefined();
  });
});
