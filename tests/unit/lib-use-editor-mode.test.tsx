...
describe('useEditorMode', () => {
  ...
  it('returns initial state shape with zeroed externalDrag', () => {
    ...
    expect(result.current.forceHoverBlockId).toBeNull();
  ...
  // -------------------------------------------------------------------------
  // FORCE_HOVER_BLOCK (VEQA-041)
  // -------------------------------------------------------------------------

  it('forceHoverBlockId starts null', () => {
    const { result } = renderHook(() => useEditorMode());
    expect(result.current.forceHoverBlockId).toBeNull();
  });

  it('FORCE_HOVER_BLOCK updates forceHoverBlockId', () => {
    const { result } = renderHook(() => useEditorMode());
    act(() => {
      dispatchParent(PARENT_MESSAGES.FORCE_HOVER_BLOCK, { blockId: 'b1' });
    });
    expect(result.current.forceHoverBlockId).toBe('b1');
    act(() => {
      dispatchParent(PARENT_MESSAGES.FORCE_HOVER_BLOCK, { blockId: null });
    });
    expect(result.current.forceHoverBlockId).toBeNull();
  });

  it('FORCE_HOVER_BLOCK is independent of hoveredBlockId', () => {
    const { result } = renderHook(() => useEditorMode());
    act(() => {
      dispatchParent(PARENT_MESSAGES.HOVER_BLOCK, { blockId: 'b1' });
    });
    act(() => {
      dispatchParent(PARENT_MESSAGES.FORCE_HOVER_BLOCK, { blockId: 'b2' });
    });
    expect(result.current.hoveredBlockId).toBe('b1');
    expect(result.current.forceHoverBlockId).toBe('b2');
  });
