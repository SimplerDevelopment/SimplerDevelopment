...
const discount1 = {
  ...
  amount: 1000, // 10% stored as basis-points-of-percent (amount / 100 = 10)
  minOrderAmount: 5000,
  maxUses: 100,
  usedCount: 5,
  active: true,
...
describe('DiscountsPage', () => {
  ...
  describe('discount table', () => {
    ...
    it('renders minOrderAmount as currency when set', async () => {
      const { container } = await renderLoaded();
      expect(container.textContent).toContain('$50.00');
    });

    it('renders "--" for minOrderAmount when null', async () => {
      const { container } = await renderLoaded();
      // FLAT5 has null minOrderAmount
      const tds = container.querySelectorAll('td');
  ...
  describe('create modal', () => {
    ...
    it('minOrderAmount is null when field is 0', async () => {
      const fetchSpy = vi.fn((url: string, init?: RequestInit) =>
        Promise.resolve(handlers[0](url, init)),
      );
      vi.stubGlobal('fetch', fetchSpy);
      ...
      await waitFor(() => {
        ...
        const body = JSON.parse((postCall![1] as RequestInit).body as string);
        expect(body.minOrderAmount).toBeNull();
      });
    });
  });
