...
describe('PUT /api/portal/cms/websites/[siteId]/code', () => {
  ...
  it('keeps empty string as a real draft value ("clear published code") — null means no draft (QAD-006)', async () => {
    authMock.mockResolvedValue(STAFF_SESSION);
    getPortalClientMock.mockResolvedValue({ id: 33 });
    ...
    expect(res.status).toBe(200);
    // Empty string persists as-is: it is the "clear the published code"
    // draft; coercing to null made cleared code unpublishable (400 no-draft).
    expect(updateCalls[0].patch.draftCustomCss).toBe('');
    expect(updateCalls[0].patch.draftCustomJs).toBe('');
    const body = await res.json();
    expect(body.data.customCss).toBe('');
    expect(body.data.customJs).toBe('');
  });
