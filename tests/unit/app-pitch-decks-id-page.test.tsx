...
describe('Save / publish / delete', () => {
  ...
  it('togglePublish preserves unsaved local slide edits — does not clobber with the server row (QAD-041 Bug B)', async () => {
    // The status-only PATCH echoes a STALE deck whose slide label predates the
    // user's unsaved rename. The fix must merge only status/updatedAt and keep
    // the local slides; the old full-object overwrite reverted the rename.
    apiMocks.patchDeck.mockResolvedValueOnce({
      success: true,
      data: makeDeck({
        status: 'published',
        slides: [makeSlide('s1', 'Slide 1'), makeSlide('s2', 'Slide 2')],
    ...
    const { container } = renderPage();
    // Unsaved local edit — rename slide 0 in React state only.
    clickCb(container, 'SlideList-onRenameSlide', [0, 'Locally Renamed']);
    await waitFor(() => {
      expect(getProps('SlideList', container).slides[0].label).toBe('Locally Renamed');
    });
    // Toggle publish — the stale server slides must NOT overwrite local edits.
    clickCb(container, 'EditorHeader-onTogglePublish');
    await waitFor(() => {
      expect(apiMocks.patchDeck).toHaveBeenCalled();
    });
    const list = getProps('SlideList', container);
    expect(list.slides[0].label).toBe('Locally Renamed');
  });
