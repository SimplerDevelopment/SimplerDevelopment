...
// oauth-state — the google/auth route now signs a CSRF-bound state param
// (HMAC + TTL, AUTH79-014) instead of passing the raw siteId. Stub it so the
// route doesn't need OAUTH_STATE_SECRET, and so the signed value is stable.
vi.mock('@/lib/google/oauth-state', () => ({
  signState: () => 'signed-state-stub',
  verifyState: () => ({ clientId: 42, userId: 7, surfaces: [], returnTo: '5' }),
}));
