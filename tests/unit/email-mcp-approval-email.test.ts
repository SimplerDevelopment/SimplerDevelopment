...
const H = vi.hoisted(() => {
  ...
  return {
    ...
    // BAR-003: per-userId delivery mode returned by the mocked
    // shouldDeliverNotification. Defaults to 'instant' for any userId not
    // present in the map, matching the real function's "no preference row"
    // default.
    deliveryModeByUser: new Map<number, 'instant' | 'digest_daily' | 'off'>(),
    shouldDeliverNotificationMock: vi.fn(),
  };
});
...
// BAR-003: mock the digest-preference gate. Default mode is 'instant' unless
// a test seeds H.deliveryModeByUser for a given userId.
vi.mock('@/lib/crm/notifications', () => ({
  shouldDeliverNotification: (...args: unknown[]) => H.shouldDeliverNotificationMock(...args),
}));
...
beforeEach(() => {
  ...
  H.deliveryModeByUser.clear();
  H.shouldDeliverNotificationMock.mockReset();
  H.shouldDeliverNotificationMock.mockImplementation(
    async (_clientId: number, userId: number, _type: string) => {
      const mode = H.deliveryModeByUser.get(userId) ?? 'instant';
      return { deliver: mode !== 'off', mode };
    },
  );
...
// ---------------------------------------------------------------------------
// BAR-003 — digest-preference branching
// ---------------------------------------------------------------------------

describe('sendApprovalEmails — BAR-003 digest-preference branching', () => {
  it('skips the instant email for a digest_daily user on a non-high-risk operation', async () => {
    H.dbState.clientRow = [{ company: 'Acme' }];
    H.dbState.recipientUsers = [{ id: 10, email: 'digest@example.test', name: 'Digest' }];
    H.dbState.recentCounts = [{ userId: 10, count: 1 }];
    H.deliveryModeByUser.set(10, 'digest_daily');
    const { sendApprovalEmails } = await importSut();
    ...
    expect(H.shouldDeliverNotificationMock).toHaveBeenCalledWith(1, 10, 'mcp_pending_change');
    expect(H.resendSendMock).not.toHaveBeenCalled();
  });

  it('still sends the instant email for a digest_daily user on a high-risk delete operation', async () => {
    H.dbState.clientRow = [{ company: 'Acme' }];
    H.dbState.recipientUsers = [{ id: 10, email: 'digest@example.test', name: 'Digest' }];
    H.dbState.recentCounts = [{ userId: 10, count: 1 }];
    H.deliveryModeByUser.set(10, 'digest_daily');
    ...
    await sendApprovalEmails({
      ...
      userIds: [10],
      pendingId: 1,
      summary: 'Delete deal #42',
      entityType: 'crm_deal',
      operation: 'crm_deal.delete',
    });

    expect(H.resendSendMock).toHaveBeenCalledTimes(1);
    expect(H.resendSendMock.mock.calls[0][0].to).toBe('digest@example.test');
  });

  it('still sends the instant email for a digest_daily user on a high-risk send operation', async () => {
    H.dbState.clientRow = [{ company: 'Acme' }];
    H.dbState.recipientUsers = [{ id: 10, email: 'digest@example.test', name: 'Digest' }];
    H.dbState.recentCounts = [{ userId: 10, count: 1 }];
    H.deliveryModeByUser.set(10, 'digest_daily');
  ...
  it('always sends the instant email for an instant-preference user, high-risk or not', async () => {
    H.dbState.clientRow = [{ company: 'Acme' }];
    H.dbState.recipientUsers = [{ id: 10, email: 'instant@example.test', name: 'Instant' }];
    H.dbState.recentCounts = [{ userId: 10, count: 1 }];
    H.deliveryModeByUser.set(10, 'instant');
    const { sendApprovalEmails } = await importSut();
    ...
    expect(H.resendSendMock).toHaveBeenCalledTimes(1);
    expect(H.resendSendMock.mock.calls[0][0].to).toBe('instant@example.test');
  });

  it('defensively skips a recipient whose mode resolves to "off"', async () => {
    H.dbState.clientRow = [{ company: 'Acme' }];
    H.dbState.recipientUsers = [{ id: 10, email: 'off@example.test', name: 'Off' }];
    H.dbState.recentCounts = [{ userId: 10, count: 1 }];
    H.deliveryModeByUser.set(10, 'off');
    const { sendApprovalEmails } = await importSut();
  ...
  it('handles a mixed batch: digest_daily skipped, instant sent, in parallel mode lookups', async () => {
    H.dbState.clientRow = [{ company: 'Acme' }];
    H.dbState.recipientUsers = [
      { id: 10, email: 'digest@example.test', name: 'Digest' },
      { id: 11, email: 'instant@example.test', name: 'Instant' },
    ];
    H.dbState.recentCounts = [
      { userId: 10, count: 1 },
      { userId: 11, count: 1 },
    ...
    H.deliveryModeByUser.set(10, 'digest_daily');
    H.deliveryModeByUser.set(11, 'instant');
    const { sendApprovalEmails } = await importSut();
    ...
    expect(H.resendSendMock).toHaveBeenCalledTimes(1);
    expect(H.resendSendMock.mock.calls[0][0].to).toBe('instant@example.test');
    // Both recipients' modes were looked up.
    expect(H.shouldDeliverNotificationMock).toHaveBeenCalledTimes(2);
  });
});
