...
describe('useVisualEditorParent', () => {
  ...
  it('sendForceHoverBlock: no-ops when iframeReady=false', () => {
    const cbs = makeCallbacks();
    const { result } = renderHook(() =>
      useVisualEditorParent({ blocks: [], selectedBlockId: null, ...cbs }),
    );
    ...
    act(() => {
      result.current.sendForceHoverBlock('b1');
    });
  ...
  it('sendForceHoverBlock: calls sendToIframe with FORCE_HOVER_BLOCK + blockId', () => {
    const cbs = makeCallbacks();
    const { result } = makeReadyHook({ blocks: [], selectedBlockId: null, ...cbs });

    act(() => {
      result.current.sendForceHoverBlock('fh-1');
    });

    expect(sendToIframe).toHaveBeenCalledWith(
      null,
      PARENT_MESSAGES.FORCE_HOVER_BLOCK,
      expect.objectContaining({ blockId: 'fh-1' }),
    );
  });

  it('sendForceHoverBlock: accepts null blockId to clear force-hover', () => {
    const cbs = makeCallbacks();
    const { result } = makeReadyHook({ blocks: [], selectedBlockId: null, ...cbs });

    act(() => {
      result.current.sendForceHoverBlock(null);
    });
