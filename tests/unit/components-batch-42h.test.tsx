...
import SidebarCreditBadge from '@/components/portal/SidebarCreditBadge';
...
// ---------------------------------------------------------------------------
// CreditPurchaseControls (extracted from the former dashboard CreditBalance
// widget — VEQA-003; now rendered on /portal/settings/ai)
// ---------------------------------------------------------------------------
describe('CreditPurchaseControls', () => {
  it('renders null while data is still loading', async () => {
    ...
    const { container } = render(<CreditPurchaseControls />);
  ...
  it('shows at most three quick-buy packages and the PAYG status chip', async () => {
    mockFetchOnce({
      balance: 1_500_000,
  ...
  it('shows PAYG Off when pay-as-you-go is disabled', async () => {
    mockFetchOnce({
      balance: 10_000,
      monthlyGrant: 0,
      monthlyUsage: 0,
...
// ---------------------------------------------------------------------------
// SidebarCreditBadge — compact readout that replaced the dashboard widget
// ---------------------------------------------------------------------------
describe('SidebarCreditBadge', () => {
  it('renders the compact balance linking to the AI settings page', async () => {
    mockFetchOnce({
      balance: 12_000,
      monthlyGrant: 50_000,
      payAsYouGo: false,
    ...
    const { container } = render(<SidebarCreditBadge />);
    await flushMicrotasks();

    const link = container.querySelector('a');
    expect(link?.getAttribute('href')).toBe('/portal/settings/ai');
    expect(link?.textContent).toContain('AI Credits');
    expect(link?.textContent).toContain('12K');
    // Material icon, not emoji
    const icon = container.querySelector('.material-icons');
    expect(icon?.textContent).toBe('token');
  });
  ...
  it('carries the balance in the title tooltip when collapsed', async () => {
    mockFetchOnce({
      balance: 1_500_000,
      monthlyGrant: 0,
      payAsYouGo: false,
    ...
    const { container } = render(<SidebarCreditBadge collapsed />);
    await flushMicrotasks();

    const link = container.querySelector('a');
    expect(link?.getAttribute('title')).toBe('AI Credits: 1.5M');
  });
});

// ---------------------------------------------------------------------------
// AISettingsPage — hosts the relocated purchase controls (VEQA-003)
// ---------------------------------------------------------------------------
describe('AISettingsPage — credit purchase controls', () => {
  it('renders the purchase controls on the AI settings page', async () => {
    // Page fetch order (Promise.all): conversations, credits, profile,
    // image-usage; the mounted CreditPurchaseControls then fetches credits.
    mockFetchSequence([
      ...
      { value: {} }, // image-usage
      {
        value: {
          ...
          payAsYouGo: false,
          monthlyUsage: 100,
          packages: [{ id: 1, name: 'Small', tokens: 50_000, price: 500 }],
        },
      }, // CreditPurchaseControls fetch
    ]);

    const { container } = render(<AISettingsPage />);

    await waitFor(() => {
      expect(container.textContent).toContain('Buy AI Credits');
    });
    expect(container.textContent).toContain('PAYG Off');
    expect(container.textContent).toContain('+50K $5');
    // The page's read-only balance card still renders alongside the controls
