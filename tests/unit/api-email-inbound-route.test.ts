...
vi.mock('@/lib/ai/portal-tools', () => {
  // Mirror the real high-risk classifier (lib/ai/portal-tools/gating.ts) so the
  // inbound route's surface-filter + refusal guard behave realistically.
  const APPROVAL_REQUIRED = new Set([
    'publish_page', 'send_crm_proposal', 'invite_team_member', 'pay_invoice',
    'request_service', 'create_automation', 'toggle_automation',
    'create_crm_deal', 'update_crm_deal',
  ]);
  return {
    ...
    isApprovalRequired: (name: string) => APPROVAL_REQUIRED.has(name),
    unattendedRefusal: (name: string) => ({ error: `refused: ${name}` }),
  };
});
