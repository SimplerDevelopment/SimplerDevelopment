...
describe('SpacerBlockRender', () => {
  ...
  // VEQA-058: a Style-tab / resize custom height must win over the enum class.
  it('honors a raw CSS length passed as block.height (no enum class)', () => {
    const { container } = render(<SpacerBlockRender block={makeSpacer({ height: '40px' })} />);
    const div = container.querySelector('div') as HTMLDivElement;
    expect(div.style.height).toBe('40px');
    expect(div.className).not.toMatch(/\bh-\d/);
  });

  it('honors style.height over the enum height class', () => {
    const { container } = render(
      <SpacerBlockRender block={makeSpacer({ height: 'md', style: { height: '5rem' } })} />,
    );
    const div = container.querySelector('div') as HTMLDivElement;
    expect(div.style.height).toBe('5rem');
    expect(div.className).not.toContain('h-8');
  });

  it('honors responsiveStyle.desktop.height above style.height and the enum', () => {
    const { container } = render(
      <SpacerBlockRender
        block={makeSpacer({
          height: 'md',
          style: { height: '5rem' },
          responsiveStyle: { desktop: { height: '10vh' } },
        })}
      />,
    );
    const div = container.querySelector('div') as HTMLDivElement;
    expect(div.style.height).toBe('10vh');
    expect(div.className).not.toContain('h-8');
  });
});
