...
import type { StepProps } from '@/components/portal/onboarding/steps/types';
...
describe('OnboardingWizard', () => {
  ...
  it('shows Skip for now button on non-done steps', () => {
    ...
    render(<OnboardingWizard initialState={makeState('welcome', { reopenedAt: '2026-01-01T00:00:00.000Z' })} />);
  ...
  it('hides Skip for fresh signups', () => {
    global.fetch = makePostFetchMock();
    render(<OnboardingWizard initialState={makeState('welcome')} />);
    expect(screen.queryByTestId('onboarding-skip-all')).toBeNull();
  });
...
// ── StepModuleSetup (real component) — OBQA-028 wizard-tier collapse ────────
//
// The stub registered above replaces StepModuleSetup wholesale for the
// OnboardingWizard navigation tests. These tests bypass that mock via
// vi.importActual to exercise the real per-module walk/acknowledge/summary
// tiering (lib/onboarding/module-segments.ts WIZARD_TIERS) and the new
// bookings inline form — complicated modules collapse to one "activated"
// summary screen instead of a per-module walk (card 563).

let RealStepModuleSetup: React.ComponentType<StepProps>;

beforeAll(async () => {
  const actual = await vi.importActual<
    typeof import('@/components/portal/onboarding/steps/StepModuleSetup')
  >('@/components/portal/onboarding/steps/StepModuleSetup');
  RealStepModuleSetup = actual.StepModuleSetup;
});

function makeStepProps(overrides: Partial<StepProps> & { state: OnboardingState }): StepProps {
  return {
    setAnswers: vi.fn(),
...
/** Fetch mock that answers the status poll, the bookings create POST, and
 *  the brain note create POST. */
function makeModuleSetupFetchMock() {
  return vi.fn((url: unknown) => {
    if (typeof url === 'string' && url.includes('/api/portal/onboarding/status')) {
      return Promise.resolve({
        ok: true,
        json: async () => ({ success: true, data: { domains: {} } }),
      });
    }
    if (typeof url === 'string' && url.includes('/api/portal/tools/booking')) {
      return Promise.resolve({
        ok: true,
        json: async () => ({ success: true, data: { id: 42 } }),
      });
    }
    if (typeof url === 'string' && url.includes('/api/portal/brain/knowledge')) {
      return Promise.resolve({
        ok: true,
...
describe('StepModuleSetup (real component, OBQA-028 wizard tiers)', () => {
  it('summary-tier module never gets its own walk segment', async () => {
    global.fetch = makeModuleSetupFetchMock() as unknown as typeof fetch;
    const state = makeState('module-setup', { selectedModules: ['websites', 'store'] });

    render(<RealStepModuleSetup {...makeStepProps({ state })} />);

    // First (and only walk) segment is websites — 2 total segments: 1 walk + 1 grouped summary.
    // (The websites inline form's own heading also reads "Name your first
    // site", so anchor on its unique input testid instead of the duplicated text.)
    await waitFor(() => expect(screen.getByTestId('ob-setup-site-name')).toBeTruthy());
    expect(screen.queryByText('Also activated')).toBeNull();

    fireEvent.click(screen.getByRole('button', { name: /Next module/ }));

    // Store never gets its own walk segment — it's grouped onto the summary screen.
    await waitFor(() => expect(screen.getByText('Also activated')).toBeTruthy());
    expect(screen.getByText('Storefront & Commerce')).toBeTruthy();
    expect(screen.queryByText('Add your first product')).toBeNull();
  });

  it('surveys shows acknowledge copy with zero pending rows', async () => {
    global.fetch = makeModuleSetupFetchMock() as unknown as typeof fetch;
    const state = makeState('module-setup', { selectedModules: ['surveys'] });

    render(<RealStepModuleSetup {...makeStepProps({ state })} />);

    await waitFor(() => expect(screen.getByText('Surveys enabled')).toBeTruthy());
    expect(
      screen.getByText('Create your first survey whenever you need it — Surveys is in your sidebar.'),
    ).toBeTruthy();
    // No pending action rows — check the rows' unique description copy rather
    // than their label, since the segment *header* title also happens to read
    // "Create your first survey" (title mirrors the primary action's label).
    expect(
      screen.queryByText('Build a form, poll, or qualification questionnaire.'),
    ).toBeNull();
    expect(
      screen.queryByText('Share the survey link and watch results arrive.'),
    ).toBeNull();
  });

  it('bookings segment renders the inline form and POST goes to /api/portal/tools/booking', async () => {
    global.fetch = makeModuleSetupFetchMock() as unknown as typeof fetch;
    const state = makeState('module-setup', { selectedModules: ['bookings'] });

    render(<RealStepModuleSetup {...makeStepProps({ state })} />);

    await waitFor(() => expect(screen.getByTestId('ob-setup-booking-title')).toBeTruthy());
    expect(screen.getByTestId('ob-setup-booking-create')).toBeTruthy();

    fireEvent.change(screen.getByTestId('ob-setup-booking-title'), {
      target: { value: 'Discovery call' },
    });
    fireEvent.click(screen.getByTestId('ob-setup-booking-create'));

    await waitFor(() => {
      const calls = (global.fetch as ReturnType<typeof vi.fn>).mock.calls;
      const postCall = calls.find((c) => c[0] === '/api/portal/tools/booking');
      expect(postCall).toBeDefined();
  ...
  it('brain segment renders the inline note form and POST goes to /api/portal/brain/knowledge (OBQA-004)', async () => {
    global.fetch = makeModuleSetupFetchMock() as unknown as typeof fetch;
    const state = makeState('module-setup', { selectedModules: ['brain'] });

    render(<RealStepModuleSetup {...makeStepProps({ state })} />);

    await waitFor(() => expect(screen.getByTestId('ob-setup-brain-note-title')).toBeTruthy());
    expect(screen.getByTestId('ob-setup-brain-note-create')).toBeTruthy();

    fireEvent.change(screen.getByTestId('ob-setup-brain-note-title'), {
      target: { value: 'Company overview' },
    });
    fireEvent.click(screen.getByTestId('ob-setup-brain-note-create'));

    await waitFor(() => {
      const calls = (global.fetch as ReturnType<typeof vi.fn>).mock.calls;
      const postCall = calls.find((c) => c[0] === '/api/portal/brain/knowledge');
      expect(postCall).toBeDefined();
      expect((postCall![1] as RequestInit).method).toBe('POST');
      const body = JSON.parse((postCall![1] as RequestInit).body as string);
  ...
  it('email joins the grouped summary screen instead of its own walk segment (OBQA-005)', async () => {
    global.fetch = makeModuleSetupFetchMock() as unknown as typeof fetch;
    const state = makeState('module-setup', { selectedModules: ['email'] });

    render(<RealStepModuleSetup {...makeStepProps({ state })} />);

    // email is now summary-tier — it never gets a walk segment, so the
    // terminal "Also activated" screen renders immediately (no inline form,
    // no per-action checklist).
    await waitFor(() => expect(screen.getByText('Also activated')).toBeTruthy());
    expect(screen.getByText('Email Marketing')).toBeTruthy();
    expect(screen.queryByText('Verify your sender address')).toBeNull();
    expect(screen.queryByTestId('ob-setup-brain-note-title')).toBeNull();
  });

  it('last screen button reads "Continue"', async () => {
    global.fetch = makeModuleSetupFetchMock() as unknown as typeof fetch;

    // Single walk-tier module — the only segment is also the last one.
    const singleState = makeState('module-setup', { selectedModules: ['websites'] });
    const { unmount } = render(<RealStepModuleSetup {...makeStepProps({ state: singleState })} />);
    await waitFor(() => expect(screen.getByRole('button', { name: /Continue/ })).toBeTruthy());
    unmount();

    // Multi-module case: the grouped summary screen is the terminal segment too.
    const groupedState = makeState('module-setup', { selectedModules: ['websites', 'store'] });
    render(<RealStepModuleSetup {...makeStepProps({ state: groupedState })} />);
    await waitFor(() => expect(screen.getByRole('button', { name: /Next module/ })).toBeTruthy());
    fireEvent.click(screen.getByRole('button', { name: /Next module/ }));
    await waitFor(() => expect(screen.getByText('Also activated')).toBeTruthy());
