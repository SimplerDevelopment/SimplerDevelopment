...
describe('usePostForm — autosave', () => {
  ...
  it('does NOT autosave when ydoc is connected', async () => {
    ...
    const { result } = renderHook(() =>
      usePostForm(defaultArgs({ editorMode: 'iframe', ydoc: fakeYdoc, realtimeStatus: 'connected' }))
    );
    act(() => {
      result.current.setBlocks([{ id: 'n1', type: 'text', values: {} }]);
    });
  ...
  it('DOES autosave when a ydoc exists but realtime never connected (QAD-004 data-loss regression)', async () => {
    const fakeYdoc = {
      on: vi.fn(), off: vi.fn(), transact: vi.fn(),
    } as unknown as import('yjs').Doc;
    const { result } = renderHook(() =>
      usePostForm(defaultArgs({ editorMode: 'iframe', ydoc: fakeYdoc, realtimeStatus: 'disconnected' }))
    );
    act(() => {
      result.current.setBlocks([{ id: 'n1', type: 'text', values: {} }]);
    });
    await act(async () => { vi.advanceTimersByTime(3000); });
    expect(mockApi.updatePost).toHaveBeenCalledTimes(1);
  });
});
