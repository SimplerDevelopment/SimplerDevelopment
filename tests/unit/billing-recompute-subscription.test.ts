...
describe('volume-discount math (domain-catalog)', () => {
  ...
  it('computeAccountBilling applies the 40% tier at 12 modules', () => {
    const b = computeAccountBilling(Array(12).fill(1_000), 1);
    expect(b.discountPercent).toBe(40);
    expect(b.discountedModuleCents.every((c) => c === 600)).toBe(true);
    expect(b.moduleSubtotalCents).toBe(7_200);
  });

  it('all-12 à-la-carte subtotal never exceeds the Complete bundle (OBQA-016)', () => {
    const b = computeAccountBilling(FEATURE_DOMAINS.map((d) => d.monthlyPriceCents), 1);
    expect(b.moduleSubtotalCents).toBeLessThanOrEqual(BUNDLE.monthlyPriceCents);
  });
});
