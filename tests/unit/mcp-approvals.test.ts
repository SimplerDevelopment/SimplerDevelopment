...
describe('applyPendingChange — email_campaign:schedule (MEB-002)', () => {
  it('sets the campaign to scheduled after a tenancy re-check', async () => {
    queueSelect([[{ id: 30, status: 'draft' }]]);
    queueUpdateReturning([{ id: 30, status: 'scheduled' }]);
    const change = mkChange({
      entityType: 'email_campaign', operation: 'schedule', entityId: 30,
      payload: { id: 30, scheduledAt: '2030-01-01T00:00:00.000Z' },
    });
    const r = await applyPendingChange(change as never, 10, 1);
    expect(r).toEqual({ id: 30, status: 'scheduled' });
  });

  it('throws when the campaign can no longer be scheduled (already sent)', async () => {
    queueSelect([[{ id: 30, status: 'sent' }]]);
    const change = mkChange({
      entityType: 'email_campaign', operation: 'schedule', entityId: 30,
      payload: { id: 30, scheduledAt: '2030-01-01T00:00:00.000Z' },
    });
    await expect(applyPendingChange(change as never, 10, 1)).rejects.toThrow(/Cannot schedule/);
  });
});

describe('applyPendingChange — crm_deal:delete (MEB-001)', () => {
  it('deletes the deal after a tenancy re-check', async () => {
    queueSelect([[{ id: 7 }]]);
    queueDelete();
    const change = mkChange({ entityType: 'crm_deal', operation: 'delete', entityId: 7 });
    const r = await applyPendingChange(change as never, 10, 1);
    expect(r).toEqual({ success: true, id: 7 });
  });

  it('throws when the deal is missing or cross-tenant', async () => {
    queueSelect([[]]);
    const change = mkChange({ entityType: 'crm_deal', operation: 'delete', entityId: 7 });
    await expect(applyPendingChange(change as never, 10, 1)).rejects.toThrow('Deal not found');
  });
});
