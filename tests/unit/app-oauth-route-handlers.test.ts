...
// Rate limiter — /oauth/register throttles per-IP. The real in-memory limiter
// accumulates across tests in this file (module-level Map), so later register
// tests would trip it; no test here asserts the 429 branch, so a blanket allow
// keeps the limiter out of scope for these handler tests.
