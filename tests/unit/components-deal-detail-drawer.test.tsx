...
describe('DealDetailDrawer — initial render', () => {
  ...
  it('handles a null companyId/contactId/notes on the deal prop', async () => {
    ...
    await flush();
    // QAD-027: the standalone Notes textarea was removed from the Details tab
    // (PUX-020 — replaced by threaded deal comments; `notes` is still a
    // pass-through field on the payload but has no UI). Assert instead that a
    // null companyId/contactId renders cleanly, showing the "select a company
    // first" placeholder on the (now company-filtered) contact picker.
...
describe('DealDetailDrawer — details form editing', () => {
  ...
  // QAD-027: the Notes textarea UI was removed from the Details tab
  // (PUX-020 — threaded deal comments replaced it). `notes` is still
  // round-tripped on the update payload from the deal prop, but there is no
  // longer an input to edit it through, so this case is dropped rather than
  // asserting on a textarea that no longer renders.
  it.skip('updates the notes textarea content', async () => {
    const props = defaultProps();
    const { container } = render(<DealDetailDrawer {...props} />);
    await flush();
  ...
  // QAD-027: the Write/Preview markdown toggle for deal notes was removed
  // along with the rest of the Notes UI (PUX-020 — threaded deal comments
  // replaced it). Neither a "Preview"/"Write" toggle nor a markdown preview
  // pane exist in the Details tab anymore.
  it.skip('switches Notes between Write and Preview modes', async () => {
    const props = defaultProps();
    const { container } = render(<DealDetailDrawer {...props} />);
    await flush();
  ...
  // QAD-027: see above — the notes preview mode (and its "Nothing to
  // preview" empty state) no longer exists.
  it.skip('shows "Nothing to preview" when notes are blank in preview mode', async () => {
...
describe('DealDetailDrawer — comments tab', () => {
  ...
  it('posts a comment with text body and refreshes', async () => {
    ...
    // QAD-027: postComment now takes a 4th `replyingToId` arg (threaded deal
    // comments) — null here since this is a root-level comment, not a reply.
    expect(apiMock.postComment).toHaveBeenCalledWith(42, 'hello there', [], null);
  });
  ...
  it('posts a comment via Cmd+Enter keystroke', async () => {
    ...
    // QAD-027: postComment's 4th arg is `replyingToId` (threaded comments).
    expect(apiMock.postComment).toHaveBeenCalledWith(42, 'shortcut', [], null);
  });
  ...
  it('posts via Ctrl+Enter as well', async () => {
    ...
    // QAD-027: postComment's 4th arg is `replyingToId` (threaded comments).
    expect(apiMock.postComment).toHaveBeenCalledWith(42, 'ctrl', [], null);
  });
