...
describe('redactSecretsOnly', () => {
  it('strips secret-bearing keys the same way redactInputs does', () => {
    const result = redactSecretsOnly({
      password: 'hunter2',
      apiKey: 'sk-1234',
      name: 'Jane Doe',
    }) as Record<string, unknown>;
  ...
  it('does NOT truncate a large payload — unlike redactInputs', () => {
    // A recipient array whose JSON serialisation is well over the 4KB cap
    // redactInputs enforces. redactSecretsOnly must preserve every entry —
    // that's the whole forensic point of the high-risk-tool capture (AAF-001).
    const recipients = Array.from({ length: 400 }, (_, i) => `person-${i}@example.com`);
    const payload = { campaignId: 42, recipients, body: 'x'.repeat(2000) };

    const serializedSize = Buffer.byteLength(JSON.stringify(payload), 'utf8');
    expect(serializedSize).toBeGreaterThan(4096); // sanity: this WOULD trip redactInputs's cap

    // redactInputs truncates the whole payload away...
    expect(redactInputs(payload)).toEqual({ _truncated: true });

    // ...but redactSecretsOnly keeps every recipient intact.
    const result = redactSecretsOnly(payload) as { campaignId: number; recipients: string[]; body: string };
    expect(result.campaignId).toBe(42);
    expect(result.recipients).toHaveLength(400);
    expect(result.recipients).toEqual(recipients);
    expect(result.body).toHaveLength(2000);
  });

  it('redacts secrets recursively without truncating the rest of a large nested payload', () => {
    const payload = {
      password: 'hunter2',
      recipients: Array.from({ length: 300 }, (_, i) => `user-${i}@example.com`),
      meta: { apiKey: 'sk-secret', label: 'campaign' },
    };
    const result = redactSecretsOnly(payload) as Record<string, unknown>;
    expect(result.password).toBe('[REDACTED]');
    expect((result.meta as Record<string, unknown>).apiKey).toBe('[REDACTED]');
    expect((result.meta as Record<string, unknown>).label).toBe('campaign');
    expect((result.recipients as string[]).length).toBe(300);
  });

  it('does not mutate the original input', () => {
    const original = { password: 'secret', name: 'Alice' };
    redactSecretsOnly(original);
