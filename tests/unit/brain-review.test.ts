...
describe('approveReviewItem', () => {
  ...
  it('commitment: meeting source inserts a brain_tasks row with free-form "when" (NaN due date) and inherited links', async () => {
    state.brainMeetings.push({
      id: 80,
      clientId: 1,
      companyId: 210,
    ...
    state.brainAiReviewItems.push({
      ...
      sourceType: 'meeting',
      sourceId: 80,
      proposedPayload: { who: 'Sarah', what: 'Send proposal', when: 'Friday' },
    });
    const { approveReviewItem } = await importModule();
    const res = await approveReviewItem({ clientId: 1, itemId: 1, actorId: 2 });
    expect(res.resultEntityType).toBe('brain_task');
    expect(state.brainTasks).toHaveLength(1);
    const t = state.brainTasks[0];
    expect(t.title).toBe('Sarah: Send proposal');
    expect(t.dueDate).toBeNull();
    expect(t.source).toBe('meeting');
    expect(t.companyId).toBe(210);
    expect(t.dealId).toBe(310);
  });

  it('commitment: an ISO "when" string sets a valid due date', async () => {
    state.brainAiReviewItems.push({
      id: 1,
      clientId: 1,
      status: 'pending',
      ...
      sourceType: 'other',
      sourceId: 0,
      proposedPayload: { who: 'Sarah', what: 'Send proposal', when: '2026-03-01' },
    });
    const { approveReviewItem } = await importModule();
    await approveReviewItem({ clientId: 1, itemId: 1, actorId: 2 });
    const t = state.brainTasks[0];
    expect(t.dueDate).toBeInstanceOf(Date);
    expect(Number.isNaN((t.dueDate as Date).getTime())).toBe(false);
  });

  it('commitment: throws when "what" is missing', async () => {
    state.brainAiReviewItems.push({
      id: 1,
      clientId: 1,
      status: 'pending',
      ...
      sourceType: 'other',
      sourceId: 0,
      proposedPayload: { who: 'Sarah' },
    });
    const { approveReviewItem } = await importModule();
    await expect(
      approveReviewItem({ clientId: 1, itemId: 1, actorId: 2 }),
    ).rejects.toThrow(/missing what/i);
  });

  it('compliance_warning: inserts a brain_tasks row with complianceFlag=true and severity "high" → priority "urgent"', async () => {
    state.brainAiReviewItems.push({
      id: 1,
      clientId: 1,
      status: 'pending',
      ...
      sourceType: 'other',
      sourceId: 0,
      proposedPayload: { message: 'Missing disclosure', severity: 'high' },
    });
    const { approveReviewItem } = await importModule();
    const res = await approveReviewItem({ clientId: 1, itemId: 1, actorId: 2 });
    expect(res.resultEntityType).toBe('brain_task');
    ...
    expect(t.title).toBe('Compliance: Missing disclosure');
  });

  it('compliance_warning: defaults to priority "medium" when severity is undefined', async () => {
    state.brainAiReviewItems.push({
      id: 1,
      clientId: 1,
      status: 'pending',
      ...
      sourceType: 'other',
      sourceId: 0,
      proposedPayload: { message: 'Unclear disclosure' },
    });
    const { approveReviewItem } = await importModule();
    await approveReviewItem({ clientId: 1, itemId: 1, actorId: 2 });
    const t = state.brainTasks[0];
  ...
  it('relationship_update: inserts a brain_notes row tagged relationship-update', async () => {
    state.brainAiReviewItems.push({
      id: 1,
      clientId: 1,
      status: 'pending',
      ...
      sourceType: 'other',
      sourceId: 0,
      proposedPayload: { field: 'sentiment', value: 'Warming up to the deal', rationale: 'Positive tone in the meeting' },
    });
    const { approveReviewItem } = await importModule();
    const res = await approveReviewItem({ clientId: 1, itemId: 1, actorId: 2 });
    expect(res.resultEntityType).toBe('brain_note');
    expect(state.brainNotes).toHaveLength(1);
    const n = state.brainNotes[0];
    expect(n.source).toBe('ai_review');
    expect(n.reviewItemId).toBe(1);
    expect(n.tags).toContain('relationship-update');
  });

  it('relationship_update: throws when field or value is missing', async () => {
    state.brainAiReviewItems.push({
      id: 1,
      clientId: 1,
      status: 'pending',
      ...
      sourceType: 'other',
      sourceId: 0,
      proposedPayload: { field: 'sentiment' },
    });
    const { approveReviewItem } = await importModule();
    await expect(
      approveReviewItem({ clientId: 1, itemId: 1, actorId: 2 }),
    ).rejects.toThrow(/missing field or value/i);
  });
