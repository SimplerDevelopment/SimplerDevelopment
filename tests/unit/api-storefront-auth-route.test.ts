...
// Isolate from the real in-memory rate limiter — the storefront auth routes now
// correctly `await checkRateLimit`, so without this mock the shared per-key
// in-memory window accumulates across tests in this file and the 6th+ same-action
// request would 429. (No unit test here asserts the 429 path.)
