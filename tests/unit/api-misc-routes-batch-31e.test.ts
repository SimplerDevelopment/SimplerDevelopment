...
vi.mock('@/lib/google/oauth-state', () => ({
  ...
  // The callback route now verifies the signed CSRF state (AUTH79-014) and
  // reads `returnTo` for the site-scoped redirect. All callback tests below
  // seed client id 5 (getPortalClientMock) + session user id '7' (SESSION)
  // and pass `state=42` in the request URL, so return a payload matching
  // that fixture shape — site 42 belongs to client 5, initiated by user 7.
  verifyState: () => ({ clientId: 5, userId: 7, surfaces: [], returnTo: '42' }),
}));
