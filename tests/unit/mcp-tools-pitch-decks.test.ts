...
describe('registerPitchDecksTools — registration & scope gating', () => {
  ...
  it('registers only write tools with decks:write', () => {
    ...
    // decks_delete now requires the granular decks:delete scope, not decks:write.
    expect(tools.has('decks_delete')).toBe(false);
  });

  it('registers decks_delete only with decks:delete', () => {
    const tools = registerAll(['decks:delete']);
