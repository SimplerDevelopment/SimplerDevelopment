...
describe('listNotes', () => {
  ...
  it('filters by needsReview — surfaces only AI-created notes awaiting human review (UAG-004)', async () => {
    seedNote({ id: 32, clientId: 1, needsReview: true });
    seedNote({ id: 33, clientId: 1, needsReview: false });
    const { listNotes } = await importModule();
    const rows = await listNotes(1, { needsReview: true });
    expect(rows.map((r) => r.id)).toEqual([32]);
  });
...
describe('createNote', () => {
  ...
  it('defaults needsReview to false when omitted (UAG-004)', async () => {
    const { createNote } = await importModule();
    const created = await createNote({ clientId: 1, title: 'no review flag' });
    expect(created.needsReview).toBe(false);
  });

  it('persists needsReview: true when passed — flags agent-created notes for human review (UAG-004)', async () => {
    const { createNote } = await importModule();
    const created = await createNote({
      clientId: 1,
      title: 'AI-authored note',
      source: 'ai_review',
      needsReview: true,
      createdBy: 9,
    });
    expect(created.needsReview).toBe(true);
    expect(created.source).toBe('ai_review');
  });
