...
describe('lib/ab/assign', () => {
  ...
  describe('normalizeSplit', () => {
    ...
    it('three equal variants sum to exactly 100 (QAD-044: largest-remainder, not 99)', () => {
      const out = normalizeSplit({ a: 1, b: 1, c: 1 });
      expect(out.a + out.b + out.c).toBe(100);
      // 34/33/33 in some order — each within one unit of the exact 33.33.
      expect(Object.values(out).every((v) => v === 33 || v === 34)).toBe(true);
    });
