...
// GradientBuilder — only used by StyleSettings.tsx (Style tab), pulled in
// below for the section-block background-color test now that Background
// Color/Image, Text Color, Max Width and Font Family live there instead of
// on the Content tab.
vi.mock('@/components/blocks/visual/GradientBuilder', () => ({
  GradientBuilder: ({ onChange }: any) => (
    <div data-testid="gradient-builder">
      <button type="button" data-testid="gradient-set-color" onClick={() => onChange({ backgroundColor: '#abcdef', backgroundGradient: '' })}>
        set-color
      </button>
    </div>
  ),
...
// Background Color / Max Width / Text Color for section blocks now live on
// the Style tab rather than the Content tab — imported directly for the
// "section splitColor branches" describe block below.
...
describe('BlockContentEditor — image block', () => {
  ...
  it('uses media api with siteId when provided', () => {
    // The image content-tab media picker now always points at the full
    // client-wide media library (/api/portal/media), matching the Style tab's
    // background "Select Media" picker — see MediaPanel.tsx. It's tenant-safe
    // (server-side filtered by clientId); siteId no longer narrows the URL.
    ...
    expect(mp.getAttribute('data-api')).toBe('/api/portal/media');
  });
});
...
describe('BlockContentEditor — code block', () => {
  ...
  it('updates language via SelectField', () => {
    // Language is now a constrained SelectField (dropdown of known languages),
    // not a freeform text Field.
    ...
    expect(onUpdate).toHaveBeenCalledWith({ language: 'typescript' });
  });
});
...
describe('BlockContentEditor — cta block', () => {
  it('renders CTA content fields without a background control (VEQA-065)', () => {
    const { container } = renderBlock({ id: 'c1', type: 'cta', order: 0, title: 'T' });
    // Background moved to the Style tab — the CTA Content tab no longer has a <select>.
...
describe('BlockContentEditor — section block', () => {
  it('renders section with split details panel and nested block count', () => {
    // Font Family (and Background Color/Image, Text Color, Max Width) moved
    // off the Content tab entirely — they now live on the Style tab
    // (StyleSettings.tsx) per LayoutPanel.tsx's comment. The Content tab's
    // section panel is left with HTML Tag + the diagonal-split details panel
    // + the nested-block count.
    ...
    expect(container.textContent).toContain('Diagonal Split (advanced)');
  });
...
describe('BlockContentEditor — ListEditor SortableListItem expansion', () => {
  ...
  // VEQA-068 follow-up: the per-card icon color control was added to
  // DynamicPanel's CardGridBlockSettings (a different, non-portal block
  // editor surface), but the portal visual editor's right panel renders
  // card-grid via MarketingPanel -> ListEditor, which never got the field.
  // Guard against the portal panel regressing back to missing it.
  it('expanding a card-grid item renders an Icon Color control and updates the right card', () => {
    const { container, onUpdate } = renderBlock({
      id: 'cg1', type: 'card-grid', order: 0,
      cards: [
        { id: 'cd1', title: 'Card One', description: 'Desc', icon: 'star', link: '/x' },
        { id: 'cd2', title: 'Card Two', description: 'Desc 2', icon: 'bolt', iconColor: 'brand.accent' },
      ],
      columns: 3,
    });
    const headers = Array.from(container.querySelectorAll('div')).filter((d) =>
      d.className.includes('cursor-pointer') && (d.textContent?.includes('Card One') || d.textContent?.includes('Card Two')),
    );
    const cardTwoHeader = headers.find((d) => d.textContent?.includes('Card Two'));
    fireEvent.click(cardTwoHeader!);

    const colorInput = container.querySelector('[data-testid="color-Icon Color"]') as HTMLInputElement;
    expect(colorInput).toBeTruthy();
    expect(colorInput.value).toBe('brand.accent');

    fireEvent.change(colorInput, { target: { value: 'brand.primary' } });
    expect(onUpdate).toHaveBeenCalledWith({
      cards: [
        { id: 'cd1', title: 'Card One', description: 'Desc', icon: 'star', link: '/x' },
        { id: 'cd2', title: 'Card Two', description: 'Desc 2', icon: 'bolt', iconColor: 'brand.primary' },
      ],
    });
  });
...
describe('BlockContentEditor — marquee item editing branches', () => {
  it('image item shows alt + link fields and edits each', () => {
    // Marquee items are now collapsible and start collapsed (expandedId is
    // null by default) — the per-item fields only render once the row header
    // is clicked to expand it.
    const { container, onUpdate } = renderBlock({
      id: 'mq1', type: 'marquee', order: 0,
  ...
  it('icon item shows icon name field', () => {
    // Same collapsed-by-default behavior as above, plus the icon-name field
    // is now an IconPicker (mocked below with an `ip-input-Icon` testid),
    ...
    const iconNameInput = container.querySelector('[data-testid="ip-input-Icon"]') as HTMLInputElement;
    expect(iconNameInput.value).toBe('star');
...
describe('BlockContentEditor — section splitColor branches', () => {
  ...
  it('updates background color + maxWidth + text color', () => {
    // Background Color, Max Width and Text Color moved from this Content-tab
    // panel to the Style tab (StyleSettings.tsx) — see the comment atop
    // LayoutPanel.tsx's section-block branch. Exercise them there instead.
    const onChange = vi.fn();
    const block = { id: 's1', type: 'section', order: 0, blocks: [], style: {} } as any;
    const { container } = render(
      <StyleSettings block={block} onChange={onChange} currentViewport="desktop" />,
    );
    const openStyleSection = (title: string) => {
      const header = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes(title));
      if (!header) throw new Error(`Could not find section: ${title}`);
      fireEvent.click(header);
    };

    openStyleSection('Background');
    fireEvent.click(screen.getByTestId('gradient-set-color'));
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ style: expect.objectContaining({ backgroundColor: '#abcdef' }) }),
    );

    openStyleSection('Size');
    const maxWidth = screen.getAllByPlaceholderText('none')[0] as HTMLInputElement;
    fireEvent.change(maxWidth, { target: { value: '1200px' } });
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({
        responsiveStyle: expect.objectContaining({
          desktop: expect.objectContaining({ maxWidth: '1200px' }),
        }),
      }),
    );

    openStyleSection('Typography');
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ style: expect.objectContaining({ color: '#000' }) }),
    );
  });
});
