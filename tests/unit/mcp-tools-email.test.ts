...
describe('registerEmailTools — registration', () => {
  ...
  it('registers the canonical write tools when scopes include email:write', () => {
    ...
    // The delete tools now require the granular email:delete scope, not email:write.
    for (const name of ['email_campaigns_delete', 'email_lists_delete', 'email_subscribers_remove']) {
      expect(tools.has(name), `should NOT register ${name} under email:write alone`).toBe(false);
    }
  });

  it('registers the delete tools only when email:delete scope is granted', () => {
    const tools = registerAll(['email:delete']);
    expect(tools.has('email_campaigns_delete')).toBe(true);
    expect(tools.has('email_lists_delete')).toBe(true);
    expect(tools.has('email_subscribers_remove')).toBe(true);
...
describe('email_lists_delete', () => {
  it('returns not-found if list missing', async () => {
    dbState.selectRows = [];
    const tools = registerAll(['email:read', 'email:write', 'email:send', 'email:delete']);
