...
/**
 * Unit tests for `app/portal/crm/proposals/page.tsx` — the CRM Proposals-only
 * list page. It fetches proposals + lookup data (contacts/companies/deals/
 * templates) on mount, and exposes new-proposal / duplicate / send flows. We
 * stub `next/navigation`, `fetch`, and `navigator.clipboard` and drive every
...
async function renderPage() {
  const result = render(<ProposalsPage />);
  await waitFor(() => {
...
describe('ProposalsPage', () => {
  describe('initial load', () => {
    it('renders loaded proposals list by default', async () => {
      await renderPage();
    ...
    it('renders the header subtitle', async () => {
      await renderPage();
      expect(screen.getByText('Send proposals to clients')).toBeTruthy();
    });
  ...
  describe('proposals list — stats + table', () => {
