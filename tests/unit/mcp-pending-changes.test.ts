...
describe('stageOrApply — apply path', () => {
  ...
  it('drives approval off ctx.requireCmsApproval, NOT a keyId lookup — a numerically colliding portal key is ignored (QAD-048)', async () => {
    // Regression: an OAuth token whose id (999) collides with a portal API key
    // row that DOES require approval must still apply directly, because the
    // credential's OWN flag (requireCmsApproval:false) governs — the old code
    // read portal_api_keys by id and cross-picked the wrong identity's policy.
    state.keys.push({ id: 999, requireCmsApproval: true });
    const { stageOrApply } = await importModule();
    const apply = vi.fn(async () => ({ id: 1 }));
    const result = await stageOrApply({
      ctx: makeCtx({ keyId: 999, requireCmsApproval: false }) as never,
      entityType: 'post',
      entityId: null,
      operation: 'create',
      summary: 's',
...
describe('stageOrApply — stage path', () => {
  it('does NOT call apply and inserts a pending-change row when approval is required', async () => {
    ...
    const apply = vi.fn(async () => ({ id: 1 }));
    const result = await stageOrApply({
      ctx: makeCtx({ requireCmsApproval: true }) as never,
      entityType: 'post',
      entityId: null,
      operation: 'create',
      summary: 'staged post',
