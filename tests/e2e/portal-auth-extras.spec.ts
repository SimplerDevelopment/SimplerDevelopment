...
test.describe('Portal change-password @auth @password', () => {
  ...
  test('POST changes password and old credentials no longer authenticate', async ({ clientApi }) => {
    ...
    // Old password should now fail. NextAuth's credentials callback REDIRECTS to
    // the login page on bad creds (a 200), so ApiClient.ensure() can't surface
    // the failure via a thrown status — verify instead that a login with the old
    // password yields no working session (an authed call is rejected with 401).
    const tryOld = new ApiClient(email, oldPassword);
    await tryOld.ensure().catch(() => {});
    const oldCheck = await tryOld.get('/api/portal/default-portal');
...
test.describe('Portal default-website @auth @default-website', () => {
  ...
  test('GET returns websites for the admin (seeded owner member of the demo client)', async ({ adminApi }) => {
    // seed-admin-e2e makes admin@example.com an OWNER member of the demo client
    // so adminApi can exercise owner-gated flows — so this returns that client's
    // websites, not a 404. (A genuinely client-less user's 404 is unit-covered.)
...
test.describe('Portal my-subdomain @auth @my-subdomain', () => {
  ...
  test('GET for the admin (seeded owner member of the demo client) returns its portal', async ({ adminApi }) => {
    // admin@example.com is a seeded owner member of the demo client, so it has at
    // least one portal — not the empty list a genuinely client-less user gets.
    const res = await adminApi.get('/api/portal/my-subdomain');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.data?.portals)).toBe(true);
    expect(res.data.portals.length).toBeGreaterThan(0);
    expect(typeof res.data?.needsChoice).toBe('boolean');
  });
});
...
test.describe('Portal resolve-subdomain @auth @resolve-subdomain', () => {
  ...
  test('GET resolves a subdomain co-owned via shared client membership', async ({ clientApi, adminApi }) => {
    // admin@example.com is seeded as an owner member of the SAME demo client that
    // clientApi owns, so it can resolve that client's subdomain — resolution is
    // client-scoped, not per-user. Genuine cross-tenant denial (a caller with no
    // membership in the owning client) is covered by the tenancy integration
    // suite (bun test:tenancy), the designated gate for cross-tenant leaks.
    const { website, cleanup } = await createTestWebsite(clientApi);
    cleanups.push(cleanup);
    expect(website.subdomain).toBeTruthy();
