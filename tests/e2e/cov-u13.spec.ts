...
test.describe('Sites — Custom code draft-then-publish @sites @custom-code', () => {
  ...
  test('POST /code/publish returns 400 when no draft exists', async ({ clientApi }) => {
    // Discard any pending draft so the site has no draft to publish. (A PUT
    // with empty-string customCss/customJs does NOT clear the draft — the
    // route treats '' as a real draft value — so we must use /code/discard.)
    await clientApi.post(`/api/portal/cms/websites/${siteId}/code/discard`, {});

    // With no draft pending, publish should be refused
    const pub = await clientApi.post(`/api/portal/cms/websites/${siteId}/code/publish`, {});
