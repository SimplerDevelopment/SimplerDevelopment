...
async function fetchNotifs(
  api: { get: (path: string) => Promise<{ data: { data: NotifRow[] } }> },
  limit?: number,
): Promise<NotifRow[]> {
  // GET /notifications caches per (clientId, userId, limit, unreadOnly) with a 15s
  // TTL, and Next 16 route-handler revalidateTag is EVENTUAL (only Server-Action
  // updateTag is immediate). So a test asserting a *just-emitted* notification for
  // a user whose default-limit cache was already warmed by earlier tests can pass
  // a distinct `limit` to read a cold (fresh-from-DB) cache key immediately,
  // rather than waiting out the TTL. This tests the emission logic, not the cache.
  const path = limit ? `/api/portal/crm/notifications?limit=${limit}` : '/api/portal/crm/notifications';
...
test.describe('Portal CRM — notification emitters @crm @notifications', () => {
  ...
  test('proposal_viewed: notifies the creator on first view only, no duplicate on second view', async ({ clientApi }) => {
    ...
    // Distinct limits (77 / 78 below) for the two reads so each is a cold cache
    // key → a fresh-from-DB read, uncontaminated by the default-limit cache other
    // tests warmed. Two different values (not the same) keep the second read fresh
    // too, so the dedup assertion can still catch a genuine duplicate insert.
    const ownerNotifs1 = await fetchNotifs(clientApi, 77);
    ...
    const ownerNotifs2 = await fetchNotifs(clientApi, 78);
