...
test.describe('Custom Fields CRUD @cms @custom-fields', () => {
  ...
  test('setup: create test website and resolve post type', async ({ clientApi }) => {
    ...
    // Get or create a post type for testing, scoped to the portal site
    const ptRes = await clientApi.get(`/api/portal/cms/websites/${siteId}/content-types`);
    expect(ptRes.status).toBe(200);
    const pageType = ptRes.data.data.find((pt: { slug: string }) => pt.slug === 'page');
    if (pageType) {
      postTypeId = pageType.id;
    } else {
      const createRes = await clientApi.post(`/api/portal/cms/websites/${siteId}/content-types`, {
        name: 'Page',
        slug: 'page',
      ...
      cleanups.push(async () => {
        await clientApi.delete(`/api/portal/cms/websites/${siteId}/content-types/${postTypeId}`).catch(() => {});
      });
    }
  });
