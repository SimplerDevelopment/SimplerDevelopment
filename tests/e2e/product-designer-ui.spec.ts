...
test.describe('Product designer — storefront UI @product-designer @ui', () => {
  test.beforeAll(async ({ clientApi }) => {
    ...
    // createTestWebsite() leaves publicAccess at its schema default (false),
    // which renders a "This site is private" overlay on the public storefront
    // and hides the "Customize this product" CTA under test. Flip it on, same
    // as gap-sites-deck-preview-coverage.spec.ts and cov-u40.spec.ts do.
  ...
  test('welcome screen "Add Text" entry switches into the add-text panel', async ({ page }) => {
    ...
    // The WelcomeScreen renders when no layers exist yet. NOTE: ProductDesigner
    // mounts <LeftPanel /> twice — once in the desktop pane (`.sdEditor`,
    // `hidden md:flex`) and once in the mobile tab pane (`md:hidden`) — so
    // both copies of WelcomeScreen (and this data-testid) are simultaneously
    // present in the DOM; only one is CSS-hidden depending on viewport width.
    // That is a real duplicate-testid smell worth a follow-up (dedupe the
    // testid or don't double-mount both panels), but for this test we just
    // need the copy visible at Playwright's default desktop viewport
    // (1280x720, above Tailwind's `md` breakpoint) — scope to the desktop
    // pane so the locator resolves to exactly one element.
    const addText = page.locator('.sdEditor [data-testid="welcome-add-text"]');
  ...
  test.skip('add a text layer → Save → reload restores it', async ({ page }) => {
    ...
    // 1) Welcome → Add Text → type (scoped to the desktop pane — see the
    //    duplicate-testid note on the enabled "switches into the add-text
    //    panel" test above).
    await page.locator('.sdEditor [data-testid="welcome-add-text"]').click();
