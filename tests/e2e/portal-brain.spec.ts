...
test.describe('Portal Brain — Relationships @brain @brain-relationships', () => {
  ...
  test('GET /relationships/[id] returns detail', async ({ clientApi }) => {
    ...
    expect(res.status).toBe(200);
    // getRelationship() (lib/brain/relationships.ts ~178-215) returns a
    // RelationshipDetail { overlay, underlying, contacts, meetings, tasks } —
    // the overlay id lives at data.overlay.id, not data.id.
    expect(res.data.data.overlay.id).toBe(id);
  });
