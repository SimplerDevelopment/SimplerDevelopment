...
test.describe('Onboarding wizard browser flow @onboarding', () => {
  ...
  test('Skip button stamps complete and bounces to dashboard', async ({ page, loginAsOtherClient }) => {
    await loginAsOtherClient(page);
    // beforeEach above reopens clientApi's user; stamp reopenedAt on this page's
    // session too (page.request shares the session cookies) so the skip button
    // — now gated on "reached done OR reopened" — is actually visible.
    await page.request.post(BASE_URL + '/api/portal/onboarding', { data: { action: 'reopen' } });
