...
test.describe('Brain Agent — auth guards @brain-agent @auth', () => {
  ...
  test('rejects unknown conversationId with 404', async ({ clientApi }) => {
...
test.describe('Brain Agent — SSE stream @brain-agent', () => {
  ...
  // untagged from @critical: needs AI env CI doesn't provide (JUL9-014)
  test(
    'returns text/event-stream with intent + done frames',
    async ({ clientApi }) => {
      await ensureBrainEnabled(clientApi);
...
test.describe('Brain Agent — conversation continuation @brain-agent', () => {
  ...
  // untagged from @critical: needs AI env CI doesn't provide (JUL9-014)
  test(
    'second POST with conversationId continues the same conversation',
    async ({ clientApi }) => {
      await ensureBrainEnabled(clientApi);
