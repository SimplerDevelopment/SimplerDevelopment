...
// untagged from @critical: needs approval-workflow env CI doesn't provide (JUL9-014)
test.describe('Portal Approvals — staging + decision lifecycle @approvals @mutations', () => {
  ...
  test('rejects unauthenticated mutations (401) @critical', async ({ unauthApi }) => {
    const cases = [
  ...
  test('bulk routes reject empty + oversized batches with 400 @critical', async ({ clientApi }) => {
