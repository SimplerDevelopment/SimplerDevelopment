...
//
// MULTI-01: The submit route (app/api/surveys/[slug]/route.ts POST) checks
// `survey.allowMultiple` and blocks a same-email second submission with 403.
...
test.describe('Surveys — allowMultiple=false blocks duplicate submission @surveys @allow-multiple', () => {
  ...
  test('second same-email submission returns 403 when allowMultiple=false',
    async ({ clientApi, unauthApi }) => {
      const ts = Date.now();
      ...
      // Second submission with same email — blocked with 403 (MULTI-01)
