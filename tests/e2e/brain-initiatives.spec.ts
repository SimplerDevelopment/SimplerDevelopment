...
test.describe('Brain Initiatives — lifecycle @brain @brain-initiatives-lifecycle', () => {
  test('create → list → detail → patch → close-with-lessons → reopen-from-cancelled succeeds → list reflects reactivation', async ({
    clientApi,
  }) => {
    const ts = uniq();
    ...
    try {
      ...
      // REOPEN from 'cancelled' — lib/brain/initiatives.ts reopenInitiative()
      // (~lines 486-521) intentionally allows reopening from EITHER
      // 'completed' or 'cancelled' (only a non-terminal status throws).
      // Reopening flips status back to 'active' and clears closedAt.
      ...
      expect(reopen.data?.success).toBe(true);
      const reopenedInit = reopen.data.data.initiative ?? reopen.data.data;
      expect(reopenedInit.status).toBe('active');
      expect(reopenedInit.closedAt).toBeFalsy();

      // LIST default — reopened initiative is back in the `status=active` slice.
      const listAfter = await clientApi.get(
        '/api/portal/brain/initiatives?status=active&limit=100',
      );
      expect(listAfter.status).toBe(200);
      const afterItems: Array<{ id: number }> = listAfter.data.data.items;
      expect(afterItems.some((r) => r.id === id)).toBe(true);

      // And LIST with status=cancelled no longer includes it — it's active again.
      ...
      expect(cancelledItems.some((r) => r.id === id)).toBe(false);
    } finally {
