...
test.describe('Brain Documents — tenancy isolation @brain @brain-documents-tenancy', () => {
  test('client A documents invisible to unauth; visible to admin as a legit owner member', async ({ clientApi }) => {
    // The default seed has only one "client" portal user (client@example.com /
    // Acme Corp, client 102). Without a second client account in the env we
    // can't cross two real tenants here. scripts/seed-admin-e2e.ts (lines
    // ~90-118) deliberately provisions admin@example.com as an 'owner'-role
    // clientMembers row on this exact demo client so adminApi can exercise
    // owner-gated portal flows — admin is NOT a tenancy outsider for this
    // client. The real isolation boundary this test can exercise is
    // unauth vs. authenticated; admin is expected to see the record.
    const ts = uniq();
    let docId: number | null = null;
    ...
    try {
      ...
      // Admin is an owner-role member of this same demo client (seeded by
      // scripts/seed-admin-e2e.ts), so the active-client resolver returns
      // the same clientId as clientApi — admin legitimately sees the doc.
      const adminGet = await adminApi.get(`/api/portal/brain/documents/${docId}`);
      expect(adminGet.status).toBe(200);
      expect(adminGet.data?.success).toBe(true);
      const adminDoc = adminGet.data.data?.document ?? adminGet.data.data;
      expect(adminDoc?.id).toBe(docId);
