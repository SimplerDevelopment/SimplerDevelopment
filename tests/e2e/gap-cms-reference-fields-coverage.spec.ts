...
test.describe('CMS reference fields @gap @cms-reference', () => {
  ...
  test('PUT validates the reference points at a real post; GET resolves it', async ({ clientApi }) => {
    const put = await clientApi.put(`/api/posts/${hostId}/custom-fields`, {
      customFieldId: fieldId,
      value: String(refId),
    });
    expect(put.status).toBe(200);

    const get = await clientApi.get(`/api/posts/${hostId}/custom-fields`);
    expect(get.status).toBe(200);
    const row = (get.data.data as Array<{ customFieldId: number; fieldType: string; referencedPosts?: Array<{ id: number; title: string; slug: string }> }>)
  ...
  test('PUT rejects a reference to a non-existent post (400) and a non-id value (400)', async ({ clientApi }) => {
    expect((await clientApi.put(`/api/posts/${hostId}/custom-fields`, { customFieldId: fieldId, value: '999999' })).status).toBe(400);
    expect((await clientApi.put(`/api/posts/${hostId}/custom-fields`, { customFieldId: fieldId, value: 'abc' })).status).toBe(400);
  });
