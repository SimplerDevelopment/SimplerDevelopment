...
// Client used to satisfy the registered_app_jobs/registered_app_runs FK on
// client_id. There is no guarantee a hardcoded id (e.g. 101) exists in every
// environment's seed data (it doesn't in CI's simplerdev_test) — so this is
// resolved at runtime from the logged-in `clientApi` fixture's active client
// via GET /api/portal/clients (same resolution api-client.ts itself uses to
// pin the `sd-active-client` cookie). 101 remains only as a pre-resolution
// fallback value; every describe block that inserts a client_id-scoped row
// must call resolveTestClientId(clientApi) in its beforeAll first.
let TEST_CLIENT_ID = 101;
let testClientIdResolved = false;

/** Resolve (once) and cache the real client id backing `clientApi`'s session. */
async function resolveTestClientId(clientApi: {
  get(path: string): Promise<{ status: number; data: unknown }>;
}): Promise<number> {
  if (testClientIdResolved) return TEST_CLIENT_ID;
  const res = await clientApi.get('/api/portal/clients');
  const activeClientId = (res.data as { activeClientId?: number | null } | null)?.activeClientId;
  if (res.status === 200 && typeof activeClientId === 'number' && activeClientId > 0) {
    TEST_CLIENT_ID = activeClientId;
    testClientIdResolved = true;
  }
...
test.describe('Cron plugin-jobs-tick @plugins @cron', () => {
  ...
  test.beforeAll(async ({ clientApi }) => {
    await resolveTestClientId(clientApi);
  ...
  test('GET /api/cron/plugin-jobs-tick fires due job and bumps nextRunAt', async () => {
    ...
    // The prior TZ bug (a session-tz-interpreted `timestamp` column compared with
    // a UTC Date in the CAS predicate) is fixed: registered_app_jobs.next_run_at
    // is now `timestamptz` and fireDueJobs re-asserts `next_run_at <= now` (see
    // lib/plugins/handlers/content-tools/fire-due-jobs.ts), which is TZ-correct on
    // any session timezone. So the due job is claimed and enqueued.
    ...
    if (originalNextRunAt) {
      // The claim bumped nextRunAt from the original past time to a future slot.
      expect(currentEpochMs).toBeGreaterThan(Date.now());
    }
  });
