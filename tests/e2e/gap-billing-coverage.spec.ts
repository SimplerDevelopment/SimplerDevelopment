...
test.describe('Stripe platform webhook — signature-validation guards @gap @billing', () => {
  // The webhook route short-circuits to 500 "Stripe not configured" before
  // constructEvent() unless STRIPE_SECRET_KEY + STRIPE_WEBHOOK_SECRET are set, so
  // these signature-rejection (→400) assertions can only run where Stripe is
  // configured (local dev). In the prod-build CI matrix they self-skip rather
  // than fail. (A global dummy key isn't set here — it would break the sibling
  // no-key-path billing tests that currently pass.)
  test.skip(
    !process.env.STRIPE_SECRET_KEY || !process.env.STRIPE_WEBHOOK_SECRET,
    'Stripe webhook signature tests need STRIPE_SECRET_KEY + STRIPE_WEBHOOK_SECRET (not set in CI)',
  );
  test(
