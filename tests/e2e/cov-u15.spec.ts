...
test.describe('Pitch Decks — HTML upload creates single-slide deck @pitch-decks @upload', () => {
  ...
  test('POST /upload-html with HTML file creates a deck with one slide', async ({ clientApi }) => {
    ...
    // uploadToS3 throws a 500 when S3_ENDPOINT/S3_ACCESS_KEY_ID/S3_SECRET_ACCESS_KEY
    // aren't wired up (e.g. the prod-build/no-external-service nightly matrix).
    // Skip gracefully rather than failing on missing infra.
    if (res.status === 500) {
