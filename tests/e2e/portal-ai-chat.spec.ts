...
// untagged from @critical: needs AI env CI doesn't provide (JUL9-014)
test.describe('Portal AI Conversations @ai', () => {
  test('GET /ai/conversations lists client conversations @critical', async ({ clientApi }) => {
  ...
  test('GET /ai/conversations rejects unauthenticated @critical', async ({ unauthApi }) => {
    const res = await unauthApi.get('/api/portal/ai/conversations');
    expect(res.status).toBe(401);
  });

  test('GET /ai/conversations/:id returns 404 for non-existent conversation @critical', async ({ clientApi }) => {
    const res = await clientApi.get('/api/portal/ai/conversations/999999');
    expect(res.status).toBe(404);
  });

  test('POST /ai/chat rejects empty message @critical', async ({ clientApi }) => {
  ...
  test('POST /ai/chat rejects staff users @critical', async ({ adminApi }) => {
  ...
  test('POST /ai/chat rejects unauthenticated @critical', async ({ unauthApi }) => {
