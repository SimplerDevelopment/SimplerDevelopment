...
test.describe('Agentic OS — GET /api/admin/agentic-os/runs (localDev gate) @agentic-os', () => {
  ...
  test('admin user also receives 404 — localDev gate blocks even staff on prod @critical', async ({ adminApi }) => {
    ...
    // isLocalDev() = NODE_ENV === 'development' on the APP process. Both gate
    // states are correct: 200 when the app runs `bun dev` (gate open), 404 when
    // it runs a production build (gate closed — CI's e2e job builds + `bun start`).
    // We can't read the *app's* NODE_ENV from the test runner (its own env may
    // differ), so accept either — the load-bearing guarantee is that a staff
    // request is never a 500/leak and the gate never silently opens something it
    // shouldn't. The anonymous test above already pins MUST-NOT-200 on prod.
