...
test.describe('Brain Glossary — tenancy isolation @brain @brain-glossary-tenancy', () => {
  test('tenancy isolation: client A glossary invisible to unauth; visible to admin as a legit owner member', async ({
    clientApi,
  }) => {
    // The default seed has only one "client" portal user (client@example.com /
    // client 102 — Acme Corp). scripts/seed-admin-e2e.ts (lines ~90-118)
    // deliberately provisions admin@example.com as an 'owner'-role
    // clientMembers row on this exact demo client so adminApi can exercise
    // owner-gated portal flows — admin is NOT a tenancy outsider here.
    // Without a second real client account in the env, the isolation
    // boundary this test can actually exercise is unauth vs. authenticated;
    // admin is expected to see the record via its owner membership.
    const ts = uniq();
    let id: number | null = null;
