...
// The POST /api/test/email-events trigger is intentionally 404'd in production
// builds (NODE_ENV=production) and 403'd without an x-test-secret header — these
// tests only run against a dev server seeded via `scripts/seed-email-test-data.ts`.
// The GET list is NOT gated, so probe the POST trigger (not GET) and skip the
// file when it's unavailable rather than failing all 20 tests. beforeEach (not
// beforeAll) is used because test.skip() there reliably skips each test across
// all describes; the probe result is cached so it runs at most once.
let triggerAvailable: boolean | null = null;
test.beforeEach(async () => {
  if (triggerAvailable === null) {
    const ctx = await request.newContext({ baseURL: BASE_URL });
    const probe = await ctx.post('/api/test/email-events', { data: { event: '__probe__' } });
    const status = probe.status();
    triggerAvailable = status !== 404 && status !== 403;
    await ctx.dispose();
  }
  test.skip(
    !triggerAvailable,
    'email-events trigger endpoint unavailable (prod build / no x-test-secret) — run in dev with seed-email-test-data.ts',
  );
});
