...
/**
 * Grant a client an active service subscription of the given category so
 * paid-module entitlement gates (hasServiceAccess / crmEntitlementError) pass.
 * Upserts one catalog row per category, then one client_services row.
 */
export async function grantService(clientId: number, category: string): Promise<void> {
  const sql = getTestSql();
  const slug = `test-${category}`;
  const [svc] = await sql<{ id: number }[]>`
    INSERT INTO ${sql(TEST_SCHEMA)}.services (name, slug, category, price, active, included_ai_credits)
    VALUES (${`Test ${category}`}, ${slug}, ${category}, 0, true, 0)
