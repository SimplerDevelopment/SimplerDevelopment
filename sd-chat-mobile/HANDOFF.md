...
- `/settings/workspaces` → switch CY ↔ Acme Agency ↔ CY (session persists correctly now)
...
- Did not seed brain data on Acme Agency (100) or any other entitled tenant.
...
- `app/(auth)/meet-assistant.tsx` — replaced hardcoded "Acme Agency workspace" with dynamic `client.company` from `useAuth`.
...
- **Visual walkthrough first** — serving `~/Desktop/sd-chat-*.html` via `python3 -m http.server 8765` and side-by-siding mockup vs. app surfaced the Media chip bug, avatar gray-circle, and "Acme Agency" hardcoded copy in minutes.
