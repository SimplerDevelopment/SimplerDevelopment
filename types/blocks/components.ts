 ...
 * `HeroBlockRender`/`CtaBlockRender` are thin adapters that normalize their
 * legacy field names onto this shape and delegate to `HeroCtaBlockRender`.
 */
export interface HeroCtaBlock extends BaseBlock {
  type: 'hero-cta';
  /** 'hero' = full-bleed hero look (legacy `hero`); 'banner' = centered CTA band (legacy `cta`). */
  layout: 'hero' | 'banner';
  title: string;
  subtitle?: string;
  description?: string;
  ...
  secondaryButtonText?: string;
  secondaryButtonUrl?: string;
  /** 'hero' layout only — full-bleed background media. */
  backgroundImage?: string;
  backgroundVideo?: string;
  /** 'banner' layout only — legacy CtaBlock background band style. */
  backgroundStyle?: 'gradient' | 'solid' | 'none';
  /** 'hero' layout only — optional child blocks rendered at the bottom (e.g. trust bars), mirrors HeroBlock.blocks */
  blocks?: Block[];
}
...
export interface CardGridBlock extends BaseBlock {
  ...
  cards: Array<{
    ...
    link?: string;
    icon?: string;
    /** Per-card override for the icon's color; falls back to the theme's primary color when unset */
    iconColor?: string;
  }>;
