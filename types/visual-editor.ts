...
/**
 * Editor-only "force hover" — sent while the user is interacting with a
 * Content-tab control (e.g. the button block's Hover Effect select) so the
 * canvas can preview the block's `:hover` CSS without the mouse actually
 * being over the iframe (VEQA-041). The iframe adds a `.force-hover` class
 * to the selected block's SelectableBlock wrapper; render components mirror
 * their `:hover` rules under a `.force-hover` ancestor selector so the same
 * CSS drives both real and forced hover.
 */
export interface ForceHoverBlockPayload {
  blockId: string | null;
}
...
export const PARENT_MESSAGES = {
  ...
  FORCE_HOVER_BLOCK: 'FORCE_HOVER_BLOCK',
