...
export async function POST(req: Request) {
  // Public endpoint that inserts a row per call — throttle per IP so it can't be
  // used to flood oauth_clients or mass-register lookalike-named phishing clients.
  if (!(await checkRateLimit(`${getClientIp(req)}:oauth-register`, 10, 15 * 60 * 1000))) {
    return err(429, 'temporarily_unavailable', 'Too many registration requests. Try again later.');
  }
