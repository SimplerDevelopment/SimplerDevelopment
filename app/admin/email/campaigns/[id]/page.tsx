...
export default function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function sendCampaign() {
    if (!campaign) return;
    if (!confirm(`Send "${campaign.name}" to ${campaign.totalRecipients > 0 ? campaign.totalRecipients : 'all active'} subscribers now?`)) return;
    setSending(true);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      {sendResult && (
        <div className="bg-green-50 border border-green-200 text-green-800 text-sm px-4 py-3 rounded-lg">
          Campaign sent: {sendResult.sent} delivered, {sendResult.failed} failed out of {sendResult.total} recipients.
        </div>
      )}
      ...
      {tab === 'overview' && (
        <div className="bg-card border border-border rounded-lg divide-y divide-border">
          {[
            ...
            { label: 'Scheduled At', value: campaign.scheduledAt ? new Date(campaign.scheduledAt).toLocaleString() : '—' },
          ].map(row => (
          ...
          {campaign.status === 'draft' && (
            <div className="px-5 py-3">
              <Link href={`/admin/email/campaigns/${campaign.id}/edit`} className="text-sm text-primary hover:underline flex items-center gap-1">
      ...
      {tab === 'content' && (
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          <div className="p-5">
            <div
              className="border border-border rounded-md p-6 bg-white text-sm max-w-2xl mx-auto overflow-auto"
                ...
                {sends.map(s => (
                  <tr key={s.id}>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <span className={`material-icons text-base ${s.sentAt ? 'text-green-500' : 'text-muted-foreground'}`}>
                        {s.sentAt ? 'check_circle' : 'radio_button_unchecked'}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <span className={`material-icons text-base ${s.openedAt ? 'text-green-500' : 'text-muted-foreground'}`}>
                        {s.openedAt ? 'check_circle' : 'radio_button_unchecked'}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <span className={`material-icons text-base ${s.clickedAt ? 'text-green-500' : 'text-muted-foreground'}`}>
                        {s.clickedAt ? 'check_circle' : 'radio_button_unchecked'}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 text-center">
