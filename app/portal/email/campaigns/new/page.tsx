    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={save} className="space-y-6">
        ...
        <div className="bg-card border border-border rounded-lg p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input required value={form.name} onChange={set('name')} className={inputClass} placeholder="e.g. March Newsletter" />
              <p className="text-xs text-muted-foreground mt-1">Only visible to you, not recipients.</p>
            </div>
            <div className="sm:col-span-2">
            ...
            <div className="sm:col-span-2">
              ...
              <input value={form.previewText} onChange={set('previewText')} className={inputClass} placeholder="Short summary shown after subject in email clients" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.fromName} onChange={set('fromName')} className={inputClass} placeholder="Your Name or Company" />
            </div>
            <div>
          ...
          <div>
            ...
            {lists.length === 0 && (
              <p className="text-xs text-muted-foreground mt-1">
                No lists found. <Link href="/portal/email/lists" className="text-primary hover:underline">Create one first.</Link>
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-5 space-y-4">
          ...
          {preview ? (
            <div className="border border-border rounded-md p-4 min-h-64 bg-white text-sm overflow-auto"
              dangerouslySetInnerHTML={{ __html: form.htmlContent }} />
          ) : (
            <textarea required value={form.htmlContent} onChange={set('htmlContent')} rows={16}
        ...
        <div className="flex gap-3">
          ...
          <Link href="/portal/email/campaigns" className="px-5 py-2.5 border border-border rounded-lg text-sm text-muted-foreground hover:bg-accent">
            Cancel
          </Link>
        </div>
      </form>
