      ...
      {tab === 'segments' && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowCreateSegment(!showCreateSegment)} className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:bg-primary/90">
          ...
          {showCreateSegment && (
            <form onSubmit={handleCreateSegment} className="bg-card border border-border rounded-xl p-6 space-y-4">
              ...
              <div className="flex gap-3">
                <button type="submit" disabled={segSaving} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:bg-primary/90 disabled:opacity-50">
                  {segSaving ? 'Creating...' : 'Create Segment'}
                </button>
                <button type="button" onClick={() => setShowCreateSegment(false)} className="px-4 py-2 text-sm text-muted-foreground">Cancel</button>
              </div>
            </form>
          )}
      ...
      {tab === 'tags' && (
        <div className="space-y-4">
          <form onSubmit={handleCreateTag} className="flex items-end gap-3">
            ...
            <button type="submit" disabled={tagSaving} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:bg-primary/90 disabled:opacity-50 whitespace-nowrap">
          ...
          {tags.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map(tag => (
                <div key={tag.id} className="flex items-center gap-2 bg-card border border-border rounded-lg px-3 py-2 group">
