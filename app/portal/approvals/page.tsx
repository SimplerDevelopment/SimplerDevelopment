...
export default function PortalApprovalsPage() {
  ...
  const [keyIdFilter, setKeyIdFilter] = useState<string>('');
  ...
  const fetchList = useCallback(async () => {
    ...
    if (keyIdFilter) params.set('keyId', keyIdFilter);
    if (timeFilter !== 'any') {
      const ms = timeFilter === '24h' ? 24 * 60 * 60 * 1000 : 7 * 24 * 60 * 60 * 1000;
      params.set('since', new Date(Date.now() - ms).toISOString());
    }
    const qs = params.toString();
    const res = await safeJson<PendingItem[], { role: string | null; canManage: boolean }>(`/api/portal/approvals${qs ? `?${qs}` : ''}`);
    if (res.success && res.data) {
      setItems(res.data);
      setCanManage(res.meta?.canManage ?? false);
    }
    setLoading(false);
  }, [filter, entityTypeFilter, keyIdFilter, timeFilter]);
  ...
  // Distinct entityType / agent values present in the current result set,
  // used to populate the filter dropdowns below the status tabs.
  const entityTypeOptions = Array.from(new Set(items.map(i => i.entityType))).sort();
  const agentOptions = Array.from(
    new Map(
      items
        .filter((i): i is PendingItem & { keyId: number } => i.keyId !== null)
        .map(i => [String(i.keyId), i.keyName ?? `Key #${i.keyId}`] as const),
    ).entries(),
  );
  const hasExtraFilters = entityTypeFilter !== '' || keyIdFilter !== '' || timeFilter !== 'any';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {/* Secondary filters: entity type / agent / time */}
      <div className="flex flex-wrap items-center gap-2">
        <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <select
            ...
            className="px-2 py-1.5 rounded-md border border-border bg-background text-foreground text-xs focus:outline-none focus:ring-2 focus:ring-primary"
          >
        ...
        <div className="flex gap-0.5 rounded-md border border-border overflow-hidden text-xs">
          {([
