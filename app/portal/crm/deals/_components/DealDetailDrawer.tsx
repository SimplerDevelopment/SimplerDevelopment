...
export default function DealDetailDrawer({
  ...
  // One-level threading: when set, the compose bar posts a reply targeted at
  // this root comment's id instead of a new root comment. Replies never show
  // their own "Reply" button (one level only).
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setArtifactsLoading(true); setCommentsLoading(true);
      const [a, av, c, m] = await Promise.allSettled([ // allSettled: one failing fetch must not hang a panel
      ...
      if (cancelled) return;
      if (a.status === 'fulfilled') setArtifacts(a.value);
      if (av.status === 'fulfilled') setAvailableArtifacts(av.value);
      if (c.status === 'fulfilled') setComments(c.value);
      if (m.status === 'fulfilled') setMentionUsers(m.value);
  ...
  async function handlePostComment() {
    ...
    const res = await api.postComment(deal.id, commentBody, commentFiles, replyingToId);
  ...
  function startReply(rootId: number) {
  ...
  // One-level threading: split into root comments (parentCommentId == null)
  // and a lookup of replies grouped by their root's id. Both preserve the
  // existing fetch order (desc by createdAt).
  ...
  const replyingToComment = replyingToId != null ? comments.find((c) => c.id === replyingToId) : undefined;
  ...
  // Shared markup for both a root comment and a reply. Replies never render
  // their own "Reply" button — one level of threading only.
  function renderCommentItem(c: Comment, { isReply }: { isReply: boolean }) {
    return (
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isReply && (
              ...
              >
                <span className="material-icons text-xs">reply</span>
                Reply
              </button>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {panelTab === 'details' && (
            <form onSubmit={saveDeal} className="p-6 space-y-5">
              ...
              {/* PUX-020: the Notes UI section was removed in favor of threaded
                  deal comments. `deal.notes` / `editForm.notes` are kept as a
                  pass-through (schema + update payload untouched) so existing
                  data isn't lost; migrating old notes content into a comment
          ...
          {panelTab === 'comments' && (
            <div className="flex flex-col h-full">
              <div className="p-4 border-b border-border space-y-2">
                {replyingToId != null && (
                  <div className="flex items-center justify-between bg-accent/50 rounded-lg px-3 py-1.5 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <span className="material-icons text-sm">reply</span>
                      Replying to {replyingToComment?.authorName || 'comment'}
                    </span>
                    <button
                      type="button"
                      onClick={cancelReply}
                ...
                {commentsLoading ? (
                  ...
                  rootComments.map((root) => (
                    <div key={root.id} className="space-y-3">
                      {renderCommentItem(root, { isReply: false })}
                      {(repliesByRoot.get(root.id) ?? []).length > 0 && (
                        <div className="ml-9 pl-3 border-l-2 border-border space-y-3">
                          {repliesByRoot.get(root.id)!.map((reply) => renderCommentItem(reply, { isReply: true }))}
                        </div>
                      )}
                    </div>
                  ))
