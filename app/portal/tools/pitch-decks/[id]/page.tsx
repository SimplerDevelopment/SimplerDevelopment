...
function PitchDeckEditorContent({ id }: { id: string }) {
  ...
  async function togglePublish() {
    ...
    // Merge only status/updatedAt — a full replace clobbers unsaved slide/theme
    // edits (and re-pushes stale slides to Yjs in collab). QAD-041 Bug B.
    if (data.success && data.data) {
      const s = normalizeDeckBlockIds(data.data);
      setDeck((prev) => (prev ? { ...prev, status: s.status, updatedAt: s.updatedAt } : prev));
    }
    setPublishing(false);
  }
