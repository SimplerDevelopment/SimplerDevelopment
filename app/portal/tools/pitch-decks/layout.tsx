...
export default async function PitchDecksLayout({ children }: { children: React.ReactNode }) {
  ...
  const subscription = await checkServiceSubscription(client.id, 'pitch-decks');
  if (!subscription) {
    const service = await getServiceByCategory('pitch-decks');
    if (service) {
      return (
        <ServicePaywall
    ...
    return (
      <div className="max-w-lg mx-auto mt-8 text-center space-y-4">
        <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto">
          <span className="material-icons text-3xl text-primary">slideshow</span>
        </div>
        <h1 className="text-2xl font-bold text-foreground">Subscription Required</h1>
        <p className="text-muted-foreground text-sm">
          Pitch Decks requires an active subscription. Contact us to get started.
        </p>
      </div>
    );
  }
