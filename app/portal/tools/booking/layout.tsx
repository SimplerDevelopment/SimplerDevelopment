...
export default async function BookingLayout({ children }: { children: React.ReactNode }) {
  ...
  const subscription = await checkServiceSubscription(client.id, 'booking');
  if (!subscription) {
    const service = await getServiceByCategory('booking');
    if (service) {
      return (
        <ServicePaywall
    ...
    return (
      <div className="max-w-lg mx-auto mt-8 text-center space-y-4">
        ...
        <p className="text-muted-foreground text-sm">
          Booking requires an active subscription. Contact us to get started.
        </p>
      </div>
    );
  }
