...
// 
// NOTE: This won't run on Vercel's default serverless functions due to Playwright's size.
// Options:
// 1. Deploy to a VPS (Railway, Render, DigitalOcean)
// 2. Use Vercel with a custom Docker runtime
// 3. Use a headless browser service like Browserless.io
//
...
async function captureScreenshot(
  ...
  const filename = `${siteDomain}-${viewportName}-${Date.now()}.png`;
...
async function analyzeWebsite(url: string): Promise<AnalysisResult> {
  ...
  try {
    ...
    // Extract domain for screenshot filenames
    const siteDomain = new URL(url).hostname.replace(/\./g, '-');
    
    // Capture screenshots at different viewports and upload to S3
    const screenshots = {
      desktop: await captureScreenshot(page, VIEWPORTS.desktop, 'desktop', siteDomain),
      tablet: await captureScreenshot(page, VIEWPORTS.tablet, 'tablet', siteDomain),
      mobile: await captureScreenshot(page, VIEWPORTS.mobile, 'mobile', siteDomain),
    };
