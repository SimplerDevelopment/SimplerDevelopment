      ...
      if (invoice.billing_reason === 'subscription_cycle' && typeof invoice.customer === 'string') {
        ...
        if (client) {
          ...
          const [recent] = await db
            ...
            // NULLS LAST — an active row that was never stamped (admin-assigned
            // free service) must not mask the most recent real grant.
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (session.metadata?.type === 'module_subscription') {
        ...
        if (modClientId && modServiceIds.length > 0) {
          // Shared with the verify-session return path (OBQA-014): advisory-
          // locked upsert + guarded month-one credit grant, so this webhook,
          // its redeliveries, and the buyer's return can all process the same
          // purchase without duplicating rows or credits.
          await activateModuleSubscription({
            clientId: modClientId,
            serviceIds: modServiceIds,
            stripeSubscriptionId: (session.subscription ?? null) as string | null,
