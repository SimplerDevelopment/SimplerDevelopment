...
export async function GET(req: NextRequest, { params }: { params: Promise<{ siteId: string; orderNumber: string }> }) {
  ...
  // Fetch items with design thumbnail join, order status history, and EasyPost tracking events.
  const [itemsWithDesign, history, trackingEvents] = await Promise.all([
    db.select({
      id: orderItems.id,
