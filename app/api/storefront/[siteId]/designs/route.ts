...
export async function GET(
  ...
  try {
    ...
    // Portal-staff path — lists ALL productDesigns rows on the site (no
    // customerId/sessionId owner filter), so staff-created rows (customerId
    // and sessionId both NULL — see POST's staff branch below) are visible
    // in the "My Designs" picker instead of being permanently invisible.
    if (await isPortalStaffWithSiteAccess(req, websiteId)) {
      ...
      if (qProductId) {
        const pid = parseInt(qProductId, 10);
        if (!isNaN(pid)) staffConditions.push(eq(productDesigns.productId, pid));
      }
      const staffRows = await db.select()
        ...
        .limit(20);
      return NextResponse.json({ success: true, data: staffRows });
    }
