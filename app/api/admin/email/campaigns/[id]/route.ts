...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const sends = await db
    ...
    .where(eq(emailCampaignSends.campaignId, parseInt(id)))
    .limit(100);
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { name, subject, previewText, fromName, fromEmail, replyTo, htmlContent, scheduledAt } = body;
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!existing) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 });
  if (existing.status === 'sending') return NextResponse.json({ success: false, message: 'Cannot delete a campaign that is currently sending' }, { status: 400 });
