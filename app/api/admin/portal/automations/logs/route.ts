...
export async function GET(req: Request) {
  ...
  const logs = await db
    ...
    .from(automationLogs)
    // leftJoin: QAD-045 — a log's rule may have been deleted (ruleId is now
    // `set null`); an innerJoin would silently drop those audit rows from view.
  ...
  const data = logs.map((l) => ({ ...l, ruleName: l.ruleName ?? '(deleted automation)' }));
