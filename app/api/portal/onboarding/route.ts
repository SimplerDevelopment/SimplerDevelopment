...
import { completeOnboarding, loadOnboarding, reopenOnboarding, saveOnboardingStep, OnboardingIncompleteError } from '@/lib/onboarding/service';
...
export async function PATCH(req: Request) {
  ...
  if (answers) {
    ...
    if (typeof answers.primaryFont === 'string') patch.primaryFont = answers.primaryFont.slice(0, 255);
    if (typeof answers.secondaryFont === 'string') patch.secondaryFont = answers.secondaryFont.slice(0, 255);
    ...
    if (Array.isArray(answers.dismissedDomains)) {
      patch.dismissedDomains = answers.dismissedDomains.filter((d): d is string => typeof d === 'string').map((d) => d.slice(0, 50)).slice(0, 50);
    }
  }
