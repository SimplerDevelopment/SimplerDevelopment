...
export async function POST(req: Request) {
  ...
  // Separation of duties (BAR-001 / MEB-003): NO isSelfApproval() check here is
  // intentional. `authorizePortal` only ever yields a human `userId` (never a
  // credential keyId — bearer/mobile resolves to bearer.userId), so a credential
  // can never be the approver on this route; credential self-approval is blocked
  // where credentials actually act — the single-item MCP `approvals_approve`
  // handler (lib/mcp/approvals.ts, isSelfApproval). A human admin approving a
  // change their own session staged is allowed by design: the diff-review UI IS
  // the decision→execution separation for the human-in-the-loop path, and
  // requiring a second admin would strand single-admin tenants.
