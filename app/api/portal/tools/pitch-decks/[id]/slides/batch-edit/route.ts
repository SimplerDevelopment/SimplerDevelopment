...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Validate slide indices BEFORE the AI gate/key resolution — an out-of-range
    // index must be a clean 400 even when no AI key is configured (e.g. CI), not a
    // 500 from resolveClientApiKey throwing on a missing key.
