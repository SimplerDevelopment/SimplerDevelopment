...
export async function POST(request: NextRequest) {
  try {
    ...
    // Without this catch, a thrown error returns the framework's HTML error
    // page and the editor blows up on JSON.parse. Log + surface a real
    // message so the failure is visible in the UI and Railway logs.
    const message = err instanceof Error ? err.message : 'Unknown error';
    console.error('[html-uploads] failed:', err);
    return NextResponse.json(
      { success: false, error: `html-uploads failed: ${message}` },
      { status: 500 }
    );
  }
}
