...
export async function PUT(req: Request, { params }: { params: Promise<{ siteId: string }> }) {
  ...
  // Empty string is a legitimate draft value ("clear the published code") —
  // null is reserved for "no pending draft". Coercing '' to null here made a
  // cleared editor unpublishable: publish saw null and 400'd "No draft
  // changes to publish", so published CSS/JS could never be removed (QAD-006).
  if (customCss !== undefined) patch.draftCustomCss = customCss;
  if (customJs !== undefined) patch.draftCustomJs = customJs;
