...
export async function POST(req: Request): Promise<Response> {
  ...
  // ── 3. Plan-gate (currently a pass-through / extension point — post
  // BYOK-inversion, checkAiPlanGate always returns { allowed: true }; kept as
  // a call site for future per-tier/per-provider gating). See plan-gate.ts.
  ...
  // ── 3b. Validate an explicitly-referenced conversation up-front so an unknown
  // conversationId is a clean 404 regardless of AI-key/credit availability — the
  // key resolution below can 402 when no provider key is configured (e.g. CI),
  // and that must not mask a genuine 404.
  if (conversationId) {
    const [conv] = await db
      .select({ id: aiConversations.id, clientId: aiConversations.clientId })
  ...
  // ── 4. Resolve key (BYOK > platform). A missing provider key (no BYOK row and
  // no platform env key — e.g. CI / no configured credits) makes
  // resolveClientApiKey throw; surface that as a clean 402 (not a bare 500) so
  // callers can treat it as "AI unavailable" — the SSE e2e tests self-skip on 402.
  let resolved: Awaited<ReturnType<typeof resolveClientApiKey>>;
  try {
    resolved = await resolveClientApiKey({ clientId: client.id, provider: 'anthropic' });
  } catch {
    return jsonError(
      402,
      'No AI provider key available. Add a BYOK key in Settings → API Keys or enable platform AI credits.',
    );
  }
