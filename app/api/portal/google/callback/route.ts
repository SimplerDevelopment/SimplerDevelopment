...
export async function GET(req: Request) {
  ...
  // Verify the signed state and enforce it was issued to THIS user + client.
  // This is the CSRF defense (AUTH79-014): an attacker can't mint a state bound
  // to the victim's user/site, so a forced callback fails here.
  let websiteId: number;
  try {
    const payload = verifyState(stateParam);
    if (payload.userId !== userId || payload.clientId !== client.id) {
      return NextResponse.redirect(`${origin}/portal/dashboard?google=error`);
    }
    websiteId = parseInt(String(payload.returnTo ?? ''), 10);
  ...
  // Defense in depth: confirm the site still belongs to this client.
  const [site] = await db
    .select()
    .from(clientWebsites)
