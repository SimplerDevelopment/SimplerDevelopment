...
export async function GET(
  ...
  if (isFirstView) {
    ...
    if (recipientUserId) {
      ...
      // Awaited (not fire-and-forget): createCrmNotification's revalidateTag()
      // only busts the notifications unstable_cache while the request's async
      // work-store is still open (Next 16). An unawaited call's continuation
      // runs after the handler returns, so the tag purge silently no-ops and the
      // recipient keeps seeing a stale (pre-insert) cached list for the 15s TTL.
      // The .catch keeps a notification failure from failing the view response.
