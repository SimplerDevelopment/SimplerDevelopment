...
export async function DELETE(
  ...
  try {
    ...
    // Cascade to sub-fields: customFields.parentId is a bare self-reference with
    // NO database FK (see lib/db/schema/cms.ts), so deleting a repeater/group
    // field would otherwise orphan its children — they'd keep surfacing via
    // GET /api/custom-fields and stay attached to content forever. Delete the
    // direct children first. (ponytail: the editor produces one nesting level;
    // make this recursive if deeper nesting is ever introduced.)
    await db.delete(customFields).where(eq(customFields.parentId, customFieldId));
