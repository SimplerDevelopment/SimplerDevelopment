...
export default function PublicSurveyPage() {
  ...
  function getNextPage(): number {
    // Check if any field on current page has goToPage logic
    const pageFields = pages[currentPage] || [];
    for (const field of pageFields) {
  ...
  if (loading) {
    return (
      <div className={`${bgClass} flex items-center justify-center py-20`}>
  ...
  // Count visible question fields (not headings) for numbering
  ...
  return (
    <div className={`${bgClass} py-8 px-4`} style={wrapperStyle}>
      <div className="max-w-2xl mx-auto">
        ...
        {!isEmbed && (
          <p className="text-center text-xs text-gray-400 mt-6">Powered by SimplerDevelopment</p>
        )}
      </div>
    </div>
  );
