...
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      //
      // Thresholds ARE the coverage gate. They enforce on any full-suite run
      // (local `scripts/test.sh --layer=unit`, non-sharded CI). Sharded CI runs
      // each shard over ~1/N of the suite, so per-shard coverage is a fraction
      // of the whole and would falsely fail every floor — so shard collection
      // runs set VITEST_SKIP_THRESHOLDS=1 to collect-only, and the merge job
      // re-enables enforcement on the COMBINED coverage. Default (env unset) =
      // enforced, so existing behavior everywhere else is unchanged.
      ...(process.env.VITEST_SKIP_THRESHOLDS
        ? {}
        : {
            thresholds: {
