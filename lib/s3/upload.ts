...
import { getS3Client, getPublicS3Client, getBucketName } from './client';
...
export async function presignPut(options: PresignPutOptions): Promise<PresignPutResult> {
  // Presigned URLs are fetched directly by the browser/caller, not by our
  // server — sign against the public-facing client (see client.ts) so the
  // signature matches the host the caller actually hits.
