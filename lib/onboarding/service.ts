...
/** Thrown when a fresh signup tries to skip/complete without having reached
 *  'done' in the wizard or having their session reopened. */
...
export async function reopenOnboarding(userId: number, clientId: number | null): Promise<OnboardingState> {
  ...
  const answers = { ...(current?.answers as OnboardingAnswers ?? {}), reopenedAt: new Date().toISOString() };

  await db.update(userOnboarding)
    .set({ completedAt: null, step: 'welcome', answers, updatedAt: new Date() })
    .where(eq(userOnboarding.userId, userId));
  return loadOnboarding(userId, clientId);
}
...
async function mirrorBrandAnswers(clientId: number, patch: Partial<OnboardingAnswers>): Promise<void> {
  ...
  // Primary font = headings, secondary = body copy.
  const profileSet = {
    ...(brandColor ? { primaryColor: brandColor } : {}),
    ...(patch.primaryFont ? { headingFont: patch.primaryFont } : {}),
    ...(patch.secondaryFont ? { bodyFont: patch.secondaryFont } : {}),
  };

  if (Object.keys(profileSet).length > 0) {
    // Update the default brand profile's color/fonts (and create one if
    // none exists). Other fields are left alone — the user can refine in
    ...
    if (existing) {
      await db.update(brandingProfiles)
        .set({ ...profileSet, updatedAt: new Date() })
        .where(eq(brandingProfiles.id, existing.id));
    } else {
      await db.insert(brandingProfiles).values({
        clientId,
