...
/**
 * Whether the auth rate-limiter should be bypassed — true only in test/CI
 * (`DISABLE_AUTH_RATE_LIMIT=1`, exported by scripts/test.sh so the e2e run's
 * many logins/password calls share one localhost IP bucket without tripping the
 * limit) or under `NODE_ENV=test`. Mirrors lib/auth.ts's login-path gate so the
 * credential-mutating password routes (change/forgot/reset) stay consistent.
 * A function (not a load-time const) so it reflects env set after module load.
