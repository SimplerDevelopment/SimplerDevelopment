...
export const crmDealComments = pgTable('crm_deal_comments', {
  ...
  // Nullable self-reference for one-level threading (replies); NULL = a root comment.
  // No FK constraint declared — matches the kanban_cards.parentCardId convention that
  // sidesteps drizzle's self-reference circular-type footgun. Referential integrity
  // (parent belongs to the same deal) is enforced in the route/handler.
