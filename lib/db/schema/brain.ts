...
import { pgTable, serial, varchar, text, timestamp, boolean, integer, json, jsonb, uniqueIndex, index, vector, type AnyPgColumn } from 'drizzle-orm/pg-core';
...
export const automationRules = pgTable('automation_rules', {
  ...
  // UAG-002: when true, this rule's HIGH-RISK actions (per the gate matrix) stage
  // for human approval instead of executing unattended. AI-authored rules default
  // true; human-authored rules default false so existing automations keep running.
  ...
  schedule: jsonb('schedule').$type<AutomationSchedule>(),
...
export const automationLogs = pgTable('automation_logs', {
  id: serial('id').primaryKey(),
  clientId: integer('client_id').notNull().references(() => clients.id, { onDelete: 'cascade' }),
  // QAD-045: `set null` (nullable) so the audit trail survives rule deletion — cf. agentActionLog.ruleId below.
...
export const brainNotes = pgTable('brain_notes', {
  ...
  // UAG-004: live agent-created notes (brain_create_note) set this true so they
  // are flagged for human review rather than committed as canonical truth.
  // Mirrors brain_tasks.needs_review; queryable via listNotes({ needsReview }).
  needsReview: boolean('needs_review').default(false).notNull(),
