...
export interface PortalMcpContext {
   ...
   * credential itself (portal_api_keys.require_cms_approval or
   * oauth_access_tokens.require_cms_approval) so the two id spaces are never
   * conflated — carry the flag on the context rather than re-querying by keyId
   * downstream (keyId collides across the two token tables; see QAD-048).
   */
