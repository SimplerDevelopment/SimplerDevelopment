...
export async function createSelfServeAccount(input: {
  ...
  const weakPassword = validatePasswordStrength(input.password, { email, name });
  if (weakPassword) throw new SignupError(weakPassword);
  ...
  // Owner membership row — team lists, @mentions, and card assignment all
  // resolve people through client_members; without this the primary owner is
  // invisible to their own workspace (QAD-008; scripts/backfill-client-members.ts
  // exists for pre-fix clients).
...
export async function findOrCreateGoogleUser(input: {
  ...
  // Owner membership row — see the credentials path above (QAD-008).
  await db.insert(clientMembers).values({ clientId: client.id, userId: user.id, role: 'owner' });
