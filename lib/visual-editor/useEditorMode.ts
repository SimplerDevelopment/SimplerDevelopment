...
interface EditorState {
  ...
  /** VEQA-041 — set by PARENT_MESSAGES.FORCE_HOVER_BLOCK while the user
   *  interacts with a Content-tab control (e.g. Hover Effect). Distinct from
   *  hoveredBlockId, which reflects real mouse-over-canvas hover for the
   *  selection outline. SelectableBlock adds `.force-hover` when this
   *  matches its blockId so render components can preview `:hover` CSS. */
  forceHoverBlockId: string | null;
  ...
  useEffect(() => {
    ...
    function handleMessage(event: MessageEvent) {
      ...
      switch (event.data.type) {
        ...
        case PARENT_MESSAGES.FORCE_HOVER_BLOCK: {
          const { blockId } = event.data.payload as { blockId: string | null };
          setState((s) => ({ ...s, forceHoverBlockId: blockId }));
          break;
        }
