...
export function useVisualEditorParent({
  ...
  // Send force-hover state — VEQA-041. Distinct from sendHoverBlock (which
  // mirrors real mouse-over-canvas hover for the selection outline): this
  // drives the block's `:hover` CSS preview while the user's mouse is over a
  // Content-tab control instead of the canvas.
  const sendForceHoverBlock = useCallback(
    (blockId: string | null) => {
      if (!iframeReady) return;
      sendToIframe(iframeRef.current, PARENT_MESSAGES.FORCE_HOVER_BLOCK, {
        blockId,
      } satisfies ForceHoverBlockPayload);
    },
    [iframeReady],
  );
