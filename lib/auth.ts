...
// and doesn't leak account existence via response latency.
const TIMING_EQUALIZER_HASH = '$2b$10$OtxfnXvKYGpnkCxYOT2M6O1WrUKjv.4ejq7qptJ1AQdG.OH/UnVue'; // pragma: allowlist secret (dummy hash, not a credential)
...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  providers: [
    Credentials({
      ...
      async authorize(credentials, request) {
        ...
        // Equalize timing: run a bcrypt compare even when the account is
        // missing/inactive so response latency doesn't reveal which emails are
        // registered + active (the fast no-bcrypt path was an enumeration oracle).
        if (!user || !user.active) {
  ...
  cookies: {
    sessionToken: {
      name: USE_SECURE_COOKIES ? '__Secure-authjs.session-token' : 'authjs.session-token', // pragma: allowlist secret (cookie name, not a secret)
      options: {
        httpOnly: true,
        sameSite: 'lax',
        path: '/',
    ...
    async jwt({ token, user, account, profile }) {
      ...
      if (token.sub && Date.now() - last > REVALIDATE_MS) {
        ...
        try {
          ...
          // Stamp checkedAt anyway so the fail-open window stays bounded to the
          // intended REVALIDATE_MS throttle — otherwise a sustained DB outage
          // would re-hit the DB on *every* request (thundering herd) and leave a
          // deactivated user's session open for the whole outage, not just 60s.
          token.checkedAt = Date.now();
        }
      }
      return token;
