...
/**
 * QAD-044: stamp `sd_visitor` from middleware. The public site renders in a
 * Server Component, where Next 16's cookie store is read-only — so
 * `ensureVisitorId()`'s `.set()` silently threw and the cookie was NEVER
 * persisted, re-bucketing every visitor on each request (A/B assignment not
 * sticky). Middleware owns a writable response, so it stamps the cookie here
 * on the public-render responses. Idempotent: no-op when a valid id is present.
 */
export function ensureVisitorCookie(req: NextRequest, res: NextResponse): void {
  const existing = req.cookies.get(VISITOR_COOKIE)?.value;
  if (existing && isValidVisitorId(existing)) return;
  res.cookies.set({
    name: VISITOR_COOKIE,
    value: randomUUID(),
    httpOnly: true,
