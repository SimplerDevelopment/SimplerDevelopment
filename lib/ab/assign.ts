...
export function normalizeSplit(split: AbVariantSplit): AbVariantSplit {
  ...
  const total = entries.reduce((sum, [, w]) => sum + w, 0);
  if (total <= 0) return {};
  // QAD-044: largest-remainder rounding so the parts always sum to exactly 100.
  // Plain per-key Math.round drifts (three equal variants → 33+33+33 = 99),
  // persisting a split that never covers the full [0,100) bucket range.
  const parts = entries.map(([key, w]) => {
    const exact = (w / total) * 100;
    const floor = Math.floor(exact);
    return { key, floor, frac: exact - floor };
  });
  let remainder = 100 - parts.reduce((sum, p) => sum + p.floor, 0);
  // Hand the leftover whole units to the largest fractional remainders first.
  ...
  for (const p of parts) out[p.key] = p.floor; // preserve original key order
  return out;
}
