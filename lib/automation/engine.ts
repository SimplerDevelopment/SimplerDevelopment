 ...
 */
// UAG-002: cap each substituted event value so a huge attacker-controlled field
// (e.g. from a public lead-capture form) can't be templated wholesale into a
// tool param. Bounds abuse without altering normal-length content.
const MAX_TEMPLATE_VALUE_CHARS = 10_000;
...
function resolveTemplate(value: unknown, payload: Record<string, unknown>): unknown {
  if (typeof value === 'string') {
    return value.replace(/\{\{event\.([^}]+)\}\}/g, (_match, path: string) => {
      ...
      return current != null ? String(current).slice(0, MAX_TEMPLATE_VALUE_CHARS) : '';
    });
  }
...
async function executeAction(
  ...
  try {
    // UAG-002: for a rule flagged requiresApproval, stage its HIGH-RISK actions
    // for human review instead of executing unattended. executePortalTool's gate
    // only defers isApprovalRequired tools, so benign actions still run directly.
    // Synthetic session ctx (no API key → keyId null); stageOrApply reads only
    // client.id / userId / keyId / requireCmsApproval off it.
    const gate: PortalMcpContext | undefined = requiresApproval
      ? {
          userId,
...
export async function runRule(
  ...
  for (const action of actions) {
    const result = await executeAction(action, rule.clientId, userId, payload, rule.createdBy ?? null, rule.id, (rule.scopes ?? []) as string[], rule.requiresApproval);
    results.push(result);
    if (result.error) {
