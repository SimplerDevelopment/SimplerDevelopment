...
export interface BlockRegistryEntry {
  ...
  emailOnly?: boolean;
  /**
   * VEQA-067 Strategy A — when true, the type stays registered and fully
   * renderable (existing `posts.content` blocks of this type are never
   * rewritten) but is hidden from the "Add Block" picker so authors are
   * steered to its replacement instead. Picker surfaces
   * (`NestedBlockInserter`, `VisualBlockEditor`, `VisualEditorShell`) filter
   * `legacy === true` entries out of the list they render.
   */
  legacy?: boolean;
}
...
export const BUILT_IN_BLOCK_TYPES: BlockRegistryEntry[] = [
  ...
  { type: 'hero', label: 'Hero', icon: 'view_carousel', category: 'Components', description: 'Hero section with CTA', legacy: true },
  ...
  { type: 'cta', label: 'Call to Action', icon: 'campaign', category: 'Components', description: 'CTA section', legacy: true },
  { type: 'hero-cta', label: 'Hero CTA', icon: 'view_carousel', category: 'Components', description: 'Unified hero/CTA section — choose a full-bleed hero or a centered banner layout' },
