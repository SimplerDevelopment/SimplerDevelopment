...
export function createDefaultBlock(
  ...
  switch (type) {
    ...
    case 'button':
      return { ...base, type: 'button', text: 'Click me', url: '', variant: 'primary', size: 'md', alignment: 'center', style: { maxWidth: '250px' } };
    case 'quote':
    ...
    case 'gallery':
      return {
        ...base, type: 'gallery',
        images: [
          { id: `gi-${Date.now()}-1`, url: '', alt: '' },
          { id: `gi-${Date.now()}-2`, url: '', alt: '' },
          { id: `gi-${Date.now()}-3`, url: '', alt: '' },
        ],
        layout: 'grid', columns: 3, lightbox: true, gap: 'md',
      };
    ...
    case 'hero-cta':
      // VEQA-067 Strategy A — unified type for new blocks. Defaults to the
      // 'hero' look; authors can toggle to 'banner' in the editor panel.
      return {
        ...base, type: 'hero-cta', layout: 'hero',
        title: 'Your Headline Here',
        subtitle: 'A short, compelling line that builds on the headline.',
        description: 'Describe the value you offer in one or two sentences. This is placeholder copy — replace it with your own words.',
