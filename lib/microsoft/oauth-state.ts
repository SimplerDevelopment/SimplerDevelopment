...
/**
 * The OAuth state parameter is opaque to Microsoft but critical to us — it
 * carries the trust we need to know which client/user initiated this flow.
 * Microsoft passes it back unchanged, so anything signed at /connect time
 * arrives at /callback intact.
 *
 * Format: base64url(payload).base64url(hmac_sha256(payload))
 *   payload = JSON({ clientId, userId, surfaces, nonce, expiresAt })
 *
 * HMAC keyed by OAUTH_STATE_SECRET (env, shared with the Google flow). Forging
 * a state requires the secret; tampering with the payload invalidates the HMAC.
 *
 * TTL is intentionally short (10 min) — OAuth flows complete in <2 min in
 * practice, and a stale state replay should be rejected.
 *
 * Forked from lib/google/oauth-state.ts. The two are byte-identical except for
 * the surface type. If a third provider lands, generalize to a single
 * lib/oauth-state.ts with surfaces typed as readonly string[].
 */
