 ...
 *
 * UAG-005: every tool result is passed through `sanitizeToolResult` before it
 * re-enters the model context — parity with the streaming/non-streaming chat
 * routes, which sanitize inline. The AI SDK would otherwise serialize the raw
 * result, so a `get_*` tool returning portal data with an embedded token/secret
 * would reach the model unredacted on this (currently inbound-email) path.
 */
  ...
  for (const t of tools) {
    set[t.name] = tool({
      ...
      execute: async (input: unknown) => {
        const result = await execute(t.name, (input ?? {}) as Record<string, unknown>);
