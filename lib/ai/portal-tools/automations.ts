...
export const automationHandlers: Record<string, AutomationHandler> = {
  ...
  create_automation: async (input, clientId, userId) => {
    ...
    const [rule] = await db.insert(automationRules).values({
      ...
      // UAG-002: rules the assistant authors default to requiring approval for
      // their high-risk actions — an injected chat can't self-arm autonomous writes.
      source: 'ai', createdBy: userId, requiresApproval: true,
    }).returning();
