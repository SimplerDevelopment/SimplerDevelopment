...
- **Call `checkAiPlanGate` before making AI calls on behalf of a client (call sites keep calling it).** Post-BYOK-inversion, it currently returns `{ allowed: true }` unconditionally for every tier — it is a pass-through / extension point, not an active blocker (Starter no longer requires BYOK; BYOK is a Scale-only opt-in gated elsewhere, at the key-storage route and in `resolveClientApiKey`). Keep calling it at new call sites so future per-tier/per-provider gating can hook in without touching every caller. Source: `plan-gate.ts`.
...
- **AI is never the source of truth (enforced live + async as of project 198).** The *asynchronous* pipelines route through the human-review queue (`brainAiReviewItems`): meeting extractions (`lib/brain/process-meeting.ts`) and CRM auto-classifications. The *live conversational* agents now also gate: the Brain agent's `brain_create_note`/`brain_create_task` set `needsReview: true` so agent-authored notes/tasks are flagged for human review (queryable via `listNotes`/`listTasks` `needsReview`), not committed as canonical truth (UAG-004); the Portal AI assistant's high-risk writes stage for approval via `stageOrApply` when the client opts in (`clients.ai_chat_requires_approval`) or the credential requires it (UAG-003); and AI-authored automations (`source='ai'`) gate high-risk actions (UAG-002). Never add a NEW auto-commit path for AI output without a review or approval step.
