...
export function registerEmailTools(server: McpServer, ctx: PortalMcpContext): void {
  ...
  hasScope(ctx.scopes, 'email:delete') && server.registerTool(
    'email_lists_delete',
    {
  ...
  hasScope(ctx.scopes, 'email:write') && server.registerTool(
    ...
    async ({ id, scheduledAt, unschedule }) => {
      ...
      // GATE (write-approval matrix: scheduling commits the campaign to a send
      // that the cron later dispatches without a human — so scheduling IS the
      // gated decision). Stage for approval when the credential requires it.
      const result = await stageOrApply({
        ctx,
        entityType: 'email_campaign',
        operation: 'schedule',
        entityId: id,
        summary: `Schedule campaign #${id} for ${when.toISOString()}`,
        payload: { id, scheduledAt: when.toISOString() },
        apply: async () => {
          const [row] = await db.update(emailCampaigns)
