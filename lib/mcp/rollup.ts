...
export async function runDailyRollup(opts?: { day?: Date; daysAgo?: number }): Promise<RollupResult> {
  ...
  // postgres.js 3.x can't serialize Date instances in db.execute(sql`...`) —
  // Buffer.byteLength() rejects them. Pass an ISO string and cast in SQL.
  ...
  const result = await db.execute(sql`
    ...
    FROM mcp_tool_calls
    WHERE created_at >= ${dayIso}::timestamp
      AND created_at < (${dayIso}::timestamp + interval '1 day')
