 ...
 *   - approvals_reject
 *
 * All require the `approvals:manage` scope. Separation of duties (QAD-049): the
 * user/credential that STAGED a change cannot approve it (scope alone is not a
 * boundary — a `*` key holds write + approvals:manage). See ./self-approval.
 */

import type { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { isSelfApproval } from './self-approval';
import { z } from 'zod';
...
export async function applyPendingChange(change: typeof mcpPendingChanges.$inferSelect, clientId: number, applierUserId: number) {
  ...
  switch (key) {
    ...
    case 'email_campaign:schedule': {
      const id = change.entityId!;
      const { scheduledAt } = change.payload as { scheduledAt: string };
      const [existing] = await db.select({ id: emailCampaigns.id, status: emailCampaigns.status })
        .from(emailCampaigns)
        .where(and(eq(emailCampaigns.id, id), eq(emailCampaigns.clientId, clientId))).limit(1);
      if (!existing) throw new Error('Campaign not found');
      if (existing.status !== 'draft' && existing.status !== 'scheduled') {
        throw new Error(`Cannot schedule — current status is ${existing.status}`);
      }
    ...
    case 'crm_deal:delete': {
      // Re-check tenancy on replay — never trust the stored payload.
      const id = change.entityId!;
      ...
      await db.delete(crmDeals).where(and(eq(crmDeals.id, id), eq(crmDeals.clientId, clientId)));
      return { success: true, id };
    }
...
export function registerApprovalToolsOnSdk(server: McpServer, ctx: PortalMcpContext) {
  ...
  hasScope(ctx.scopes, 'approvals:manage') && server.registerTool(
    ...
    async ({ id, note }) => {
      ...
      // Separation of duties (QAD-049): the staging identity can't self-approve.
      if (isSelfApproval(change, ctx)) return json({ error: 'Separation of duties: a change cannot be approved by the same user or credential that requested it. Approve it from the portal or have a different reviewer approve.' });
      try {
