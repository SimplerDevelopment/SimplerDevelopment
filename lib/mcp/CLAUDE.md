...
- `tools/resources.ts` — read-only MCP **resources** (context docs, not tools): `blocks://schema`, `brand://default`, `catalog://services`, `portal://capabilities`. Tenant-scoped resources gate registration on `hasScope` just like tools; unscoped ones (block schema, capabilities) carry no tenant data. Resources are an enhancement for capable clients — never the *only* path to a capability. No MCP **prompts** are registered yet (capability not declared in `server.ts`).
