  ...
  async handle(req, ctx, params) {
    ...
    if (parsed.data.outcome === 'succeeded') {
      ...
      if (result.kind === 'research-brief') {
        ...
        // discovered page, deduped on (clientId, sourceUrl). No per-run
        // result table — the notes themselves are the artifact — so
        // resultId stays null. Stats serialise into the run's logTail so
        // operators can see what the batch did from the Runs UI.
