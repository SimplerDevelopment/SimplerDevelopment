...
export async function fetchCimdDocument(url: string): Promise<CimdDocument | null> {
  try {
    ...
    // SSRF guard: this fetch is reachable pre-authentication (the /oauth/authorize
    // decision resolves a URL client_id before the session check), so an attacker
    // could point it at internal HTTPS services. Reject private/loopback/link-local
    // targets and re-check at fetch time (DNS-rebind guard). Throws → caught below.
