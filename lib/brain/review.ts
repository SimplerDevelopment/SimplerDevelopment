...
import {
  ...
  type BrainReviewItemCommitmentPayload,
  type BrainReviewItemRelationshipUpdatePayload,
  type BrainReviewItemComplianceWarningPayload,
} from '@/lib/db/schema';
...
export async function approveReviewItem(args: ApproveItemArgs): Promise<ApproveItemResult> {
  const result = await db.transaction(async (tx) => {
    ...
    // Inherit the source meeting's CRM links (tenant-scoped) so review-created
    // entities surface on the relationship page. Used by task/commitment/
    // compliance_warning/relationship_update sinks.
    const inheritMeetingLinks = async (): Promise<{ companyId: number | null; dealId: number | null }> => {
      if (item.sourceType !== 'meeting') return { companyId: null, dealId: null };
      const [m] = await tx.select({ companyId: brainMeetings.companyId, dealId: brainMeetings.dealId })
        .from(brainMeetings)
        .where(and(eq(brainMeetings.id, item.sourceId), eq(brainMeetings.clientId, args.clientId)))
        .limit(1);
      return { companyId: m?.companyId ?? null, dealId: m?.dealId ?? null };
    };
    ...
    switch (item.proposedType) {
      ...
      case 'commitment': {
        const cp = payload as BrainReviewItemCommitmentPayload;
        if (!cp.what) throw new Error('commitment: missing what');
        const links = await inheritMeetingLinks();
        const due = cp.when ? new Date(cp.when) : null; // 'when' is free-form ("Friday") — NaN guard required
        const [task] = await tx.insert(brainTasks).values({
          clientId: args.clientId,
          meetingId: item.sourceType === 'meeting' ? item.sourceId : null,
          companyId: links.companyId,
          dealId: links.dealId,
          title: `${cp.who ? cp.who + ': ' : ''}${cp.what}`.slice(0, 500),
          description: [`Commitment${cp.who ? ` by ${cp.who}` : ''}`, cp.when ? `When: ${cp.when}` : null].filter(Boolean).join('\n'),
          ownerId: null,
          status: 'open',
          priority: 'medium',
      ...
      case 'compliance_warning': {
        const wp = payload as BrainReviewItemComplianceWarningPayload;
        if (!wp.message) throw new Error('compliance_warning: missing message');
        const links = await inheritMeetingLinks();
        const [task] = await tx.insert(brainTasks).values({
          clientId: args.clientId,
          meetingId: item.sourceType === 'meeting' ? item.sourceId : null,
          companyId: links.companyId,
          dealId: links.dealId,
          title: `Compliance: ${wp.message}`.slice(0, 500),
          description: wp.message,
          ownerId: null,
          status: 'open',
          priority: wp.severity === 'high' ? 'urgent' : wp.severity === 'medium' ? 'high' : 'medium',
          source: item.sourceType === 'meeting' ? 'meeting' : 'ai_suggestion',
          createdByAi: true,
          needsReview: false,
          complianceFlag: true,
      ...
      case 'relationship_update': {
        const rp = payload as BrainReviewItemRelationshipUpdatePayload;
        if (!rp.field || !rp.value) throw new Error('relationship_update: missing field or value');
        const links = await inheritMeetingLinks();
        const [note] = await tx.insert(brainNotes).values({
          clientId: args.clientId,
          title: `Relationship update: ${rp.field}`.slice(0, 255),
          body: [rp.value, rp.rationale ? `Rationale: ${rp.rationale}` : null].filter(Boolean).join('\n\n').slice(0, 50_000),
          meetingId: item.sourceType === 'meeting' ? item.sourceId : null,
          companyId: links.companyId,
          dealId: links.dealId,
        ...
        resultEntityType = 'brain_note';
        resultEntityId = note.id;
        break;
      }
      // Acknowledge-only: 'note' and 'follow_up' (and any unknown type) are
      // marked approved without creating a target record. 'note' items come
      // from the playbook runner (lib/brain/playbook-runs.ts); wiring them is
      // out of scope. commitment/relationship_update/compliance_warning now
      // create real entities above.
      case 'note':
      case 'follow_up':
      default:
