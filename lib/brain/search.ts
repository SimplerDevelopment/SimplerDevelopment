...
export async function searchBrain(
    ...
    types.has('note')
      ? db.select({
          ...
          .where(and(
            ...
            isNull(brainNotes.deletedAt), // trashed notes must not surface in search (QAD-037 review #4)
            or(
  ...
  // Semantic note hits come from brain_embeddings, which the soft-delete path
  // does not eagerly purge — so drop any that point at a trashed note before
  // merging, mirroring the lexical branch's deletedAt filter (QAD-037 review #4).
  let semanticNoteHits = semanticByType.get('note') ?? [];
  if (semanticNoteHits.length > 0) {
    const liveRows = await db.select({ id: brainNotes.id }).from(brainNotes)
      .where(and(
        eq(brainNotes.clientId, clientId),
        inArray(brainNotes.id, semanticNoteHits.map((h) => h.id)),
        isNull(brainNotes.deletedAt),
      ));
    const liveIds = new Set(liveRows.map((r) => r.id));
    semanticNoteHits = semanticNoteHits.filter((h) => liveIds.has(h.id));
  }
