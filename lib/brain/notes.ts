...
interface ListOpts {
  ...
  meetingId?: number;
  pinnedOnly?: boolean;
  /** Filter by the AI-review flag — `true` surfaces agent-created notes awaiting
   *  human review (UAG-004), mirroring the tasks queue. */
  needsReview?: boolean;
  search?: string;
  tag?: string;
  /**
...
function buildNoteFilters(clientId: number, opts: ListOpts) {
  ...
  if (opts.needsReview !== undefined) conds.push(eq(brainNotes.needsReview, opts.needsReview));
...
interface CreateNoteInput {
  ...
  /** Flag the note for human review rather than committing it as canonical truth.
   *  Live agent-created notes (brain_create_note) set this true (UAG-004). */
  needsReview?: boolean;
