...
export async function analyzeAttachment(att: AttachmentLike, clientId?: number): Promise<AttachmentAnalysis | null> {
  ...
  if (isText(att.contentType)) {
    ...
    prompt = `${userText}\n\n${delimitUntrusted('file_content', text)}`;
  } else {
