...
export async function advanceRun(
  ...
  return db.transaction(async (tx) => {
    ...
    // Chain forward from already-terminal (completed/skipped) NON-branch steps
    // whose successors haven't spawned yet. completeStep()/skipStep() — and the
    // wait-drainer cron via completeStep() — mark a task/wait step terminal and
    // then call advanceRun; but the branch-only loop above never touches those,
    // so without this their nextStepKeys would never spawn and the run would
    // stall after the first manually-completed step. spawn()'s visited-set +
    // terminal-existing guard (an already-active/completed successor returns
    // early) make these calls idempotent, so re-running advanceRun is safe.
    const terminalRunSteps = await tx.select().from(brainPlaybookRunSteps)
      .where(and(
        eq(brainPlaybookRunSteps.clientId, clientId),
        eq(brainPlaybookRunSteps.runId, runId),
        inArray(brainPlaybookRunSteps.status, ['completed', 'skipped']),
      ));
    for (const rs of terminalRunSteps) {
      const step = stepsById.get(rs.stepId);
      if (!step || step.kind === 'branch') continue; // branches handled above
      for (const nk of step.nextStepKeys ?? []) {
        const next = stepsByKey.get(nk);
        if (next) await spawn(next);
      }
