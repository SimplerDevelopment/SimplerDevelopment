...
import { delimitUntrusted, UNTRUSTED_DATA_SYSTEM_RULE } from '@/lib/ai/untrusted';
...
function buildUserPrompt(note: NoteRow, hints: PrefillHints): string {
  ...
  const lines: string[] = [];
  // UAG-005: title/body are untrusted (may be scraped/imported) — fence them so an injected
  // body can't pose as classification instructions (e.g. forcing status=canonical/confidence=1).
  lines.push(`Title: ${delimitUntrusted('note_title', note.title)}`);
  ...
  lines.push('Body:');
  lines.push(delimitUntrusted('note_body', body || '(empty)'));
  lines.push('');
...
function parseClassification(raw: string, noteId: number): NoteClassification {
  ...
  // UAG-005 review-gate clamp: the classifier must never self-anoint the canonical RAG
  // anchor from untrusted note content — an injected/scraped body could otherwise force
  // status=canonical to bypass the human-review routing (see the confidence note in the
  // system prompt). Canonical is a human-confirmed promotion, so downgrade any
  // model-proposed 'canonical' to 'draft'. Fencing lowers the injection odds; this
  // deterministic clamp closes the promotion path regardless of prompt outcome.
  const status: NoteStatusSlug = validated.status === 'canonical' ? 'draft' : validated.status;
