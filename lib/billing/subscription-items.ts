...
/**
 * A desired item as a Checkout line priced against an INLINE product
 * (product_data — Stripe creates the product at session time). Fallback for
 * instances whose Stripe account doesn't contain the catalog's pre-provisioned
 * product ids (self-hosters with their own Stripe account).
 */
export function toNamedCheckoutLineItem(
  item: DesiredItem,
  name: string,
