...
async function setup() {
  ...
  const email = 'postcaptain@simplerdevelopment.com';
  const companyName = 'Post Captain Consulting';
  const siteName = 'Post Captain Website';
  ...
  // 1. Create user
  const hashedPassword = await hash('postcaptain-temp-2026', 10);
  const [user] = existingUser
    ? [existingUser]
    : await db.insert(users).values({
        name: 'Post Captain Admin',
        email,
  ...
  const [client] = await db.insert(clients).values({
    ...
    notes: 'Migrated from postcaptain.com - Slate CRM consulting for higher education. Platinum Preferred Partner.',
  }).returning();
  ...
  const [website] = await db.insert(clientWebsites).values({
    clientId: client.id,
    name: siteName,
    domain: 'postcaptain.com',
    subdomain,
    description: 'Slate consulting services for higher education - implementations, projects, support, portals, and audits.',
    active: true,
