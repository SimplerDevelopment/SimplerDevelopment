/**
 * Mobile Terms and Conditions (post id 825) — iteration 1.
 *
 * Biggest visual gap vs cardiff.co/mobile-terms-and-conditions/: the original
 * hero is a TALL (~450px) light-to-pale-blue gradient band (slate → periwinkle)
 * with ONLY a left-aligned "Mobile Terms and Conditions" h1 in white — no
 * subtitle, no CTAs — followed by a narrow deep-blue breadcrumb bar
 * ("Home / Mobile Terms and Conditions"). The port currently shows a deep-navy
 * marketing hero with subtitle + two CTAs ("Apply Now" / "Talk to a Specialist"),
 * which is wrong tone for a legal page.
 *
 * Fix: replace the `hero-mobile-terms-and-conditions` section block with a
 * single html-render block that renders both pieces — the tall gradient band
 * + the breadcrumb strip — matching the established legal-page pattern used
 * for Privacy Policy.
 *
 * Idempotent: re-runs replace block by id (`hero-mobile-terms-and-conditions`).
 */
...
async function main() {
  ...
  const POST_ID = 825;
  const BLOCK_ID = 'hero-mobile-terms-and-conditions';
...
`.trim();
  ...
  const heroBlock = {
    ...
    html: HERO_HTML,
    fields: [
      { name: 'title', label: 'Page title', type: 'text' as const, default: 'Mobile Terms and Conditions' },
      ...
      { name: 'homeUrl', label: 'Home URL', type: 'url' as const, default: '/' },
      {
        ...
        type: 'text' as const,
        default: 'Mobile Terms and Conditions',
      },
    ],
    values: {
      ...
      homeLabel: 'Home',
      homeUrl: '/',
      currentLabel: 'Mobile Terms and Conditions',
    },
  };
