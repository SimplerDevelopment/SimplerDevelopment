/**
 * Privacy Policy (post id 827) — iteration 1.
 *
 * Biggest visual gap vs cardiff.co/privacy-policy/: the original hero is a
 * TALL (~450px), light-to-pale-blue gradient band (slate → periwinkle) with
 * ONLY a left-aligned "Privacy Policy" h1 in white — no subtitle, no CTAs —
 * followed by a narrow deep-blue breadcrumb bar ("Home / Privacy Policy").
 * The port currently shows a short dark-blue centered hero with a subtitle
 * AND two buttons ("Apply Now" / "Talk to a Specialist"), which is wrong
 * tone for a legal page and the wrong shape/color.
 *
 * Fix: replace the `hero-privacy-policy` section block (index 0) with a
 * single html-render block that renders both pieces — the tall gradient
 * band + the breadcrumb strip — in a brand-true Cardiff style.
 *
 * Idempotent: re-runs replace block by id (`hero-privacy-policy`).
 */
...
async function main() {
  ...
  const POST_ID = 827;
  const BLOCK_ID = 'hero-privacy-policy';
...
`.trim();
  ...
  const heroBlock = {
    ...
    html: HERO_HTML,
    fields: [
      { name: 'title', label: 'Page title', type: 'text' as const, default: 'Privacy Policy' },
