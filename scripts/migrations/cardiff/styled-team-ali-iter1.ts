/**
 * Replace the Ali Irani Tehrani bio page (post 831) top section with a clean
 * white 2-column "team-bio" hero matching cardiff.co/about/ali-irani-tehrani/.
 *
 * Original layout: photo left (~280px), name + green role label + multi-paragraph
 * bio right, on a white background under the global nav. No giant blue gradient.
 *
 * Idempotent: detects an existing `team-bio-hero` html-render block and
 * updates it in place; otherwise replaces blocks[0] and drops the orphan
 * "Managing Partner" section at blocks[1] if it's still there.
 */
...
const BIO_HTML = `<p>Ali Irani Tehrani is the Chief Technology Officer at Cardiff, where he leads the firm&rsquo;s people and operations. A lifelong entrepreneur who launched his first venture at just 13, Ali applies a founder&rsquo;s mindset to architecting systems that scale the company.</p>
<p>He successfully built Cardiff&rsquo;s off-shoring capabilities, a critical component of its operational strategy. Prior to Cardiff, Ali founded and led a large third-party electronics specialty company.</p>`;
...
async function main() {
  ...
  if (existingIdx >= 0) {
    ...
    const orig1IsManagingPartnerLine =
      orig1 &&
      orig1.type === 'section' &&
      JSON.stringify(orig1).toLowerCase().includes('managing partner');

    parsed.blocks[0] = newBlock;
    if (orig1IsManagingPartnerLine) {
      parsed.blocks.splice(1, 1);
      console.log('Removed orphan "Managing Partner" paragraph section');
    }
