/**
 * Privacy Policy (post id 827) — iteration 2.
 *
 * Iter 1 rebuilt the hero band. Remaining gap: body section background is
 * light gray (#f6f9fc); cardiff.co/privacy-policy/ uses pure white. Legal
 * pages on cardiff.co also don't carry a final CTA — drop the trailing
 * `final-cta` block so the page ends naturally after the policy body.
 *
 * Idempotent:
...
async function main() {
  ...
  const POST_ID = 827;
  const HERO_ID = 'hero-privacy-policy';
