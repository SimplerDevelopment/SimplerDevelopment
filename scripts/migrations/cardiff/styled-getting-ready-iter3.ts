/**
 * Iteration 3: Replace the 3-column card-grid (sec-4-grid-3) inside the
 * "Have these ready to make the application process as smooth as possible"
 * section on post 803 with a flat, styled bulleted list matching the iter2
 * pattern (orange Material `check` icon + Raleway title + Open Sans body,
 * single column).
 *
...
async function main() {
  ...
  const POST_ID = 803;
  const PARENT_SECTION_ID = 'sec-4';
  const TARGET_BLOCK_ID = 'sec-4-grid-3';
  const NEW_BLOCK_ID = 'sec-4-list-3-iter3';
...
`.trim();
  ...
  const newBlock = {
    ...
    values: {
      items: [
        {
          title: 'Check if you have a UCC filing on your business',
          description: '',
        },
        {
          title: 'Make sure your Secretary of State filing is up to date',
          description: 'Sole proprietors do not need this.',
        },
        {
          title: 'Be aware of any liens or judgments on your business',
          description: '',
        },
      ],
    },
  ...
  if (existing.id === NEW_BLOCK_ID) {
    console.log(`Post ${POST_ID}: iter3 list already applied; refreshing in place.`);
