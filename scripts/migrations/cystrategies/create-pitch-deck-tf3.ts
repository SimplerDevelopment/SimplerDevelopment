...
/**
 * Creates the CY Strategies "TF1 Deck v8" pitch deck (slug: pitch-deck-2).
 *
 * Pure deck — the qualifier flow lives in pitch-deck-3 (TF2 Qualifier v4),
 * created by create-pitch-deck-tf2.ts. The CTA slide here links out to that
 * standalone qualifier rather than embedding the survey inline.
 *
 * The shared `cy-strategy-qualifier` survey is still upserted by this script
 * because pitch-deck-3 reuses it.
 *
...
/* Kill the component's space-y-8 rhythm — we set spacing per-element below */
.deck-root .slide-stage:not([data-slide-id]) > .w-full.flex.flex-col > .max-w-3xl > * + * {
  margin-top: 0 !important;
}

/* Survey-title badge → s-eyebrow (drop the icon, restyle the text) */
...
function buildSlides() {
  // Pure TF1 deck — the qualifier survey lives in pitch-deck-3.
  return [
    buildCoverSlide(),
    buildHowIThinkSlide(),
    buildStrategySlide(),
...
async function main() {
  ...
  // ── Upsert pitch deck ───────────────────────────────────────────────
  // surveyId is unused now (deck no longer embeds the survey), but the
  // upsert above still creates it because pitch-deck-3 reuses it.
  void surveyId;
