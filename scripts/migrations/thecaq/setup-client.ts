...
async function setup() {
  ...
  const email = 'caq@simplerdevelopment.com';
  const companyName = 'Center for Audit Quality';
  const siteName = 'CAQ Website';
  ...
  // 1. Create user
  const hashedPassword = await hash('caq-temp-2026', 10);
  ...
  const [client] = await db.insert(clients).values({
    ...
    company: companyName,
    phone: '',
    website: 'https://www.thecaq.org',
    notes: 'Migrated from thecaq.org - Center for Audit Quality',
  }).returning();
  ...
  const [website] = await db.insert(clientWebsites).values({
    ...
    domain: 'thecaq.org',
    subdomain,
    description: 'The Center for Audit Quality (CAQ) is an autonomous public policy organization dedicated to enhancing investor confidence and public trust in the global capital markets.',
    active: true,
