...
async function setup() {
  ...
  const companyName = 'Delco Counseling & Therapy';
  const siteName = 'Delco Counseling Website';
  ...
  // 1. Create user
  const hashedPassword = await hash('noraanger-temp-2026', 10);
  ...
  const [client] = await db.insert(clients).values({
    ...
    company: companyName,
    phone: '610-364-5743',
    website: 'https://noraanger.com',
    address: '200 North Monroe Street, Media, PA 19063',
    emailPrefix: 'noraanger', // enables noraanger@simplerdevelopment.com inbound mail gateway
    notes: 'Migrated from noraanger.com - Licensed Professional Counselor specializing in trauma, grief, anxiety, and depression.',
  }).returning();
  ...
  const [website] = await db.insert(clientWebsites).values({
    clientId: client.id,
    name: siteName,
    domain: 'noraanger.com',
    subdomain,
    description: 'Licensed Professional Counselor in Delaware County, PA. Specializing in trauma, grief, anxiety, depression. In-person & virtual sessions.',
    active: true,
