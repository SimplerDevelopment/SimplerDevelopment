...
# setup-written .env.local already points DATABASE_URL at localhost:5432 (the
# published db port), so no env changes are needed.
#
...
echo "→ infra up: db :5432 · mailpit :8025 · realtime :3030 · minio :9000 (console :9001) (app + agents containers stopped)"
