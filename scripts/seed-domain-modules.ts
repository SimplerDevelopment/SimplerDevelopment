...
export { seedDomainModules };

// Run directly (`bun scripts/seed-domain-modules.ts`) — also importable from
// seed-dev.ts, which seeds the catalog automatically when it's empty.
if (import.meta.main) {
  seedDomainModules()
    .then(() => process.exit(0))
    .catch((err) => {
