...
- **Human-facing sequential IDs must be DB-atomic and tenant-scoped.** Ticket #, invoice #, deck/post slug, any per-tenant counter: reject `SELECT max()-then-INSERT` / count-based generation — it duplicates under concurrency. Use a DB sequence, `ON CONFLICT` / unique-constraint retry, or an atomic `UPDATE ... RETURNING`. The uniqueness scope must be keyed by `clientId`/`siteId`, not global — a ticket-number race once shipped *alongside* a cross-tenant leak in the same workflow. (distill #8)
- **Paid-module writes need the entitlement gate, not just tenant scoping.** A handler that mutates a paid module (store, esign, hosting, booking, pitch-decks) must clear `requireService`/`hasServiceAccess`/`resolveStoreSite` for that category — scope/auth alone lets an unsubscribed client use the feature free. (distill #1)
