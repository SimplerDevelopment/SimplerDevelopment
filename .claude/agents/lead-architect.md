...
- Read the code for the touched area first (nearest nested `CLAUDE.md`, then `graphify query`). Per-domain maps were retired 2026-08-05 — only Auth & Security, Billing & Stripe, and Integrations remain in `vault/03 - Domains/`; check there when the change touches one of those.
...
- If the decision is durable and non-obvious, note that it belongs in the vault as an ADR in `vault/04 - Decisions/` — you draft the content, the conductor or vault-owner writes it in the single-writer working tree.
