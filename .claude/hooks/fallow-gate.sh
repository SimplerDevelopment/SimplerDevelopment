...
# `npx` mis-resolves the platform binary on darwin-x64), then `npx` as a fallback.
if command -v fallow >/dev/null 2>&1; then
  ...
  echo "fallow-gate: fallow binary not found (tried PATH, bunx, npx), skipping audit." >&2
  exit 0
fi
...
# Scope to the commit's own diff (--base HEAD = staged/working changes), and
# grandfather pre-existing debt via saved baselines so only NET-NEW issues fail.
# Baseline flags are added only when the files exist (graceful fallback to the
# default new-only attribution if a baseline is missing).
PROJECT_DIR="${CLAUDE_PROJECT_DIR:-$(pwd)}"
AUDIT_ARGS=(audit --base HEAD --format json --quiet --explain)
for pair in "dead-code-baseline:dead-code.json" "health-baseline:health.json" "dupes-baseline:dupes.json"; do
  bl_file="$PROJECT_DIR/fallow-baselines/${pair##*:}"
