...
| Selection overlay | `components/visual-editor/SelectableBlock.tsx` (589 lines) | Blue outline, resize handles, spacing handles, context menu, drag handle |
...
| Block type defs | `types/blocks/` (barrel + 9 category leaves) | Every block shape lives here; `index.ts` re-exports them and holds the `Block` union |
...
1. **Open the editor** in Chrome DevTools: navigate to `http://localhost:3000/portal/websites/<siteId>/posts/<postId>/edit` (or localhost).
...
Adding a new **block type** has its own sub-flow and is out of scope for this skill — use the `simplerdev-block-type` skill, which moves all five integration points (interface, render component, registry entry, production renderer case, `/api/blocks` metadata) in lockstep. Hand-rolling a block has missed at least one of the five every time it has been tried.
...
- `types/blocks/` — block type defs (barrel; interfaces split by category)
