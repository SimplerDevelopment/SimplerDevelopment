...
// A schemeless url is resolved against the plan file's directory, so plans can
// ship next to their fixtures instead of hardcoding someone's home directory.
if (!/^[a-z][a-z0-9+.-]*:/i.test(plan.url)) {
  plan.url = pathToFileURL(path.resolve(path.dirname(planPath), plan.url)).href;
}
