...
**Do not create client-prefixed blocks** (no `acme-hero`, `example-stats`, etc.). Blocks ship to every client's editor — a client-specific block bloats the picker and creates technical debt. Any client-prefixed blocks already in the codebase are technical debt and must not be extended.
...
- Example: `example.co` → `example@simplerdevelopment.com`
...
Common block patterns for home pages — read `types/blocks/` for the full type definitions:
...
| Block type definitions | `types/blocks/` |
...
**Script:** `.claude/skills/site-migration/scripts/lighthouse-compare.ts`  
...
**Script:** `.claude/skills/site-migration/scripts/visual-compare-pages.ts`  
