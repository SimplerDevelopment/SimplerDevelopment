...
Any existing client-prefixed blocks are technical debt. Do not create more client-prefixed blocks. If the user insists, escalate.
...
| # | File | What to add |
|---|---|---|
| 1 | `types/blocks/<category>.ts` | `interface <PascalName>Block extends BaseBlock` in the matching category leaf (layout/content/media/form/commerce/components/dynamic), then add the type to the `Block` union in `types/blocks/index.ts` |
...
- `types/blocks/components.ts` — the `StatsBlock` interface
- `components/blocks/render/StatsBlockRender.tsx` (component, 59 lines)
- `lib/visual-editor/registry.ts` — its `BUILT_IN` entry
- `app/api/blocks/route.ts` — its metadata entry
...
- **No client name prefixes.** Reject any client-prefixed names (e.g. `AcmeX`, `ClientNameX`). If the user insists, escalate.
