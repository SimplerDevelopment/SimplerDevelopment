...
2. **Move it to In Progress** with `kanban_move_card` — before the first edit, not after.
3. **Leave the trail on the card, not in chat.** `kanban_card_add_comment` for what you found, what you decided, and what you ran (branch name, PR link, gate output, the file/line that turned out to be the real cause). A reviewer reading only the card should be able to reconstruct the work. Tick checklist items with `kanban_checklist_update` as you go.
4. **On completion** — run the right gate (`bun test:tenancy` after any data-access change, `bun test:critical` before declaring done), comment the result, then move the card: **Validating** if it's code-complete but unmerged/unverified by a human, **Shipped** once merged and confirmed.
5. **Then clear or compact context.** A card moved to Validating or Shipped is a natural context boundary — the card now holds the state, so the transcript doesn't need to. Compact (or `/clear`) before pulling the next card rather than carrying a finished ticket's context into it.
...
- **Ticket SKUs (always):** every card/ticket on a project or QA board carries a stable, unique **SKU prefix** at the start of its title, e.g. `` `PUX-081` `` (master board 153). Scheme: a short board abbreviation + zero-padded sequential number (`PUX-###`). The SKU *is* the card's identity: `kanban_create_card` never populates the native `cards.number`, so agent-created cards have no `KEY-12` key of their own (see the comment in `app/api/portal/cards/route.ts`). Assign once and **never renumber or reuse** — the SKU travels with the card across lanes. Reference tickets by SKU in commits, PRs, and notes. When adding new cards, continue the sequence from the board's current max.
