...
// Plugin iframes are served from a plugin host — a sibling subdomain of the
// apex today (self-host overrides via PLUGIN_HOST_ORIGIN). Without this entry,
// promoting CSP to enforcing would break every plugin iframe (QAD-043). When
// plugin hosts move to a separate registrable domain (QAD-043 follow-up), point
// PLUGIN_HOST_ORIGIN at that domain.
const PLUGIN_HOST_ORIGIN = process.env.PLUGIN_HOST_ORIGIN ?? 'https://*.simplerdevelopment.com';
...
const CSP_REPORT_ONLY = [
  ...
  `frame-src 'self' https://js.stripe.com https://hooks.stripe.com https://www.youtube.com https://player.vimeo.com https://calendly.com ${PLUGIN_HOST_ORIGIN}`,
