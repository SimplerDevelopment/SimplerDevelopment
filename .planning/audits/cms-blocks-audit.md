...
*No NEW user-decision items surfaced this batch.* The pre-existing nested-inserter / `createDefaultBlock` duplication (items #8 and #11 from batch-1) was deliberately left untouched per orchestrator guidance, with the following observations on what remains blocked behind that decision:

- **All three nested block-type lists are stale and will keep drifting.** `ColumnsBlockPreview.tsx` and `TabsBlockPreview.tsx` each hardcode a 22-block `blockTypes` array (lines ~48–70 and ~19–41 respectively), and the universal block picker (`BUILT_IN_BLOCK_TYPES` in `VisualEditorShell.tsx`) currently exposes ~47 user-pickable blocks. So a user adding e.g. a `bento-grid` or `team-flip-grid` inside a column will not see those options in the column's nested inserter. **Blocked by:** decision #8 (extract shared `<NestedBlockInserter />` vs route to global picker vs accept the drift).
- **Three independent `createDefaultBlock` factories in these files.** Same code, three copies (columns/tabs both have one in this file; accordion adds items via inline `addItem` not via the factory). They all support the same ~22 types and would each silently accept any new block type in the picker only if updated in lockstep. **Blocked by:** decision #11 (consolidate into `lib/blocks/defaults.ts`).
