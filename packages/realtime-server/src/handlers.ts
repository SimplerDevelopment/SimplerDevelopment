...
export class DocRoom {
  ...
  applyExternalUpdate(update: Uint8Array, origin: string): void {
    // MCP fan-out updates are authoritative REPLACEMENTS of the entity's
    // blocks/slides. The incoming `update` is the encoded state of a FRESH
    // Y.Doc holding only the new content (see internal-publisher's
    // publishBlocksUpdate/publishSlidesUpdate), so its ops are insert-only with
    // no causal link to whatever this room's doc already holds. A bare
    // Y.applyUpdate therefore MERGES additively onto any stale content — a room
    // seeded by an editor tab that loaded before this write — duplicating the
    // array (existing + new). Clear the canonical arrays first, in the SAME
    // transaction, so the apply is a true replace and peers see one atomic
    // swap. Only the entity's own array is populated (post→blocks, deck→slides,
    // email→blocks); the length guard skips the irrelevant one. /internal/apply
    // is MCP-fan-out-only, so every update here is replace-intent — normal peer
    // sync goes through the y-protocols message path, not this method.
    this.doc.transact(() => {
      for (const name of ['blocks', 'slides'] as const) {
        const arr = this.doc.getArray(name);
