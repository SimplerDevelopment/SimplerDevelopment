...
During the migration this was encountered as soon as the portal was first opened for a newly provisioned client (clientId=<id>). The fix (inserting a default `store_settings` row) was incorporated into `setup-client.ts`, making it part of the idempotent provisioning step.
...
`scripts/migrations/<client>/setup-client.ts` demonstrates the pattern.
