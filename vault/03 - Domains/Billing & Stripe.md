...
| `lib/onboarding/module-segments.ts` (412) | Segment registry keyed by domain key. Rich segments for all 12 catalog domains (per-domain onboarding mission, DOB-008..014) with detect keys + pre-credited first steps; generic fallback for unknown keys. Consumed by the wizard steps, dashboard checklist, and DomainGetStarted cards. |
...
| `lib/billing/activate-modules.ts` (125) | `activateModuleSubscription()`: the single writer for module-checkout activation, shared by the Stripe webhook and the verify-on-return route (OBQA-014). Advisory-locked transaction (`pg_advisory_xact_lock(74014, clientId)`) upserts `client_services`, stamps `stripeCustomerId`/`trialUsedAt`, and grants month-one credits only on a genuine activation transition (insert always; reactivation outside a 20-day window). `grantMonthlyCredits` must stay outside the transaction — pool max=1. See [[ADR verify-on-return-module-activation]]. |
...
Admin billing override columns (hand-applied migration; `bun run db:generate` was blocked by meta-snapshot drift at the time of implementation; see [[ADR admin-billing-overrides-comp-coupon]]):
...
| `app/api/portal/billing/modules/checkout/route.ts` (215) | POST | Create Stripe Checkout session (`mode=subscription`). Accepts `slugs[]` — builds `price_data` line items: one per module at its post-volume-discount price, plus an "Additional seats" line at `min(M,$30) × extra seats` when extra seats exist. `trial_period_days: 14` (card required, $0 today). Trial granted only once per client via `clients.trial_used_at`. `metadata.type='module_subscription'`; `success_url` carries `session_id={CHECKOUT_SESSION_ID}` for verify-on-return; activation rows are written by `activateModuleSubscription` (webhook + verify-session). Coupon mechanism removed — volume discount is now baked into the per-module line item price. |
| `app/api/portal/billing/modules/verify-session/route.ts` (105) | POST | Verify-on-return activation (OBQA-014): retrieves the Checkout Session (`expand: subscription`), requires `metadata.clientId` to match the active client AND the live subscription to be active/trialing (a completed session is a permanent token — replaying it must not restore cancelled modules), then calls `activateModuleSubscription`. Webhook is the backstop. |
...
**Webhook idempotency:** The main webhook dispatches on `session.metadata.type`; invoice updates use `db.update` by id (safe to re-run); `module_subscription` activations route through the shared advisory-locked `activateModuleSubscription()` (`lib/billing/activate-modules.ts`) so redeliveries and the verify-on-return path can't duplicate rows or double-grant credits. The legacy single-service branch still uses the old unguarded inline upsert + unconditional grant (known follow-up, noted on OBQA-014).
...
| `tests/unit/billing/activate-modules.test.ts` | `activateModuleSubscription`: insert/reactivate/replay orderings, transition-gated credit grants, 20-day dedupe, grant-after-commit ordering |
| `tests/unit/api-billing-verify-session.test.ts` | verify-session route: tenancy 403 on clientId mismatch, live-subscription replay guard, metadata parsing |
...
0. **Stripe is optional-but-loud, never silently broken (2026-07-08).** `STRIPE_SECRET_KEY` is deliberately NOT required (self-host / local dev / demo instances must run without a Stripe account; `lib/stripe/index.ts` lazy singleton must not throw at import time). The onboarding payment step is **mandatory** — no per-step skip, and the wizard-level "Skip setup" hides from the payment step onward — but on a Stripe-less instance it degrades to a warning + "Continue without payment": `POST /api/portal/billing/modules/checkout` detects the missing key **server-side** and grants the selected modules as active `client_services` rows (same mechanism as the dev seed) instead of creating a Checkout Session (`{ data: { bypassed: true } }`). A client can never force the bypass where Stripe is configured. The CLI setup wizard offers an opt-in Stripe section (keys + 3 webhook secrets → `.env.local`); full self-serve checkout additionally needs catalog rows with price IDs (`scripts/create-tier-stripe-products.ts` + catalog re-seed).

1. **Scope/auth gating ≠ entitlement gating — both are required on paid-module writes (2026-06-24).** Scope (`hasScope` / `requireScope`) verifies key/role permission. Entitlement (`requireService(clientId, category)` for MCP; `authorizePortal({ requireService })` / `hasServiceAccess` for REST) verifies an active subscription. Either gate alone is insufficient; a client without a subscription could otherwise mutate paid-module data. The canonical REST helper for store routes is `resolveStoreSite` in `lib/portal-auth.ts` (299), which wraps `resolveClientSite` + `hasServiceAccess('store')`. Three deliberately ungated routes (`store/stripe/test`, `store/stripe-connect/*`, `store/easypost/test`) are asserted on an allow-list in `tests/unit/paid-module-entitlement-guard.test.ts` (85). MCP store write tools gate via `requireStore()` in `lib/storefront/mcp-sdk-adapter.ts` (947). See [[ADR paid-module-entitlement-vs-scope-gating]].
...
10. **`booking` vs `bookings` category mismatch locked out paying subscribers (OBQA-021, PR #80, pending merge — all CI green incl. tenancy + critical e2e).** The billed catalog row seeded by `scripts/seed-domain-modules.ts` uses `category: domain.key`, which is `'bookings'` (plural, the `FEATURE_DOMAINS` key) — but all 42 booking/gift-certificate write routes and MCP tools call `requireService(clientId, 'booking')` (singular). `hasServiceAccess`'s strict `s.category === category` equality (see gotcha 1 above) meant a subscribed client's `'bookings'` row never matched a `'booking'` check: 403 + upsell instead of access. Fixed with a `serviceCategoryMatches` alias inside `lib/portal-auth.ts`, applied at the same single choke point that already covers REST `authorizePortal({ requireService })` and MCP `requireService` — rather than flipping the 42 call-site literals from `'booking'` to `'bookings'`, which would have broken the legacy dev seed (`scripts/seed-services.ts`) that still mints the singular `'booking'` category row. Unit-locked in `tests/unit/lib/` (new spec, ships with PR #80). See [[ADR booking-bookings-category-alias-at-entitlement-gate]].
