...
sources:
  ...
  - lib/surveys/aggregate-results.ts
---
...
| `app/api/portal/surveys/[id]/variants/[variantId]/route.ts` | Portal single-variant GET/PATCH/DELETE |
| `app/api/portal/surveys/[id]/webhooks/[webhookId]/route.ts` | Portal single-webhook GET/PUT/DELETE |
| `app/api/portal/surveys/[id]/email-sequences/[sequenceId]/route.ts` | Portal single-sequence PUT/DELETE |
...
| GET/PUT/DELETE | `/api/portal/surveys/[id]` | Session | Full record including `recommendation`/`scoringConfig` |
...
| GET/PATCH/DELETE | `/api/portal/surveys/[id]/variants/[variantId]` | Session | Single-variant read/update/delete |
| GET/PUT/DELETE | `/api/portal/surveys/[id]/webhooks/[webhookId]` | Session | Single-webhook read/update/delete |
| PUT/DELETE | `/api/portal/surveys/[id]/email-sequences/[sequenceId]` | Session | Single-sequence update/delete |
...
- `app/portal/surveys/new/page.tsx` — creation wizard; pre-populates field sets from `SURVEY_TEMPLATES` in `lib/survey-templates.ts` (6 built-in categorised templates)
...
| `tests/unit/aggregateSurveyResults.test.ts` | unit | `aggregateSurveyResults` — per-field response aggregation |
| `tests/unit/route-survey-*.test.ts` (×6) | unit | Public API route handlers |
| `tests/unit/components-survey-*.test.tsx` (×6) | unit | Portal UI components |
| `tests/integration/api/surveys/crud.test.ts` | integration | Portal CRUD API |
| `tests/integration/api/surveys/responses.test.ts` | integration | Response listing API |
| `tests/integration/surveys/QuestionEditor.test.tsx` | integration | Editor component integration |
| `tests/integration/surveys/QuestionTypePicker.test.tsx` | integration | Type-picker integration |
...
- **Draft → active requires approval.** `surveys_create` and `surveys_update` (MCP) mint an approval URL (`lib/mcp/approval-links.ts`). The public `/s/<slug>` route returns 403 for any survey whose `status != 'active'`. Direct portal PUT to `status='active'` bypasses approval — the MCP path enforces it.
