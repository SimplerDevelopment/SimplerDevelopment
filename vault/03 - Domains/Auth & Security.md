...
| `lib/security/password-policy.ts` | `validatePasswordStrength` (NIST-aligned strength: length + common-password blocklist + identifier guard) / `isPasswordReused` / `nextPasswordHistory` — shared by signup, reset, change |
| `lib/security/safe-callback-url.ts` | `safeCallbackUrl(raw)` — validates a post-login redirect target stays same-origin (blocks the `/\evil.com` backslash open-redirect); shared by `lib/auth.ts` + `app/portal/login` |
...
|---|---|---|
| `users` | `id`, `email`, `password` (bcrypt), `role`, `active`, `inviteToken`, `passwordResetToken`, `passwordHistory` (jsonb, last 5 bcrypt hashes), `mfaEnabled`, `totpSecret`, `defaultClientId` | Global users; `active=false` triggers immediate session invalidation on next re-validation. `passwordHistory` backs reset/change reuse rejection (migration `9005_password_history_manual.sql`) |
...
| `app/oauth/register/route.ts` | `{ip}:oauth-register` | 10 / 15 min |
| `app/api/portal/settings/mfa/setup/route.ts` | `mfa-setup:{userId}` | 10 / 15 min |
| `app/api/portal/settings/mfa/verify-and-enable/route.ts` | `mfa-verify:{userId}` | 10 / 15 min |
| `app/api/portal/settings/mfa/disable/route.ts` | `mfa-disable:{userId}` | 5 / 15 min |
...
- **Password policy is shared, not per-endpoint** (`lib/security/password-policy.ts`). Signup, reset, and change all call `validatePasswordStrength` + `isPasswordReused`; reset/change roll `passwordHistory` forward via `nextPasswordHistory` (last 5). Don't re-implement a length check inline — route through the helper (AUTH79-002/003).
- **Post-login redirect targets go through `safeCallbackUrl`** (`lib/security/safe-callback-url.ts`) — a string-prefix check is insufficient (`/\evil.com` resolves off-site). Any new place that consumes a `callbackUrl` must use this helper (AUTH79-007).
- **Every MFA-mutating endpoint is rate-limited by userId** (setup/verify-and-enable/disable) — an authed endpoint that brute-checks a code or password needs a per-account cap, not just IP (AUTH79-008).
- **Known open auth gaps (July 2026 audit):** consent scopes are NOT enforced on the `/api/portal/**` REST surface (AUTH79-011), MFA is bypassable via the Google path (AUTH79-012), and the apex-pinned session cookie + tenant custom-JS is a cross-subdomain CSRF surface (AUTH79-013). Tracked as AUTH79-### on portal project 172.
...
- `docs/guides/TWO_FACTOR_AUTH.md` — TOTP 2FA flow + how to test it
