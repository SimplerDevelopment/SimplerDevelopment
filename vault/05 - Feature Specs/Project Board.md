...
- [ ] Billing revenue-leak fixes: failed-payment dunning + retry, and customer self-serve billing portal (`billingPortal.sessions.create`) `[from E2E audit — active revenue leak]` — see [[Billing Stripe E2E Audit]]
- [ ] CRM email engagement: two-way Gmail/Outlook sync + sequences/cadences; pipeline rename/delete API (PUT/DELETE `/crm/pipelines/[id]`); signed→onboarded lifecycle `[from E2E audit]` — see [[CRM E2E Audit]]
- [ ] Bookings scheduling depth: reschedule flow, external-calendar free/busy check, SMS reminders `[from E2E audit]` — see [[Bookings Services E2E Audit]]
- [ ] E-sign assurance: signer identity verification (OTP/KBA) + automated reminder nudges for pending approvals `[from E2E audit]` — see [[ESign Approvals E2E Audit]]
- [ ] Pitch-deck sharing: viewer analytics, link access control (password/expiry), publish approval gate, deck fork (`decks_fork`) `[from E2E audit]` — see [[Pitch Decks Product Designer E2E Audit]]
- [ ] Company Brain ingestion + ACL: auto-ingest connectors (Slack/Confluence/SharePoint) + ACL-aware cross-tenant retrieval `[from E2E audit]` — see [[Company Brain AI E2E Audit]]
- [ ] Email automation depth: branching/drip journeys, deliverability testing (inbox preview/spam score), embedded list-growth forms, approval-vs-send gate `[from E2E audit]` — see [[Email Campaigns E2E Audit]]
- [ ] CMS content modeling: reference/relational fields, scheduled post auto-publish (cron wiring), post + block-template fork `[from E2E audit]` — see [[CMS Blocks E2E Audit]]
- [ ] Sites publishing safety: true staging env + publish-to-prod, automated backup on publish, auto-rollback on failed publish `[from E2E audit]` — see [[Sites Hosting Publishing E2E Audit]]
- [ ] Storefront conversion: abandoned-cart recovery, automatic tax calculation, wallet checkout (Apple/Google Pay) `[from E2E audit]` — see [[Storefront Commerce E2E Audit]]
- [ ] Portal-REST parity (currently MCP-only): storefront review moderation, customer messages, survey fork as portal routes `[from E2E audit]` — see [[Storefront Commerce E2E Audit]] · [[Surveys E2E Audit]]
- [ ] Surveys in-form fields: payment field + e-signature field `[from E2E audit]` — see [[Surveys E2E Audit]]
- [ ] Plugins ecosystem: install/uninstall lifecycle + extension marketplace browse/install `[from E2E audit]` — see [[Plugins Extension E2E Audit]]
- [ ] Public developer surface: outbound webhook delivery + headless content-delivery API `[from E2E audit]` — see [[Integrations E2E Audit]]
- [ ] A/B testing statistical rigor: sequential / valid-peeking stats + sample-ratio-mismatch (SRM) guardrail `[from E2E audit]` — see [[AB Testing E2E Audit]]
- [ ] Projects/Tickets reporting: burndown/velocity + SLA/CSAT on tickets `[from E2E audit]` — see [[Projects Tickets Kanban E2E Audit]]
- [ ] Visual editor advanced authoring: in-canvas AI section generation + scroll/timeline interaction blocks `[from E2E audit]` — see [[Visual Editor E2E Audit]]
- [ ] Small API completeness: GET `/automations/[id]`; DELETE + PATCH-rename `/ai/conversations/[id]`; Agentic OS agent-action audit trail `[from E2E audit]` — see [[00 - E2E Audit Index]]
