...
- [ ] Bookings scheduling depth: reschedule flow + Google-Calendar free/busy pre-check + SMS reminder channel `[verified 2026-06-21 real-gap]` `[2026-06-22: DESIGN DRAFTED — P1 reschedule local; P2 freeBusy=Google; P3 SMS=Twilio; found dead booking.rescheduled event]` — see [[Spec - Bookings Scheduling Depth]] · [[Bookings Services E2E Audit]]
...
- [ ] Email automation depth: branching/drip journeys + deliverability testing (inbox/spam) + embedded list-growth forms `[verified 2026-06-21; approval-vs-send gate already shipped]` `[2026-06-22: DESIGN DRAFTED — P1 journeys (reuse CRM-sequence enrollment/advance) local; P2 deliverability external; P3 signup forms local]` — see [[Spec - Email Branching Journeys]] · [[Email Campaigns E2E Audit]]
...
- [ ] Storefront conversion: abandoned-cart recovery + automatic jurisdiction-based tax calculation `[verified 2026-06-21; wallet checkout already shipped via Stripe PaymentElement]` `[2026-06-22: DESIGN DRAFTED — P1/P2 abandoned-cart local (+fix dead order.placed event); P3 tax=Stripe Tax/table]` — see [[Spec - Storefront Conversion]] · [[Storefront Commerce E2E Audit]]
...
- [ ] Public developer surface: API-key auth on the headless content API `[2026-06-22: site-level outbound webhooks SHIPPED (d90587fd)]` `[2026-06-22: ⚠SECURITY HOLES FIXED (57abb226) — keys now hashed at rest (was plaintext) + v1 surface now requires a key (was open). REMAINING: per-key scope enforcement + portal key-management UI (spec Phase 1 follow-ups)]` — see [[Spec - Public Developer API-Key Auth]] · [[Integrations E2E Audit]]
...
- [ ] Agentic OS: agent-action audit trail `[verified 2026-06-21; GET /automations/[id] + ai-conversations DELETE/PATCH shipped this session]` `[2026-06-22: DESIGN DRAFTED — agent_action_logs + runId correlation, instrument wrapRegisterTool choke point, link to mcp_pending_changes; all local]` — see [[Spec - Agentic OS Audit Trail]] · [[00 - E2E Audit Index]]
