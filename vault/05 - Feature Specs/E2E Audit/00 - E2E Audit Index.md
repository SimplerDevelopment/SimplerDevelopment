...
- [ ] OPEN (real bug, route-smoke): `/portal/tools/pitch-decks/[id]/presenter` throws `TypeError: Cannot read properties of undefined (reading 'blocks')` on load (presenter view crashes on a deck/slide shape) — needs a focused fix in the pitch-deck presenter component
