...
- [x] RESOLVED: visual workflow builder API (list/create/patch/delete/test-run/runs/templates) + legacy rules CRUD covered — gap-automations-coverage.spec.ts
...
- [ ] GAP (no implementation): Scope-gated action denial: rule without required scope produces scope_denied log entry, not action execution — PARTIAL: scope auto-derivation + auth guards covered in gap-automations-coverage.spec.ts; scope_denied log path unit-tested (tests/unit/automation-scope-gate.test.ts), no REST trigger to e2e it
