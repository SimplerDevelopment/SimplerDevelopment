...
> **REVERSED 2026-07-09 (OBQA-003, [[ADR demo-seed-vs-onboarding-detections]]):** the pre-seeded demo workspace was removed — its sample rows falsely pre-satisfied the quick-setup onboarding detections. Signups now start empty; the agent-led "set up YOUR business" flow carries activation from cold. The paragraph below is the original (now-reversed) strategy, kept for context.
