...
**Per-model cost rates — SHIPPED (2026-06-24):** `lib/ai/evals/cost.ts` — `MODEL_RATES` (USD/MTok per Claude model, from the published pricing), a `SUITE_MODEL` static map (suite→model, kept in sync with each core's `MODEL` const; the framework doesn't record the model per run, so a map rather than per-core plumbing — kept dependency-free so the cost path doesn't import `@/lib/db`), and `estimateCostUsd(suiteId, in, out)` with a Sonnet blended fallback. `job.ts` now prices the rollup via `estimateCostUsd` instead of the hardcoded blend. The 4 registry suites all run Sonnet ($3/$15) = the old blended rate, so numbers are unchanged for them; the table prices Haiku/Opus correctly for the judge and future suites. Unit test `tests/unit/eval-cost.test.ts` (4 cases) pins the math + per-suite pricing. _(scheduleCron wiring, @critical promote test, dataset management, per-model cost rates — all done; nothing functional remains deferred.)_
